...
icmp_ping(const char *hostname, int count, double timeout)
{
	...
	for (int i = 0; i < count; ++i) {
		...
		if (recvfrom(sockfd, buffer, sizeof(buffer), 0,
				(struct sockaddr *)&recv_addr, &addrlen) <= 0) {
		...
		rtt += (double)(time_end.tv_sec - time_start.tv_sec) * 1000.0
			+ (double)(time_end.tv_nsec - time_start.tv_nsec) / 10;
	}

	clock_gettime(CLOCK_MONOTONIC, &tfe);
	double timeElapsed = (double)(tfe.tv_sec - tfs.tv_sec) * 1000.0
		+ (double)(tfe.tv_nsec - tfs.tv_nsec) / 1e6;
...
{
	// TODO: We need to parse pkgman repos and automatically get
	//       a central mirror listing from the infra.
	...
	char* url;
	CURLUcode result = curl_url_get(bestMirror->url, CURLUPART_URL,
		&url, CURLU_NO_DEFAULT_PORT);
	if (result != CURLUE_OK) {
