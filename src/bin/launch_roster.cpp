...
print_summary(bool target, BMessage info)
{
	...
	if (!target) {
		result = info.FindBool("service", &enabled);
		if (result != B_OK)
			goto failure;
		...
		printf("  %s%s  " COLOR_RESET COLOR_BOLD "%s" COLOR_RESET " (%s, %s, %s)\n", running ? COLOR_GREEN : launched ? COLOR_BLUE : COLOR_RED, enabled ? "✿" : "❀", name,
			service ? "Service" : "Job", enabled ? "Enabled" : "Disabled", running ? "Running" : launched ? "Idle" : "Stopped");
		return;
	}
