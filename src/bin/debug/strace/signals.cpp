...
signal_name(int signal)
{
	...
	sprintf(buffer, "%" B_PRId32, signal);
	return buffer;
}
...
signal_info(siginfo_t& info)
{
	...
	if (info.si_errno != 0) {
		string += ", si_errno=0x";
		sprintf(buffer, "%" B_PRIx32, info.si_errno);
		string += buffer;
	}

	string += ", si_pid=";
	sprintf(buffer, "%" B_PRId32, info.si_pid);
	string += buffer;
	string += ", si_uid=";
	sprintf(buffer, "%" B_PRId32, info.si_uid);
	string += buffer;
	...
	if (info.si_signo == SIGCHLD) {
		string += ", si_status=";
		sprintf(buffer, "%" B_PRId32, info.si_status);
		string += buffer;
	}

	if (info.si_signo == SIGPOLL) {
		string += ", si_band=";
		sprintf(buffer, "%" B_PRId64, (int64)info.si_band);
		string += buffer;
	}
