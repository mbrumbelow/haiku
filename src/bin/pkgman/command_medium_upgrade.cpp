...
DEFINE_COMMAND(MediumUpgradeCommand, "medium-upgrade", kShortUsage,
...
int
MediumUpgradeCommand::Execute(int argc, const char* const* argv) {
	...
	if (package->Version() >= medium.HaikuPackage.Version())
		DIE(B_NOT_ALLOWED, "cannot perform the upgrade, because the medium "
			"does not contain a newer version of Haiku");
