...
bool
ParseOffT(const char* string, off_t* value)
{
	off_t parsedValue;
	char dummy;
	...
	if (sscanf(string, "%" B_SCNdOFF "%c", &parsedValue, &dummy) == 1) {
		*value = parsedValue;
		return true;
	}
	return false;
...
main(int argc, char** argv)
{
	...
	while ((c = getopt_long(argc, argv, "ho:l:fv", kLongOptions, NULL)) != -1) {
		switch (c) {
			case 0:
				break;
			case 'o':
				if (!ParseOffT(optarg, &offset)) {
					fprintf(stderr, "%s: Invalid offset value\n", kProgramName);
					return EXIT_FAILURE;
				}
				break;
			case 'l':
				if (!ParseOffT(optarg, &length)) {
	...
	if (verbose) {
		printf("Range to trim (bytes): offset = %" B_PRIdOFF
			", length = %" B_PRIdOFF "\n", offset, length);
	}
