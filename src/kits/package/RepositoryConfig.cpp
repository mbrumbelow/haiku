...
#define KEY_IDENTIFIER_V2 "url"

#define KEY_BASE_URL_V3 "baseurl"
#define KEY_IDENTIFIER_V3 "identifier"
...
BRepositoryConfig::Store(const BEntry& entry) const
{
	...
	configString << "# This is the URL where the repository data can be "
		"accessed.\n";
	configString << KEY_BASE_URL_V3 << "=" << fBaseURL << "\n";
	configString << "\n";
	configString << "# This is an identifier for the repository that is "
		"consistent across mirrors\n";

	if (fIdentifier.IsEmpty())
		configString << "# " << KEY_IDENTIFIER_V3 << "=???\n";
	else
		configString << KEY_IDENTIFIER_V3 << "=" << fIdentifier << "\n";
	configString << "\n";
...
BRepositoryConfig::SetTo(const BEntry& entry)
{
	...
	const char *identifierKey;

	switch (versionNumber) {
		case 1:
			baseUrlKey = KEY_BASE_URL_V1;
			identifierKey = KEY_IDENTIFIER_V1;
			break;
		case 2:
			baseUrlKey = KEY_BASE_URL_V2;
			identifierKey = KEY_IDENTIFIER_V2;
			break;
		case 3:
			baseUrlKey = KEY_BASE_URL_V3;
			identifierKey = KEY_IDENTIFIER_V3;
			break;
		default:
			return B_BAD_DATA;
	}
	...
	const char* identifier = driverSettings.GetParameterValue(identifierKey);
