	...
	fFirstBootProcessing(false)
{
	...
	if (archive->FindBool("first boot processing", &fFirstBootProcessing) != B_OK)
		fFirstBootProcessing = false; // Optional field for compatibility.
...
BActivationTransaction::Archive(BMessage* archive, bool deep) const
{
	...
	if ((error = archive->AddInt32("location", fLocation)) != B_OK
		...
		|| (error = archive->AddStrings("deactivate", fPackagesToDeactivate))
			!= B_OK
		|| (error = archive->AddBool("first boot processing", fFirstBootProcessing))
			!= B_OK) {
		return error;
	}
