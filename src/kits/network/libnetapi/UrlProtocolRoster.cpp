	...
	BUrlProtocolListener* listener, BUrlContext* context)
{
	...
	result = find_directory(B_USER_NONPACKAGED_ADDONS_DIRECTORY,
		&userNonPackagedPath);
	if (result == B_OK) {
		userNonPackagedPath.Append("kernel/network/uri_handlers");
	...
	if (result == B_OK) {
		systemNonPackagedPath.Append("kernel/network/uri_handlers");
	...
	if (result == B_OK) {
		systemPath.Append("kernel/network/uri_handlers");
