...
BFtpConnection::_GetDirectoryListing()
{
	...
	char buffer[4096];
	BMemoryIO receiveStream(buffer, sizeof(buffer));
	...
	while ((received = receiveStream.Read(buffer, 4096)) > 0) {
...
BFtpConnection::_Download()
{
	...
	if (response.IsOk()) {
		...
		if (response.IsOk()) {
			BString filename = BString(fRemotePath.Path());
			filename.ReplaceAll('\\', '/');
			int32 pos = filename.FindLast('/');
			if (pos != B_ERROR)
				filename.CopyInto(filename, pos + 1,
								  filename.Length() - pos - 1);

			fLocalPath.SetTo(BString(fLocalPath.Path())
								.ReplaceAll('\\', '/').String(),
							 filename.String());
			if (fLocalPath.InitCheck() != B_OK)
...
{
	std::ifstream file(fLocalPath.Path(), std::ios_base::binary);
	if (!file)
		return BFtpResponse(B_FTP_STATUS_INVALID_FILE);

	BString filename = BString(fLocalPath.Path());
	filename.ReplaceAll('\\', '/');
	int32 pos = filename.FindLast('/');
	if (pos != B_ERROR)
