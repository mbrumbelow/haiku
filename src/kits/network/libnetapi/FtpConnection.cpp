...
BFtpResult::BFtpResult(const BUrl& url)
	:
	fUrl(url)
{
}
...
BFtpResult::BFtpResult(BMessage* archive)
	:
	BUrlResult(archive),
	fUrl(archive->FindString("ftp:url"))
{
}


BFtpResult::BFtpResult(const BFtpResult& other)
	:
	fUrl(other.fUrl)
{
}
...
void
BFtpResult::SetUrl(const BUrl& url)
{
	fUrl = url;
}
...
BFtpResult&
BFtpResult::operator=(const BFtpResult& other)
{
	if (this == &other)
		return *this;
...
status_t
BFtpResult::Archive(BMessage* target, bool deep) const
{
	status_t result = BUrlResult::Archive(target, deep);
	if (result != B_OK)
		return result;

	target->AddString("ftp:url", fUrl);
...
/*static*/ BArchivable*
BFtpResult::Instantiate(BMessage* archive)
{
	if (!validate_instantiation(archive, "BFtpResult"))
		return NULL;

	return new BFtpResult(archive);
}
