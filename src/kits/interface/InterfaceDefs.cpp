...
_init_interface_kit_()
{
	...
	BString path;
	if (get_control_look(path) && path.Length()) {
		BControlLook* (*instantiate)(image_id);

		be_control_look_addon = load_add_on(path.String());
		if (be_control_look_addon >= 0
			&& get_image_symbol(be_control_look_addon,
				"instantiate_control_look",
				B_SYMBOL_TYPE_TEXT, (void **)&instantiate) == B_OK) {
			be_control_look = instantiate(be_control_look_addon);
			if (be_control_look == NULL) {
				unload_add_on(be_control_look_addon);
				be_control_look_addon = -1;
			}
		}
	}
	if (be_control_look == NULL)
