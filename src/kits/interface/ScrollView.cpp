...
BScrollView::SetBorder(border_style border)
{
	...
	SetFlags(_ModifyFlags(Flags(), border, (Flags() & B_SUPPORTS_LAYOUT) != 0));
}
...
{
	// We pass in "layout" as a boolean rather than checking Flags()
	// because this function is called before the BView constructor is,
	// in which case Flags() would be 0.
	if (layout)
