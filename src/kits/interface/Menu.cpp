...
BMenu::AttachedToWindow()
{
	...
	// #9251: The menu is only added to the menu hierarchy
	// and made visible if the mouse is over the menu,
	// or if the user has requested the menu via the keyboard.
	// If we don't pass keydown in here,
	// keyboard navigation breaks for 'recent items' submenus
	// that are generated by Tracker related private classes
		...
		case B_RIGHT_ARROW:
			if (fLayout == B_ITEMS_IN_ROW)
				...
				if (fSelected != NULL && fSelected->Submenu() != NULL) {
					...
					// #9251: Set state so the menu tracking thread
					// and submenu "window" are aware the navigation
					// was triggered by keyboard, not by mouse.
...
BMenu::_Track(int* action, long start)
{
	...
	while (fState != MENU_STATE_CLOSED) {
		...
		if (_OverSubmenu(fSelected, screenLocation)
			...
			// #9251: To prevent segment violations, ensure a menu
			// has actually been selected and that it has a submenu.
			// Because keyboard and mouse interactions set
			// selected items differently, the menu tracking thread needs
			// to be careful in triggering the navigation to the submenu.
			if (fSelected != NULL) {
