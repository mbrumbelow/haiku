...
BMenu::MessageReceived(BMessage* message)
{
	if (message->HasSpecifiers()) {
		_ScriptingMessageReceived(message);
		return;
	}
...
void BMenu::_ScriptingMessageReceived(BMessage* message)
{
	...
	switch (propertyInfo.FindMatch(message, index, &specifier, what,
		...
		case 5: { // Mark: SET
			BMenuItem *item = Superitem();
			if (item != NULL) {
				Supermenu()->_ItemScriptingMessageReceived(message, item);
				return;
			}
			break;
		}
		case 6: // Menu: CREATE
			err = B_NOT_SUPPORTED; break;
		case 7: { // Menu: DELETE
		...
		case 8: { // Menu: *
			// TODO: check that submenu looper is running and handle it correctly
			BMenu *submenu = NULL;
			BMenuItem *item;
			err = _ResolveItemSpecifier(specifier, what, item);
			if (err >= B_OK)
			...
			if (submenu != NULL) {
				...
				submenu->_ScriptingMessageReceived(message);
				return;
			}
			break;
		}
		...
		case 13: { // MenuItem: *
			...
			if (err >= B_OK) {
				message->PopSpecifier();
				_ItemScriptingMessageReceived(message, item);
				return;
			}
		}
		default:
...
void BMenu::_ItemScriptingMessageReceived(BMessage* message, BMenuItem* item)
{
	BMessage replyMsg(B_REPLY);
	status_t err = B_BAD_SCRIPT_SYNTAX;
	int32 index;
...
status_t BMenu::_ResolveItemSpecifier(const BMessage& specifier, int32 what, BMenuItem*& item)
{
	status_t err;
	item = NULL;
	switch (what) {
