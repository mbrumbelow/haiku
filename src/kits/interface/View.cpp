...
BAffineTransform
BView::TransformTo(coordinate_space base) const
{
	if (base == B_CURRENT_STATE)
	...
	if (base == B_PARENT_VIEW || base == B_PARENT_VIEW_DRAW) {
		BView* parent = Parent();
		if (parent != NULL) {
			ConvertToParent(&origin);
			transform.TranslateBy(origin);
			if (base == B_PARENT_VIEW_DRAW)
				transform = transform.PreMultiplyInverse(parent->TransformTo(B_VIEW));
			return transform;
		}
		base = B_WINDOW;
	}
