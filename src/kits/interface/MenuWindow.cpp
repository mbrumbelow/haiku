...
private:
	friend class BMenuWindow;
			...
			void			MoveSubmenusOver(BMenu* menu, BRect menuFrame,
								BRect screenFrame);
private:
			BMenu*			fMenu;
};
...
BMenuScroller::BMenuScroller(BRect frame)
	:
	BControl(frame, "menu scroller", "", NULL, 0, B_WILL_DRAW | B_FRAME_EVENTS)
{
...
BMenuFrame::LayoutChanged()
{
	...
	// get the topmost non menu bar
	BMenu* topmost = fMenu;
	BMenu* _menu;
	while (dynamic_cast<BMenuBar*>(topmost) != NULL
		&& (_menu = topmost->Supermenu()) != NULL) {
		topmost = _menu;
	}

	// resize window to menu width
	float width = fMenu->Frame().Width();
	Window()->SetSizeLimits(2, width, 2, B_SIZE_UNLIMITED);
	ResizeTo(width, Window()->Frame().Height());

	if (fMenu == topmost) {
		// we are the topmost menu, push child menus over recursively
...
BMenuWindow::BMenuWindow(const char *name)
	...
	BWindow(BRect(0, 0, 0, 0), name, B_BORDERED_WINDOW_LOOK, kMenuWindowFeel,
		B_NOT_MOVABLE | B_NOT_ZOOMABLE | B_NOT_RESIZABLE | B_AVOID_FOCUS
			| kAcceptKeyboardFocusFlag | B_AUTO_UPDATE_SIZE_LIMITS),
	fMenu(NULL),
	fMenuFrame(NULL),
	fUpperScroller(NULL),
	fLowerScroller(NULL),
...
{
	// do default if message is NULL or what is not B_MODIFIERS_CHANGED
	// TODO are there any other message types that should work like this?
	if (message == NULL || message->what != B_MODIFIERS_CHANGED)
		return BWindow::DispatchMessage(message, handler);

	if (fMenu == NULL) // no menu, do default
		return BWindow::DispatchMessage(message, handler);

	// we have a menu and the message is for us
	BMessage* modifiersChangedMessage = DetachCurrentMessage();

	// inform all ancestor menu loopers about the change in modifiers
	BMenu* menu = fMenu;
	BMenu* _menu;
	while ((_menu = menu->Supermenu()) != NULL) {
		menu = _menu;
		if (menu->Looper() != NULL)
			menu->Looper()->PostMessage(modifiersChangedMessage, menu);
	}

	// inform our menu about the change in modifiers
...
BMenuWindow::AttachScrollers()
{
	...
	BRect screenFrame(BScreen(fMenu->Window()).Frame());
	fMenuFrame->SetExplicitSize(BSize(B_SIZE_UNSET,
		screenFrame.Height() - kScrollerHeight * 2));
	fValue = 0;
...
BMenuWindow::DetachScrollers()
{
	...
	if (fMenuFrame != NULL)
		fMenuFrame->SetExplicitSize(BSize(B_SIZE_UNSET, B_SIZE_UNSET));
}
...
BMenuWindow::_ScrollBy(const float& step)
{
	...
	// redraw the bottom area on scroll so we don't see partially drawn bottom arrows
	fMenuFrame->Invalidate(BRect(0,
		Bounds().bottom - kScrollerHeight + 1 - step * 2,
