...
BTextView::_ValidateLayoutData()
{
	...
	fLayoutData->preferred.height = _OuterHeight();
...
BTextView::_PerformAutoScrolling()
{
	...
	} else if (LineWidth(0) < _InnerWidth()) {
		// single line that fits in box, prevent horizontal scrolling
		scrollBy.x = 0;
	}
...
BTextView::_UpdateScrollbars()
{
	...
	if (horizontalScrollBar != NULL) {
		...
		long dataWidth = (long)ceilf(_OuterWidth());
	...
	if (verticalScrollBar != NULL) {
		...
		long dataHeight = (long)ceilf(_OuterHeight());
