...
{
	if (fEditable) {
		rgb_color uneditable
			= tint_color(ui_color(B_DOCUMENT_BACKGROUND_COLOR), _UneditableTint());
		if (ViewColor() == uneditable && LowColor() == uneditable
			&& HighUIColor() == B_DOCUMENT_TEXT_COLOR) {
			return true;
		}
	} else {
		if (ViewUIColor() == B_DOCUMENT_BACKGROUND_COLOR
			&& LowUIColor() == B_DOCUMENT_BACKGROUND_COLOR
			&& HighUIColor() == B_DOCUMENT_TEXT_COLOR) {
			return true;
		}
	}
