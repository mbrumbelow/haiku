...
static const float kMinScrollWidth = 30.0f;
...
BTextView::ScrollToOffset(int32 offset)
{
	...
	if (point.x < bounds.left)
		xDiff = point.x - bounds.right + kMinScrollWidth;
	else if (point.x > bounds.right)
		xDiff = point.x - bounds.left - kMinScrollWidth;
...
BTextView::_UpdateInsets(const BRect& rect)
{
	...
	bool isBounds = rect.left == bounds.left && rect.top == bounds.top
		 && rect.right == bounds.right; // not including bottom
	if (isBounds && (fEditable || fSelectable)) {
