...
#define COMPILE_PROBABLY_BAD_CODE 1
	...
	if (fAlignment != B_ALIGN_LEFT) {
		float alignmentOffset = fAlignedTextRect.Width() - LineWidth(lineNum);
		if (fAlignment == B_ALIGN_CENTER)
			alignmentOffset = floorf(alignmentOffset / 2);
...
BTextView::SetTextRect(BRect rect)
{
	...
	fAlignedTextRect = fTextRect = rect;
	_ResetTextRect();
}
...
BTextView::DoLayout()
{
	...
	fAlignedTextRect = fTextRect = bounds;
	_ResetTextRect();
}
		...
		case B_DOWN_ARROW:
		{
			if (!fEditable)
				...
				if (optionKeyDown && !commandKeyDown && !controlKeyDown)
					...
					_ScrollTo(0, fAlignedTextRect.bottom + fLayoutData->bottomInset);
...
BTextView::_Refresh(int32 fromOffset, int32 toOffset, bool scroll)
{
	...
	if (newHeight != saveHeight) {
		...
		if (newHeight < saveHeight)
			toLine = _LineAt(BPoint(0.0f, saveHeight + fAlignedTextRect.top));
		else
			toLine = _LineAt(BPoint(0.0f, newHeight + fAlignedTextRect.top));
	}
	...
	eraseRect.top = fAlignedTextRect.top + (*fLines)[fLines->NumLines()]->origin;
...
BTextView::_RecalculateLineBreaks(int32* startLine, int32* endLine)
{
	...
	*endLine = (*endLine > fLines->NumLines() - 1) ? fLines->NumLines() - 1
	...
	float width = std::max(fAlignedTextRect.Width(), 10.0f);
