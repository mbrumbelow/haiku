...
void
HaikuControlLook::DrawTabFrame(BView* view, BRect& rect, const BRect& updateRect,
	const rgb_color& base, uint32 flags, uint32 borders, border_style borderStyle,
	uint32 side)
{
	float fTabHeight = dynamic_cast<BTabView*>(view)->TabHeight();
	if (side == BTabView::kTopSide || side == BTabView::kBottomSide) {
		if (side == BTabView::kTopSide)
			rect.bottom = fTabHeight;
		else
			rect.top = rect.bottom - fTabHeight;
		...
		if (side == BTabView::kLeftSide)
			rect.right = fTabHeight;
		else
			rect.left = rect.right - fTabHeight;
