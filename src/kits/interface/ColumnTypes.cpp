...
// Return formats:
// 6 March 2024 Wednesday 22:30:15
// 6 March 2024 Wednesday 22:30
// 6 March 2024 22:30:15
// 06.03.2024 22:30:15
// 6 March 2024 22:30
// 06.03.2024 22:30
// 06.03.2024
...
BDateColumn::DrawField(BField* _field, BRect rect, BView* parent)
{
	...
	if (field->Width() != rect.Width()) {
        ...
        BDateFormatStyle dateStyles[] = {
			B_FULL_DATE_FORMAT, B_FULL_DATE_FORMAT, B_LONG_DATE_FORMAT, B_SHORT_DATE_FORMAT,
			B_LONG_DATE_FORMAT, B_SHORT_DATE_FORMAT,
		};

        BTimeFormatStyle timeStyles[] = {
			B_MEDIUM_TIME_FORMAT, B_SHORT_TIME_FORMAT, B_MEDIUM_TIME_FORMAT, B_MEDIUM_TIME_FORMAT,
...
BSizeColumn::DrawField(BField* _field, BRect rect, BView* parent)
{
	...
	if (size < 1024.0) {
		...
		format.Format(printedSize, (double)size);
		...
		if (font.StringWidth(string) > width) {
			...
			fNumberFormat.Format(printedSize, (double)size);
		...
		while (index < B_COUNT_OF(kFormats) && tempSize >= 1024.0) {
			tempSize /= 1024.0;
			index++;
		}
		...
		while (precision >= 0) {
			...
			string.SetToFormat(gSystemCatalog.GetString(kFormats[index], B_TRANSLATION_CONTEXT,
				"unit size"), printedSize.String());
				if (font.StringWidth(string) <= width)
					break;
...
BIntegerColumn::DrawField(BField *field, BRect rect, BView* parent)
{
	...
	fNumberFormat.Format(string, (int)((BIntegerField*)field)->Value());
	float width = rect.Width() - (2 * kTEXT_MARGIN);
	parent->TruncateString(&string, B_TRUNCATE_MIDDLE, width + 2);
	DrawString(string.String(), parent, rect);
}
...
GraphColumn::DrawField(BField* field, BRect rect, BView* parent)
{
	...
	fNumberFormat.FormatPercent(percentString, (double)(number / 100));
