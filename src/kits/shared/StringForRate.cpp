...
string_for_rate(double rate, char* string, size_t stringSize)
{
	...
	if (mib < 1.0) {
		if (numberFormat.SetPrecision(2) == B_OK &&
			numberFormat.Format(printedUsage, kib) == B_OK) {
	...
	if (gib < 1.0) {
		if (numberFormat.SetPrecision(2) == B_OK &&
			numberFormat.Format(printedUsage, mib) == B_OK) {
	...
	if (tib < 1.0) {
		if (numberFormat.SetPrecision(2) == B_OK &&
			numberFormat.Format(printedUsage, gib) == B_OK) {
	...
	if (numberFormat.SetPrecision(3) == B_OK && numberFormat.Format(printedUsage, tib) == B_OK) {
