...
string_for_rate(double rate, char* string, size_t stringSize)
{
	...
	if (value < 1.0) {
		...
		if (numberFormat.Format(printedUsage, (int32)rate) == B_OK) {
			...
			// FIXME: Workaround to display the idle value as 0 instead of 00
			if (rate == 0.0)
				strlcpy(string, printedUsage.String() + 1, stringSize);
			else
	...
	if (numberFormat.SetPrecision(2) == B_OK && numberFormat.Format(printedUsage, value) == B_OK) {
		snprintf(string, stringSize, gSystemCatalog.GetString(kFormats[index], B_TRANSLATION_CONTEXT,
			"unit per second"), printedUsage.String());
	} else {
		snprintf(string, stringSize, gSystemCatalog.GetString(kRawFormats[index], B_TRANSLATION_CONTEXT,
			"unit per second"), value);
	}
