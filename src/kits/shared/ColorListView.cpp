...
BColorListView::MessageReceived(BMessage* message)
{
	...
	if (message->WasDropped()) {
		...
		if (message->GetInfo(B_RGB_COLOR_TYPE, 0, &name, &type) == B_OK
			...
			uint32 command = (current
				? BColorListView::B_MESSAGE_SET_CURRENT_COLOR
				: BColorListView::B_MESSAGE_SET_COLOR);
			BMessage update = BMessage(command);
			update.AddData("RGBColor", B_RGB_COLOR_TYPE, color, size);
			// if setting different color, add which color to set
			if (current)
				update.AddBool("current", true);
			else
				update.AddUInt32("which", (uint32)item->ColorWhich());
			...
			if (messenger.IsValid()) {
				...
				if (!current) {
					// redraw item, set current color does this for us
					item->SetColor(*color);
