...
BString
string_for_frequency(int64 frequencyInHertz)
{
	BString frequencyString;
	BString printedFrequency;
	double value = frequencyInHertz;
	...
	numberFormat.SetPrecision(index > 2 ? 2 : 0);

	numberFormat.Format(printedFrequency, value);
	frequencyString.SetToFormat(
		gSystemCatalog.GetString(kFormats[index], B_TRANSLATION_CONTEXT, "frequency unit"),
