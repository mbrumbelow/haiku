...
status_t
BMemoryRingIO::_WaitForEvent(bigtime_t timeout)
{
	if (timeout == B_INFINITE_TIMEOUT) {
		pthread_cond_wait(&fEvent, &fLock);
	} else {
		...
		int err = pthread_cond_timedwait(&fEvent, &fLock, &absTimeout);
		if (err != B_OK)
			return err;
	}
...
BMemoryRingIO::WaitForRead(bigtime_t timeout)
{
	...
	while (BytesAvailable() == 0) {
		...
		status_t result = _WaitForEvent(timeout);
		if (result != B_OK)
			return result;
	}
