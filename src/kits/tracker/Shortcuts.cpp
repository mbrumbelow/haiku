...
{
	return new BMenuItem(B_TRANSLATE("Delete"), new BMessage(kDelete));
}
...
{
	return new BMenuItem(B_TRANSLATE("Edit name"), new BMessage(kEditItem), 'E');
}
...
{
	return new BMenuItem(B_TRANSLATE("Restore"), new BMessage(kRestoreFromTrash));
}
...
void
Shortcuts::UpdateCreateLinkInMenu(BMenu* menu)
{
	BMenuItem* link = menu->FindItem(kCreateLink);
	BMenuItem* relL = menu->FindItem(kCreateRelativeLink);
	if (link == NULL && relL == NULL)
		return;

	if (relL != NULL)
		link = relL;
	if (link != NULL)
		UpdateCreateLinkItem(link);
}
