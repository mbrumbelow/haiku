...
TShortcuts::UpdatePasteItem(BMenuItem* item)
{
	...
	if (fInWindow) {
		item->SetEnabled(
			IsCurrentFocusOnTextView() || (FSClipboardHasRefs() && !SelectionIsReadOnly()));
...
TShortcuts::ParentIsRoot() const
{
	...
	// check if the parent is root, if so check settings or control pressed
	BEntry entry(fContainerWindow->TargetModel()->EntryRef());
	BEntry parent;
	if (FSGetParentVirtualDirectoryAware(entry, parent) == B_OK && FSIsRootDir(&parent))
