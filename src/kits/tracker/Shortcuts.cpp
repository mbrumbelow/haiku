...
TShortcuts::UpdateArrangeByItem(BMenuItem* item)
{
	...
	if (fInWindow) {
		item->SetEnabled(PoseView()->CountItems() > 0 && (PoseView()->ViewMode() != kListMode));
...
TShortcuts::UpdateOpenParentItem(BMenuItem* item)
{
	...
	if (fInWindow) {
		item->SetEnabled(!IsDesktop() && !IsRoot() && !IsTrash() && !ParentIsRoot()
...
TShortcuts::ParentIsRoot() const
{
	...
	BDirectory parent;
	return entry.GetParent(&parent) == B_OK && parent.GetEntry(&entry) == B_OK
		&& FSIsRootDir(&entry);
}
