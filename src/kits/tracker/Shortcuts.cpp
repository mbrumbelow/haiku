...
BMenuItem*
TShortcuts::ArrangeByItem(BMenu* menu)
{
	return new BMenuItem(menu);
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? B_TRANSLATE("Clean up all") : B_TRANSLATE("Clean up");
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kCleanupAll : kCleanup;
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? B_TRANSLATE("Close all") : B_TRANSLATE("Close");
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kCloseAllWindows : B_QUIT_REQUESTED;
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? B_TRANSLATE("Copy more") : B_TRANSLATE("Copy");
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kCopyMoreSelectionToClipboard : B_COPY;
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kCreateRelativeLink : kCreateLink;
}
...
TShortcuts::CutLabel()
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? B_TRANSLATE("Cut more") : B_TRANSLATE("Cut");
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kCutMoreSelectionToClipboard : B_CUT;
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? B_TRANSLATE("Delete") : B_TRANSLATE("Move to Trash");
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kDeleteSelection : kMoveSelectionToTrash;
}
...
{
	const char* name = B_TRANSLATE("Open with" B_UTF8_ELLIPSIS);
	return new BMenuItem(name, new BMessage(kOpenSelectionWith));
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? B_TRANSLATE("Paste links") : B_TRANSLATE("Paste");
}
...
{
	return (modifiers() & B_SHIFT_KEY) != 0 ? kPasteLinksFromClipboard : B_PASTE;
}
...
TShortcuts::UpdatePasteItem(BMenuItem* item)
{
	...
	if (fInWindow) {
		bool isPastable = FSClipboardHasRefs() && !SelectionIsReadOnly();
...
{
	return fInWindow && fContainerWindow->IsDesktop();
}
...
{
	return fInWindow && fContainerWindow->IsQuery();
}
...
{
	return fInWindow && fContainerWindow->IsQueryTemplate();
}
...
{
	return fInWindow && fContainerWindow->IsRoot();
}
...
{
	return fContainerWindow->IsVirtualDirectory();
}
...
TShortcuts::ParentIsRoot() const
{
	...
	// check if root directory first
	if (IsDesktop() || IsRoot() || IsTrash() || IsQuery() || IsQueryTemplate())
		return true;

	// check if parent is root, if so check "Show disks" or control pressed
	BEntry entry(fContainerWindow->TargetModel()->EntryRef());
	if (FSGetParentVirtualDirectoryAware(entry, entry) == B_OK && FSIsRootDir(&entry))
		return !(TrackerSettings().ShowDisksIcon() || (modifiers() & B_CONTROL_KEY) != 0);
