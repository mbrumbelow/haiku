...
void
TLiveMixin::UpdateDeleteMenuItem(BMenuItem* item)
{
	...
	if ((modifiers() & B_SHIFT_KEY) != 0) {
		...
		item->SetShortcut('T', B_COMMAND_KEY | B_SHIFT_KEY);
		item->Message()->what = kDelete;
	} else {
		item->SetLabel(B_TRANSLATE("Move to Trash"));
		item->SetShortcut('T', B_COMMAND_KEY);
		item->Message()->what = kMoveToTrash;
	}
}
...
TLiveFileMenu::Update()
{
	if (Window()->LockLooper()) {
		...
		BMenuItem* trash = FindItem(kMoveToTrash);
		if (trash == NULL)
			trash = FindItem(kDelete);
		UpdateDeleteMenuItem(trash);
