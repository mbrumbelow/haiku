...
DisassemblerX8664::Init(target_addr_t address, const void* code, size_t codeSize)
{
	...
	ZydisFormatterSetProperty(&fZydisData->formatter, ZYDIS_FORMATTER_PROP_HEX_UPPERCASE, ZYAN_FALSE);
	...
	target_size_t& _size, bool& _breakpointAllowed)
{
	...
	if (!ZYAN_SUCCESS(ZydisDecoderDecodeFull(&fZydisData->decoder, buffer, fCodeSize - fZydisData->offset,
	...
	target_addr_t& _address, target_size_t& _size)
{
	...
	while (true) {
		...
		if (!ZYAN_SUCCESS(ZydisDecoderDecodeInstruction(&fZydisData->decoder,
				(ZydisDecoderContext*)ZYAN_NULL, buffer, fCodeSize - fZydisData->offset, &instruction))) {
			return B_ENTRY_NOT_FOUND;
		}
	...
	CpuState* state)
{
	...
	char string[1024];
	int written = snprintf(string, sizeof(string), "0x%016" B_PRIx64 ": %16.16s  ", address, hexString);
	char* formatted = string + written;
	if (!ZYAN_SUCCESS(ZydisFormatterFormatInstruction(&fZydisData->formatter, &instruction, operands,
		instruction.operand_count_visible, formatted, sizeof(string) - written, address, NULL))) {
