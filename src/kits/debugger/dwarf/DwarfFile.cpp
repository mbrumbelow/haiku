...
DwarfFile::_ReadAddressIndirect(BaseUnit* unit, uint64 index, uint64& value) const
{
	...
	const char *addrs = (const char*)fDebugAddressSection->Data() + addrBase;
	value = ((uint64*)addrs)[index];
	return B_OK;
}
	...
	BaseUnit* unit, DebugInfoEntry* entry, AbbreviationEntry& abbreviationEntry)
{
	...
	while (abbreviationEntry.GetNextAttribute(attributeName,
		...
		switch (attributeForm) {
			...
			case DW_FORM_strx1:
			{
				uint64 index = dataReader.Read<uint8>(0);
				const char* strValue;
				status_t res = _ReadStringIndirect(unit, index, strValue);
				if (res == B_OK)
					attributeValue.SetToString(strValue);
			...
			case DW_FORM_strx2:
			{
				uint64 index = dataReader.Read<uint16>(0);
				const char* strValue;
				status_t res = _ReadStringIndirect(unit, index, strValue);
				if (res == B_OK)
					attributeValue.SetToString(strValue);
			...
			case DW_FORM_strx3:
			{
				uint64 index = dataReader.ReadU24(0);
				const char* strValue;
				status_t res = _ReadStringIndirect(unit, index, strValue);
				if (res == B_OK)
					attributeValue.SetToString(strValue);
			...
			case DW_FORM_strx4:
			{
				uint64 index = dataReader.Read<uint32>(0);
				const char* strValue;
				status_t res = _ReadStringIndirect(unit, index, strValue);
				if (res == B_OK)
					attributeValue.SetToString(strValue);
