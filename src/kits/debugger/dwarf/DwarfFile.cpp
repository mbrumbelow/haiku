...
DwarfFile::_ResolveRangeListRngLists(CompilationUnit* unit, uint64 offset) const
{
	...
	DataReader dataReader((uint8*)fDebugRngListsSection->Data() + offset,
		fDebugRngListsSection->Size() - offset, unit->AddressSize(), unit->IsBigEndian());
	while (true) {
		...
		TRACE_RNG_LISTS("_ResolveRangeListRngLists: add range 0x%08" B_PRIxADDR 
			" - 0x%08" B_PRIxADDR "\n", start, end);
...
DwarfFile::_ResolveRangeListRanges(CompilationUnit* unit, uint64 offset) const
{
	...
	while (true) {
		...
		TRACE_RNG_LISTS("_ResolveRangeListRanges: add range 0x%08" B_PRIxADDR
			" - 0x%08" B_PRIxADDR "\n", start, end);
