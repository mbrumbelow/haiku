...
DwarfFile::_ParseLineInfo(CompilationUnit* unit)
{
	...
	if (version >= 5) {
		...
		for (int i = 0; i < dirEntryFormatCount; i++) {
		...
		for (int i = 0; i < dirCount; i++) {
			...
			for (int j = 0; j < dirEntryFormatCount; j++) {
		...
		for (int i = 0; i < fileNameEntryFormatCount; i++) {
			TRACE_LINES_ONLY(uint64 content =)
				dataReader.ReadUnsignedLEB128(0);
			TRACE_LINES_ONLY(uint64 format =)
				dataReader.ReadUnsignedLEB128(0);
		...
		for (int i = 0; i < fileNameCount; i++) {
			...
			for (int j = 0; j < fileNameEntryFormatCount; j++) {
				...
				switch (content) {
					case DW_LNCT_path:
						res = _ParseLineInfoFormatString(unit, dataReader, format, fileName);
						if (res != B_OK)
							return res;
						break;
					case DW_LNCT_directory_index:
						res = _ParseLineInfoFormatUint(unit, dataReader, format, dirIndex);
						if (res != B_OK)
							return res;
						break;
					case DW_LNCT_timestamp:
						res = _ParseLineInfoFormatUint(unit, dataReader, format, modificationTime);
						if (res != B_OK)
							return res;
						break;
					case DW_LNCT_size:
						res = _ParseLineInfoFormatUint(unit, dataReader, format, fileLength);
						if (res != B_OK)
							return res;
						break;
					case DW_LNCT_MD5:
