...
int main() {
	int fds[2];
	int domain;
	domain = AF_UNIX; // currently buggy: send hangs upon full buffer (no EBUFS, no silent drop)
 // domain = AF_INET; // SOCK_DGRAM: packets get dropped silently (send continues)
	printf("Domain: %i\n", domain);
	...
	while(true) {
		...
		// eventually: EAGAIN (on Linux), ENOBUFS (on macOS), hang (on Haiku)
		if(ret < 0) {
			perror("send");
			break;
		} else {
