...
test_remap_different_protection()
{
	...
	*addr = 'a';
	...
	status_t status = _kern_remap_memory(B_CURRENT_TEAM, (void**)&addr,
		B_EXACT_ADDRESS, B_PAGE_SIZE, B_READ_AREA | B_WRITE_AREA, 0, false,
		B_CURRENT_TEAM, info.text);

	if (status == B_OK) {
		REPORT_ERROR("remap_memory succeeded unexpectedly\n");
		return 1;
	}
	...
	status = _kern_remap_memory(B_CURRENT_TEAM, (void**)&addr, B_EXACT_ADDRESS,
		B_PAGE_SIZE, B_READ_AREA | B_WRITE_AREA, 0, true, B_CURRENT_TEAM,
		info.text);

	if (status != B_OK) {
		REPORT_ERROR("remap_memory failed: %s\n", strerror(status));
		return 1;
	}
...
test_remap_rw_rx()
{
	...
	void *rwBlock = NULL;
	status_t status = _kern_remap_memory(B_CURRENT_TEAM, &rwBlock,
...
#error "Unsupported architecture"
	...
	void *rxBlock = NULL;
	status = _kern_remap_memory(B_CURRENT_TEAM, &rxBlock, B_ANY_ADDRESS,
...
test_remap_commit()
{
	...
	*(char*)testBlock = 'a';
	...
	// Remap a large block.
	status = _kern_remap_memory(B_CURRENT_TEAM, &testBlock, B_EXACT_ADDRESS,
...
test_remap_in_place()
{
	...
	void *testBlock = (char*)memoryBlock + B_PAGE_SIZE * 3;
	status_t status = _kern_remap_memory(B_CURRENT_TEAM, &testBlock,
...
test_remap_transfer_clone_memory()
{
	...
	const char data1[] = "remap_memory can act as transfer_area, except that "
	"it operates on memory ranges, not areas. The memory range can span "
	"numerous areas or be a slice of one area.";

	const char data2[] = "remap_memory can also be a more powerful "
	"clone_area. Not only is it not limited to areas (explained above), it "
	"is also not dependent on the B_CLONE_AREA flag. remap_memory is governed "
	...
	if (pid > 0) {
		...
		status_t status = _kern_remap_memory(pid, &addressInChild,
			B_ANY_ADDRESS, B_PAGE_SIZE * 2, B_READ_AREA, 0, false,
			B_CURRENT_TEAM, memoryBlock);
		if (status != B_OK) {
			REPORT_ERROR("%i: remap_memory failed: %s\n", strerror(status));
			return 1;
		}
		...
		memoryBlock = NULL;
		status = _kern_remap_memory(B_CURRENT_TEAM, &memoryBlock,
...
test_remap_insufficient_privileges()
{
	...
	if (pid > 0) {
		...
		if (status != B_PERMISSION_DENIED) {
			REPORT_ERROR("remap_memory didn't fail: %s\n", strerror(status));
			exit(1);
		}
...
test_remap_reserved_range()
{
	...
	status = _kern_remap_memory(B_CURRENT_TEAM, &remapAddress, B_EXACT_ADDRESS,
		B_PAGE_SIZE * 2, B_READ_AREA | B_WRITE_AREA, 0, true, B_CURRENT_TEAM,
