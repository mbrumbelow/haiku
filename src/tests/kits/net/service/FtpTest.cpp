...
{
	testCaller.addThread("GetWorkingDirectoryTest", &T::GetWorkingDirectoryTest);
}
...
{
	std::string expectedResponseBody('257 "/" is the current working directory.\r\n');
	...
	BFtpConnection request(testUrl, testUrl.Protocol() == "ftp");
	...
	const BFtpDirectoryResponse &result =
		dynamic_cast<const BFtpDirectoryResponse &>(request.ResultantResponse());
	CPPUNIT_ASSERT_EQUAL(257, result.GetStatus());
	CPPUNIT_ASSERT_EQUAL(expectedResponseBody, result.GetMessage());
	CPPUNIT_ASSERT_EQUAL("/", result.GetDirectory());
