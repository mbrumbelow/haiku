...
{
	if (info.type == B_OBJECT_TYPE_FD) {
		if (info.events > 0 && (info.events & B_EVENT_WRITE) != 0)
			return EVFILT_WRITE;
		return EVFILT_READ;
	}

	// We cannot always determine the "filter" type directly.
	return 0;
}
	...
	const struct timespec *tspec)
{
	...
	const int32 errorEvents = B_EVENT_INVALID | B_EVENT_ERROR | B_EVENT_DISCONNECTED;
	for (int i = 0; i < nchanges; i++) {
		...
		switch (changelist[i].filter) {
			case EVFILT_READ:
				...
				events = B_EVENT_READ | errorEvents;
				break;

			case EVFILT_WRITE:
				waitInfo->type = B_OBJECT_TYPE_FD;
				events = B_EVENT_WRITE | errorEvents;
				break;
		...
		// We don't support ONESHOT for file descriptors, due to READ/WRITE separation.
		if (waitInfo->type == B_OBJECT_TYPE_FD && (changelist[i].flags & EV_ONESHOT) != 0) {
			__set_errno(EOPNOTSUPP);
			return -1;
		}

		if (changelist[i].filter == EVFILT_READ || changelist[i].filter == EVFILT_WRITE) {
			// kqueue treats the same file descriptor with both READ and WRITE filters attached
			// as two separate listeners. Haiku, however, treats it as one.
			// To rectify this discrepancy, we must be careful to preserve existing flags.
			...
			if (j < nchanges) {
				...
				if (status == B_OK)
					events |= (info.events & otherEvents);
			}
		}

		if ((changelist[i].flags & EV_ONESHOT) != 0)
			events |= B_EVENT_ONE_SHOT;
		...
		if (events > 0) {
			...
			for (ssize_t i = 0; i < events; i++) {
				...
				if (waitInfos[i].events < 0) {
					...
					flags |= EV_EOF;
				} else if ((waitInfos[i].events & (B_EVENT_INVALID | B_EVENT_ERROR)) != 0) {
					flags |= EV_ERROR;
					data = EINVAL;
				}
