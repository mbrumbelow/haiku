...
sort_r_cmpswap(char* __restrict itemA, char* __restrict itemB,
	size_t sizeOfElement, _compare_function_qsort_r, void* cookie)
{
	if (_compare_function_qsort_r(itemA, itemB, cookie) > 0) {
	...
	_compare_function_qsort_r, void* cookie)
{
	...
	if (numElements < 10) {
		...
		for (pivIndexA = base + sizeOfElement; pivIndexA < end;
			...
			while (pivIndexB > base
				&& sort_r_cmpswap(pivIndexB - sizeOfElement , pivIndexB,
						sizeOfElement, _compare_function_qsort_r, cookie)) {
		...
		if (_compare_function_qsort_r(pivList[0], pivList[1], cookie) > 0)
			SORT_R_SWAP(pivList[0], pivList[1], tmp);
		if (_compare_function_qsort_r(pivList[1], pivList[2], cookie) > 0) {
		...
		while (nextPivCmpItem < lastPivCmpItem) {
			...
			for (; nextPivCmpItem < lastPivCmpItem;
				...
				cmp = _compare_function_qsort_r(nextPivCmpItem, pivot, cookie);
				if (cmp > 0)
					break;
				else if (cmp == 0) {
			...
			for (; nextPivCmpItem < lastPivCmpItem;) {
				...
				cmp = _compare_function_qsort_r(lastPivCmpItem, pivot, cookie);
				if (cmp == 0) {
		...
		sort_r_simple(end - (lastPivEqualsPos - lastPivCmpItem),
			...
			_compare_function_qsort_r, cookie);
	}
}
