...
#else
	BBitmap* tmp = new BBitmap(rect, B_RGBA32);
#endif
	...
	icon_size which, BBitmap* icon)
{
	...
	if (which >= B_LARGE_ICON && nodeInfo.GetType(type) == B_OK
		&& BString(type).IStartsWith("image")) {
...
#else
		temp = new(std::nothrow) BBitmap(icon->Bounds(), B_RGBA32);
#endif
		deleter.SetTo(temp);
	...
	uint32 srcBPR, BBitmap* icon)
{
...
#ifdef HAIKU_TARGET_PLATFORM_HAIKU
		BBitmap* converted = new BBitmap(rect, B_BITMAP_NO_SERVER_LINK,
			icon->ColorSpace());
#else
		BBitmap* converted = new BBitmap(rect, icon->ColorSpace());
#endif
	...
	if (dstWidth > width && dstHeight > height
		...
		BBitmap* temp = new BBitmap(rect, B_BITMAP_NO_SERVER_LINK,
			icon->ColorSpace());
#else
		BBitmap* temp = new BBitmap(rect, icon->ColorSpace());
#endif
