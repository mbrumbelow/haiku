...
{
	if (fInverted)
		return; // this case is left unhandled currently
...
void
PerspectiveTransformer::TransformTo(BPoint leftTop, BPoint rightTop, BPoint leftBottom, BPoint rightBottom)
{
	fToLeftTop = leftTop;
...
PerspectiveTransformer::_CheckValidity()
{
	...
	// https://gitlab.gnome.org/GNOME/gimp/-/blob/a265c966/app/core/gimp-transform-utils.c#L485
	fValid = true;
	double w;
	bool positive;
