	...
	const Container<Transformer>* transformers, const Transformable* shapeTransformation)
{
	...
	// Populate the array
	StyleTransformer** styleTransformers = new StyleTransformer*[count];
	int i = 0;
	if (styleTransformation != NULL)
		styleTransformers[i++] = new Transformable(*styleTransformation);
	...
	styleTransformers[i++] = new Transformable(*shapeTransformation);

	return new CompoundStyleTransformer(styleTransformers, count);
}
