	...
	fConfig()
{
	...
	if (fOutputPreferredFormat.u.raw.audio.format == 0) {
	...
	if (fInputPreferredFormat.u.raw.audio.format == 0) {
		fInitStatus = B_BAD_VALUE;
		return;
	}
	...
	media_input* out_input)
{
	...
	if (with_format.frame_rate <= 0
		|| with_format.u.raw_audio.channel_count <= 0
		|| (with_format.u.raw.audio.format 
...
MultiAudioNode::_SetNodeOutputFrameRate(float frameRate)
{
	...
	if (frameRate <= 0
		...
		|| (fInputPreferredFormat.u.raw.audio.format
			& media_raw_audio_format::B_AUDIO_SIZE_MASK == 0))
		return B_BAD_VALUE;
