	...
	uint32 outType, BPositionIO* target)
{
	...
	system_info info;
	encoder->maxThreads = (get_system_info(&info) == B_OK) ? info.cpu_count : 1;
	...
	uint32 outType, BPositionIO* target)
{
	...
	avifResult setIOMemoryResult = avifDecoderSetIOMemory(decoder, reinterpret_cast<const uint8_t *>(streamData), streamSize);
	...
	if (convertToRGB) {
		...
		// alternatively add support for higher bit depth to Haiku bitmaps,
		// possibly with HDR too.
		if (image->depth > 8)
			return B_ILLEGAL_DATA;
