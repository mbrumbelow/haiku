	...
	fSettings(settings)
{
	...
	for (size_t i = 0; i < 4; ++i) {
		...
		BMenuItem* item = new BMenuItem(avifPixelFormatToString(pixelFormats[i]), msg);
	...
	fQualitySlider = new BSlider("quality", B_TRANSLATE("Output quality:"),
		new BMessage(kMsgQuality), AVIF_QUANTIZER_BEST_QUALITY, AVIF_QUANTIZER_WORST_QUALITY, B_HORIZONTAL, B_BLOCK_THUMB);
	...
	fSpeedSlider = new BSlider("speed", B_TRANSLATE("Compression speed:"),
		new BMessage(kMsgSpeed), AVIF_SPEED_SLOWEST, AVIF_SPEED_FASTEST, B_HORIZONTAL, B_BLOCK_THUMB);
	...
	fHTilesSlider = new BSlider("htiles", B_TRANSLATE("Horizontal tiles:"),
		new BMessage(kMsgTilesHorizontal), 1, 6, B_HORIZONTAL, B_BLOCK_THUMB);
	...
	fHTilesSlider->SetValue(fSettings->SetGetInt32(AVIF_SETTING_TILES_HORIZONTAL));

	fVTilesSlider = new BSlider("vtiles", B_TRANSLATE("Vertical tiles:"),
		new BMessage(kMsgTilesVertical), 1, 6, B_HORIZONTAL, B_BLOCK_THUMB);
	...
	fVTilesSlider->SetValue(fSettings->SetGetInt32(AVIF_SETTING_TILES_VERTICAL));
...
ConfigView::MessageReceived(BMessage* message)
{
	struct {
		...
		{ AVIF_SETTING_PIXEL_FORMAT, kMsgPixelFormat, TRAN_SETTING_INT32 },
		{ AVIF_SETTING_QUALITY, kMsgQuality, TRAN_SETTING_INT32 },
		{ AVIF_SETTING_SPEED, kMsgSpeed, TRAN_SETTING_INT32 },
		{ AVIF_SETTING_TILES_HORIZONTAL, kMsgTilesHorizontal, TRAN_SETTING_INT32 },
		{ AVIF_SETTING_TILES_VERTICAL, kMsgTilesVertical, TRAN_SETTING_INT32 },
		{ NULL }
	};
