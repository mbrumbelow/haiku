...
//	char newLabel[64];
	...
	newLabel.SetToFormat(B_TRANSLATE("Grid Height: %li"), fGridWidth->Value());
...
//	char delay[16];
	...
	if (fGridDelay->Value() <= 0)
		...
		delay.SetToFormat(B_TRANSLATE_CONTEXT(
			"Grid Life Delay: %" B_PRId32 "x",
			"This is a factor: the x represents 'times'"), fGridDelay->Value());
	}
