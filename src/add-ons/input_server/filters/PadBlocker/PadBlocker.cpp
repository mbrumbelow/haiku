...
class PadBlocker : public BInputServerFilter 
{
	...
	private:
		...
		status_t	GetSetingsPath(BPath& path);
		BPoint		fWindowPosition;
};

status_t
PadBlocker::GetSetingsPath(BPath &path)
{
...
PadBlocker::PadBlocker()
{
	...
	#endif
	BPath path;
	status_t status = GetSetingsPath(path);
	if (status == B_OK)
	{
		...
		if (status == B_OK)
		{
			(settingsFile.Read(&settings, sizeof(touchpad_settings)
				!= sizeof(BPoint)));
			{
