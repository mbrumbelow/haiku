	...
	while (get_next_team_info(&cookie, &info) == B_OK) {
		...
		if (item != NULL && isApp == item->IsApplication()) {
			...
			if (!isApp || paths.count(item->Path()->Path()) > 0) {
				...
				insert_under = fItemMap.Get(spawner_id);
				if (insert_under != NULL && insert_under->Found())
					while (insert_under != NULL && !insert_under->IsParent() && (!isApp ||
						(isApp && strcmp(insert_under->GetInfo()->name, info.name))))
						insert_under = dynamic_cast<TeamListItem*>(
							fListView->Superitem(insert_under));
			}
			...
			TeamListItem* const map_pointer = item;
			fItemMap.Put(info.team, map_pointer);
