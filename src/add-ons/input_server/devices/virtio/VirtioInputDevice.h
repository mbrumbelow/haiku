...
public:
	...
	status_t Control(const char* name, void* cookie, uint32 command,
		BMessage* message);
...
{
public:
	VirtioHandler(VirtioInputDevice* dev, const char* name,
		input_device_type type);
	virtual ~VirtioHandler();
	inline VirtioInputDevice* Device() {return fDev;}
...
{
public:
	KeyboardHandler(VirtioInputDevice* dev, const char* name);
	~KeyboardHandler();
	virtual void Reset();
	virtual status_t Control(uint32 command, BMessage* message);
...
{
public:
	TabletHandler(VirtioInputDevice* dev, const char* name);
	virtual void Reset();
	virtual status_t Control(uint32 command, BMessage* message);
	virtual void PacketReceived(const VirtioInputPacket &pkt);
