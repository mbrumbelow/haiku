...
static bool
FillTabletMessage(BMessage &msg, const TabletState &s)
{
...
TabletHandler::PacketReceived(const VirtioInputPacket &pkt)
{
	...
	case virtioInputEvSyn: {
		...
		if (fState.x != fNewState.x || fState.y != fNewState.y
			...
			if (!msg.IsSet() || !FillTabletMessage(*msg.Get(), fState))
				return;
...
static bool
IsKeyPressed(const KeyboardState &state, uint32 key)
{
...
void
KeyboardHandler::KeyString(uint32 code, char *str, size_t len)
{
	uint32 i;
	char *ch;
...
void
KeyboardHandler::StartRepeating(BMessage* msg)
{
	if (fRepeatThread >= B_OK) StopRepeating();

	fRepeatMsg = *msg;
	fRepeatThread = spawn_thread(RepeatThread, "repeat thread",
...
status_t
KeyboardHandler::RepeatThread(void *arg)
{
	status_t res;
	...
	while (true) {
		...
		if (msg.IsSet() && h->Device()->EnqueueMessage(msg) >= B_OK)
			msg.Detach();
...
KeyboardHandler::StateChanged()
{
	...
	if (IsKeyPressed(fNewState, fKeyMap->left_shift_key))
		fNewState.modifiers |= B_SHIFT_KEY   | B_LEFT_SHIFT_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->right_shift_key))
		fNewState.modifiers |= B_SHIFT_KEY   | B_RIGHT_SHIFT_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->left_command_key))
		fNewState.modifiers |= B_COMMAND_KEY | B_LEFT_COMMAND_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->right_command_key))
		fNewState.modifiers |= B_COMMAND_KEY | B_RIGHT_COMMAND_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->left_control_key))
		fNewState.modifiers |= B_CONTROL_KEY | B_LEFT_CONTROL_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->right_control_key))
		fNewState.modifiers |= B_CONTROL_KEY | B_RIGHT_CONTROL_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->caps_key))
		fNewState.modifiers ^= B_CAPS_LOCK;
	if (IsKeyPressed(fNewState, fKeyMap->scroll_key))
		fNewState.modifiers ^= B_SCROLL_LOCK;
	if (IsKeyPressed(fNewState, fKeyMap->num_key))
		fNewState.modifiers ^= B_NUM_LOCK;
	if (IsKeyPressed(fNewState, fKeyMap->left_option_key))
		fNewState.modifiers |= B_OPTION_KEY  | B_LEFT_OPTION_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->right_option_key))
		fNewState.modifiers |= B_OPTION_KEY  | B_RIGHT_OPTION_KEY;
	if (IsKeyPressed(fNewState, fKeyMap->menu_key))
