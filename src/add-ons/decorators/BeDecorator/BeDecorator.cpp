	...
	bool down, int32 width, int32 height)
{
	...
	switch (item) {
		case COMPONENT_CLOSE_BUTTON:
		{
			...
			BRegion shadow = BRegion();
			if (tab->closePressed) {
				// include main square
				shadow.Include(BRect(rect.left + 0, rect.top + 0,
					rect.left + 4, rect.top + 4));
				// include protruding square horns
				shadow.Include(BRect(rect.left + 0, rect.top + 5,
					rect.left + 0, rect.top + 5));
				shadow.Include(BRect(rect.left + 5, rect.top + 0,
					rect.left + 5, rect.top + 0));
				// exclude smaller square
				shadow.Exclude(BRect(rect.left + 3, rect.top + 3,
					rect.left + 4, rect.top + 4));
				// include top left corner of smaller square
				shadow.Include(BRect(rect.left + 3, rect.top + 3,
					rect.left + 3, rect.top + 3));
				// exclude two dots
				shadow.Exclude(BRect(rect.left + 2, rect.top + 3,
					rect.left + 2, rect.top + 3));
				shadow.Exclude(BRect(rect.left + 3, rect.top + 2,
					rect.left + 3, rect.top + 2));
			} else {
				// include main square
				shadow.Include(BRect(rect.right - 4, rect.bottom - 4,
					rect.right - 0, rect.bottom - 0));
				// include protruding square horns
				shadow.Include(BRect(rect.right - 0, rect.bottom - 5,
					rect.right - 0, rect.bottom - 5));
				shadow.Include(BRect(rect.right - 5, rect.bottom - 0,
					rect.right - 5, rect.bottom - 0));
				// exclude smaller square
				shadow.Exclude(BRect(rect.right - 4, rect.bottom - 4,
					rect.right - 3, rect.bottom - 3));
				// include bottom right corner of smaller square
				shadow.Include(BRect(rect.right - 3, rect.bottom - 3,
					rect.right - 3, rect.bottom - 3));
				// exclude two dots
				shadow.Exclude(BRect(rect.right - 2, rect.bottom - 3,
					rect.right - 2, rect.bottom - 3));
				shadow.Exclude(BRect(rect.right - 3, rect.bottom - 2,
					rect.right - 3, rect.bottom - 2));
			}

			// draw bottom right region
			sBitmapDrawingEngine->SetHighColor(buttonColorShadow1);
			sBitmapDrawingEngine->FillRegion(shadow);

			// define glint region (last)
