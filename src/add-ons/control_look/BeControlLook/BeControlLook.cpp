	...
	orientation orientation, uint32 knobStyle)
{
	...
	bool isEnabled = (flags & B_DISABLED) == 0;
	bool isScrollable = (flags & B_PARTIALLY_ACTIVATED) != 0;
	...
	const rgb_color& base, uint32 direction, uint32 flags, float tint)
{
	...
	// white triangle behind offset by 1px
	// triangle offset by a pixel
	off1.Set(tri1.x + 1, tri1.y + 1);
	off2.Set(tri2.x + 1, tri2.y + 1);
	off3.Set(tri3.x + 1, tri3.y + 1);

	view->BeginLineArray(3);
	view->AddLine(off2, off3, tint_color(base, B_LIGHTEN_MAX_TINT));
	view->AddLine(off1, off3, tint_color(base, B_LIGHTEN_MAX_TINT));
	view->AddLine(off1, off2, tint_color(base, B_LIGHTEN_MAX_TINT));
	view->EndLineArray();

	// now draw the triangel on top
	view->BeginLineArray(3);
...
BeControlLook::DrawTabFrame(BView* view, BRect& rect, const BRect& updateRect,
	const rgb_color& base, uint32 flags, uint32 borders, border_style borderStyle,
	uint32 side)
{
	rgb_color lightenmax = tint_color(base, B_LIGHTEN_MAX_TINT);
	...
	uint32 flags, const BPoint& where, const rgb_color* textColor)
{
	...
	bool isEnabled = (flags & B_DISABLED) == 0;
	bool isClicked = (flags & B_PARTIALLY_ACTIVATED) != 0;

	rgb_color invertedIfClicked = textColor != NULL ? *textColor
