...
void
BeControlLook::DrawTab(BView* view, BRect& rect,
	const BRect& updateRect, const rgb_color& base, uint32 flags,
	uint32 borders, uint32 side, tab_position position, bool full)
{
	if (position == B_TAB_FRONT)
		DrawActiveTab(view, rect, updateRect, base, flags, borders, side);
	else {
		_DrawInactiveTab(view, rect, updateRect, base, flags, borders, side,
			position, full);
	}
}
	...
	uint32 flags, const BPoint& where, const rgb_color* textColor)
{
	...
	bool isButton = (flags & B_IS_BUTTON) != 0;
	bool isEnabled = (flags & B_DISABLED) == 0;
	bool isActivated = (flags & B_ACTIVATED) != 0;
...
void
BeControlLook::_DrawInactiveTab(BView* view, BRect& rect,
	const BRect& updateRect, const rgb_color& base, uint32 flags,
	uint32 borders, uint32 side, tab_position position, bool full)
{
	if (!rect.IsValid() || !rect.Intersects(updateRect))
