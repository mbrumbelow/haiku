...
validate_resize(partition_data* partition, off_t* size)
{
	...
	if (newSize < 0)
		...
		newSize = sector_align(newSize, block_size);
	...
	newSize = sector_align_up(newSize, block_size);
	*size = newSize;
	return true;
}
	...
	fc_get_sibling_partitions getSiblingPartitions)
{
	...
	if (*size < childSize) {
		...
		*size = sector_align(*size, block_size);
		return true;
	}
	// grow partition
	...
	fc_get_sibling_partitions getSiblingPartitions)
{
	...
	start = sector_align(start, block_size);
	...
	if (start < childOffset) {
		...
		if (previousSibling && previousOffset + previousSize > start) {
			...
			start = sector_align_up(start, block_size);
		}
	} else {
		// moving right
	...
	const char* type, const char* name, const char* parameters, int32* index)
{
	...
	*index = newIndex;
	...
	if (*start < partition->offset + MBR_OFFSET * block_size) {
		*start = partition->offset + MBR_OFFSET * block_size;
		*start = sector_align_up(*start, block_size);
	}
	...
	off_t limitSize = 0, off_t headerSize = 0)
{
	...
	offset = sector_align_up(offset, block_size);
	...
	partitionable_space_data* buffer, int32 count, int32* actualCount)
{
	...
	return get_partitionable_spaces(partition, buffer, count, actualCount,
		fill_partitionable_spaces_buffer_pm, MBR_OFFSET * block_size,
		0, 0);
}
	...
	off_t offset, disk_job_id job)
{
	...
	// buffer allocation
	int32 allocated;
	uint8* buffer = allocate_buffer(block_size, MAX_MOVE_BUFFER,
		&allocated);
	if (!buffer)
		return B_NO_MEMORY;
	...
	error = move_partition(fd, child->offset, validatedOffset, child->size,
		buffer, allocated * block_size, job);
	delete[] buffer;
	if (error != B_OK)
		return error;
	...
	PartitionMapWriter writer(fd, block_size);
		// TODO: disk size?
...
ep_is_sub_system_for(partition_data* partition)
{
	...
	TRACE(("intel: ep_is_sub_system_for(%" B_PRId32 ": %" B_PRId64 ", "
		...
		partition->size, block_size, partition->content_type));
	...
	partitionable_space_data* buffer, int32 count, int32* actualCount)
{
	...
	return get_partitionable_spaces(partition, buffer, count, actualCount,
		fill_partitionable_spaces_buffer_ep,
		partition->offset + PTS_OFFSET * block_size,
		PTS_OFFSET * block_size,
		PTS_OFFSET * block_size);
}
	...
	off_t size, off_t ptsOffset)
{
	...
	off_t alignedOffset = sector_align(offset, block_size);
	...
	partition_id* childID)
{
	...
	logical->SetBlockSize(block_size);
