...
UnixStreamEndpoint::UnixStreamEndpoint(net_socket* socket)
	...
	fPeerEndpoint(NULL),
	fReceiveFifo(NULL),
	fState(UNIX_STREAM_ENDPOINT_CLOSED),
	fAcceptSemaphore(-1),
	fIsChild(false),
	fWasConnected(false)
{
...
UnixStreamEndpoint::Uninit()
{
	...
	bool closed = (fState == UNIX_STREAM_ENDPOINT_CLOSED);
	locker.Unlock();
...
UnixStreamEndpoint::Open()
{
	...
	fState = UNIX_STREAM_ENDPOINT_NOT_CONNECTED;
...
UnixStreamEndpoint::Close()
{
	...
	if (fState == UNIX_STREAM_ENDPOINT_CONNECTED) {
	...
	if (fState == UNIX_STREAM_ENDPOINT_LISTENING)
		_StopListening();
...
UnixStreamEndpoint::Bind(const struct sockaddr *_address)
{
	...
	if (fState != UNIX_STREAM_ENDPOINT_NOT_CONNECTED || IsBound())
...
UnixStreamEndpoint::Listen(int backlog)
{
	...
	if (fState != UNIX_STREAM_ENDPOINT_NOT_CONNECTED
		&& fState != UNIX_STREAM_ENDPOINT_LISTENING)
		RETURN_ERROR(EINVAL);
...
// TODO: If listening, we could set the backlog to 0 and connect.
	...
	if (!listeningEndpoint->IsBound()
		|| listeningEndpoint->fState != UNIX_STREAM_ENDPOINT_LISTENING
	...
	// harmless.
	UnixFifo* fifo = new(nothrow) UnixFifo(UNIX_MAX_TRANSFER_UNIT, UNIX_FIFO_TYPE_STREAM);
	UnixFifo* peerFifo = new(nothrow) UnixFifo(UNIX_MAX_TRANSFER_UNIT, UNIX_FIFO_TYPE_STREAM);
	...
	fState = UNIX_STREAM_ENDPOINT_CONNECTED;
	fWasConnected = true;
	...
	socklen_t *_addressLength)
{
	...
	switch (result) {
		case UNIX_FIFO_SHUTDOWN:
			...
			if (fState == UNIX_STREAM_ENDPOINT_CLOSED) {
				// The FD has been closed.
				result = EBADF;
			} else {
	...
	UnixStreamEndpointLocker& peerLocker)
{
	...
	if (fIsChild) {
		...
		if (fState != UNIX_STREAM_ENDPOINT_CONNECTED || peerEndpoint != fPeerEndpoint)
			RETURN_ERROR(ENOTCONN);
	} else
		peerLocker.SetTo(peerEndpoint, false);
...
{
	if (fState == UNIX_STREAM_ENDPOINT_CONNECTED || fState == UNIX_STREAM_ENDPOINT_LISTENING)
		RETURN_ERROR(B_BAD_VALUE);
