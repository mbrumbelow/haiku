...
TCPEndpoint::_UpdateReceiveBuffer()
{
	...
	if (fReceiveScalingTimestamp == 0) {
		fReceiveScalingTimestamp = tcp_now();
		fReceiveScalingReference = fReceiveNext;
		return;
	}

	const uint32 received = (fReceiveNext - fReceiveScalingReference).Number();
	if (received < (fReceiveQueue.Size() / 2))
		return;

	const uint32 since = tcp_diff_timestamp(fReceiveScalingTimestamp);
