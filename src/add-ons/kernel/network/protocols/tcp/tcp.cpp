		...
		switch (option->kind) {
			...
			case TCP_OPTION_SACK:
				if (size >= option->length) {
					...
					segment.sackCount = min_c((option->length - 2) / sizeof(tcp_sack),
						MAX_SACK_BLKS);
					for(int i = 0; i < segment.sackCount; ++i) {
						segment.sacks[i].left_edge = ntohl(option->sack[i].left_edge);
						segment.sacks[i].right_edge = ntohl(option->sack[i].right_edge);
					}
				}
				break;
		}
