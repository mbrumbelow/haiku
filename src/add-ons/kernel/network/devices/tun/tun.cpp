...
#define TUN_SEND_QUEUE_MAX (ETHER_MAX_FRAME_SIZE * 32)
...
tun_close(void* _cookie)
{
	...
	cookie->device->send_queue_wait.NotifyAll(EINTR);
...
tun_read(void* _cookie, off_t position, void* data, size_t* _length)
{
	...
	while (true) {
		...
		if (cookie->device->send_queue.current_bytes == 0) {
			if ((cookie->flags & O_NONBLOCK) != 0)
				return EWOULDBLOCK;

			status_t status = cookie->device->send_queue_wait.Wait(
				&cookie->device->send_queue.lock, B_CAN_INTERRUPT);
			if (status != B_OK)
				return status;
		}
		...
		status_t status = gStackModule->fifo_dequeue_buffer(
			&cookie->device->send_queue, MSG_DONTWAIT, 0, &buffer);
		if (status != B_OK)
			continue;
		break;
	}
	...
	status_t status = gBufferModule->read(buffer, 0, data, length);
	if (status != B_OK)
		return status;
	*_length = length;
...
tun_write(void* _cookie, off_t position, const void* data, size_t* _length)
{
	...
	status = gStackModule->device_enqueue_buffer(cookie->device, buffer);
	if (status != B_OK)
		gBufferModule->free(buffer);
...
tun_init(const char* name, net_device** _device)
{
	...
	status_t status = gStackModule->init_fifo(&device->send_queue,
		"TUN send queue", TUN_SEND_QUEUE_MAX);
	if (status != B_OK) {
		delete device;
		return status;
	}

	device->send_queue_wait.Init(&device->send_queue, "TUN send queue");
...
tun_uninit(net_device* _device)
{
	...
	if (status != B_OK)
		panic("devfs_unpublish_device failed: %d", status);
...
tun_send_data(net_device* _device, net_buffer* buffer)
{
	...
	device->send_queue_wait.NotifyAll();
	return status;
}
