...
tun_receive_data(net_device* _device, net_buffer** _buffer)
{
	...
	if (buffer == NULL)
		return ENOBUFS;
	// dprintf("Buffer created for read\n");
	ssize_t bytesRead;
	status_t status;
	bytesRead = read(device->fd, buffer, device->mtu);
	if (bytesRead < 0) {
	...
	// void* _data = malloc(buffer->size);
	// if (_data == NULL) {
	// 	dprintf("Couldn't allocate data buffer");
	// 	return B_NO_MEMORY;
	// }
	// status = gBufferModule->read(buffer, 0, _data, buffer->size);
	// if (status != B_OK) {
	// 	dprintf("Could not read from buffer\n");
	// 	return status;
	// }
	// uint8_t* bytePtr = static_cast<uint8_t*>(_data);
	// for (size_t i = 0; i < buffer->size; i++) {
    // 	uint8_t byte = *(bytePtr + i);
	// 	dprintf("%02x", byte);
	// }
	// dprintf("\n");
	// dprintf("Read %li bytes to %p\n", bytesRead, buffer);
