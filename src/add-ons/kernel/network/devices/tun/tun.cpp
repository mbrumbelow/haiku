...
prepend_ethernet_frame(net_buffer *buffer)
{
	...
	header.ether_type = B_HOST_TO_BENDIAN_INT16(ETHER_TYPE_IP);

	memset(header.ether_shost, 0, ETHER_ADDRESS_LENGTH);
	memset(header.ether_dhost, 0, ETHER_ADDRESS_LENGTH);
	bufferHeader.Sync();
	return B_OK;
}
...
{
	if (strncmp(name, "tun", 3) && strncmp(name, "tap", 3) && strncmp(name, "dns", 3)) /* iodine uses that */
		return B_BAD_VALUE;
...
{
	tun_device *device = (tun_device *)_device;
	device->fd = open("/dev/tap0", O_RDWR);
	if (device->fd < 0)
	{
...
{
	tun_device *device = (tun_device *)_device;
	/* TODO: Get driver to be closed and opened properly */
	close(device->fd);
	device->fd = -1;
}
