...
tunnel_write(void* _cookie, off_t position, const void* data, size_t* _length)
{
	...
	if (!cookie->device->is_tap) {
		...
		const uint16_t type = (version == 6) ? htons(ETHER_TYPE_IPV6)
			: htons(ETHER_TYPE_IP);
		...
		src.sdl_family	= dst.sdl_family	= (version == 6) ? AF_INET6 : AF_INET;
		src.sdl_index	= dst.sdl_index		= cookie->device->index;
		src.sdl_type	= dst.sdl_type		= IFT_TUNNEL;
		src.sdl_e_type	= dst.sdl_e_type	= type;
...
tunnel_send_data(net_device* _device, net_buffer* buffer)
{
	...
	if (!device->is_tap) {
		...
		if (buffer->type != B_NET_FRAME_TYPE_IPV4 && buffer->type != B_NET_FRAME_TYPE_IPV6)
			return B_BAD_DATA;

		struct sockaddr_dl& dst = *(struct sockaddr_dl*)buffer->destination;
		if (dst.sdl_family != AF_INET && dst.sdl_family != AF_INET6)
			return B_BAD_DATA;
	}
