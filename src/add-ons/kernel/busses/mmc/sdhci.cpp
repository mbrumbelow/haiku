	...
	fSemaphore(0)
{
	...
	// Spawn a polling thread, as the interrupts won't work in the meantime.
...
SdhciBus::_WorkerThread(void* cookie) {
	...
	while (bus->fStatus != B_SHUTTING_DOWN) {
		if (bus->fRegisters->interrupt_status & (SDHCI_INT_CMD_CMP | SDHCI_INT_TRANS_CMP)) {
			bus->fCommandResult = bus->fRegisters->interrupt_status;
