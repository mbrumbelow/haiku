...
mmc_bus_interface gSDHCIACPIDeviceModule = {
	...
	[](void* controller, uint32_t kilohertz) -> status_t // set_clock
	{
		SdhciBus* bus = (SdhciBus*)controller;
		bus->SetClock(kilohertz);
		return B_OK;
	},
	[](void* controller, uint8_t command, uint32_t argument,
	uint32_t* response) -> status_t // execute_command
	{
		SdhciBus* bus = (SdhciBus*)controller;
		return bus->ExecuteCommand(command, argument, response);
	},
	[](void* controller, uint8_t command, IOOperation* operation,
	bool offsetAsSectors) // do_io
	{
		CALLED();
	...
	[](void* controller, sem_id sem) // set_scan_semaphore
	{
		CALLED();
	...
	[](void* controller, int width) // set_bus_width
	{
		CALLED();
