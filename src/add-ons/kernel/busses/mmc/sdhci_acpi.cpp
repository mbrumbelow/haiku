...
#include "mmc.h"
#include "sdhci_acpi.h"


#define TRACE_SDHCI
#ifdef TRACE_SDHCI
#	define TRACE(x...) dprintf("\33[33msdhci_acpi:\33[0m " x)
#else
#	define TRACE(x...) ;
#endif
#define TRACE_ALWAYS(x...)	dprintf("\33[33msdhci_acpi:\33[0m " x)
#define ERROR(x...)			dprintf("\33[33msdhci_acpi:\33[0m " x)
#define CALLED(x...)		TRACE("CALLED %s\n", __PRETTY_FUNCTION__)


#define SDHCI_ACPI_DEVICE_MODULE_NAME "busses/mmc/sdhci_acpi/driver_v1"
#define SDHCI_ACPI_MMC_BUS_MODULE_NAME "busses/mmc/sdhci_acpi/device/v1"
...
{
	struct sdhci_acpi_crs* crs = (struct sdhci_acpi_crs*)context;
...
uninit_device(void* device_cookie)
{
	...
	device_node* acpiParent = gDeviceManager->get_parent_node(context->fNode);
	gDeviceManager->get_driver(acpiParent, (driver_module_info**)&acpi,
			(void**)&device);
...
register_device(device_node* parent)
{
	...
	return gDeviceManager->register_node(parent, SDHCI_ACPI_DEVICE_MODULE_NAME,
		attrs, NULL, NULL);
}
...
supports_device(device_node* parent)
{
	...
	// make sure parent is a ACPI SDHCI device node
...
// Root device that binds to the ACPI bus. It will register an mmc_bus_interface
// node for each SD slot in the device.
static driver_module_info sSDHCIDevice = {
	{
		SDHCI_ACPI_DEVICE_MODULE_NAME,
		0,
		NULL
	},
	supports_device,
