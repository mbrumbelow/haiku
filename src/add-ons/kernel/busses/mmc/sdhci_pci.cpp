...
SdhciBus::ExecuteCommand(uint8_t command, uint32_t argument, uint32_t* response)
{
	...
	if ((replyType & Command::k32BitResponseCheckBusy) != 0
		&& command != STOP_TRANSMISSION && command != IO_ABORT) {
...
SdhciBus::ReadNaive(off_t pos, void* buffer, size_t* _length)
{
	...
	uint32_t response;
	status_t result;
	result = ExecuteCommand(READ_SINGLE_BLOCK, pos, &response);
	...
	// FIXME currently we only know how to read 512 bytes...
	if (*_length < 512)
		panic("Trying to read %ld bytes from SD card\n", *_length);
	*_length = 512;
