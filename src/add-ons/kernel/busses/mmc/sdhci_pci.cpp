...
supports_device(device_node* parent)
{
	...
	if (strcmp(bus, "pci") != 0) {
		TRACE("Skipping device %p on %s bus\n", parent, bus);
		return 0.0f;
	}

	if (gDeviceManager->get_attr_uint16(parent, B_DEVICE_VENDOR_ID, &vendorId,
		false) != B_OK || gDeviceManager->get_attr_uint16(parent, B_DEVICE_ID,
		&deviceId, false) != B_OK) {
	...
	if (gDeviceManager->get_attr_uint16(parent, B_DEVICE_SUB_TYPE, &subType,
		false) < B_OK || gDeviceManager->get_attr_uint16(parent, B_DEVICE_TYPE,
		&type, false) < B_OK) {
		TRACE("Could not find type/subtype attributes\n");
		return -1;
	}

	TRACE("Type %d.%d (expected %d.%d\n", type, subType, PCI_base_peripheral,
		PCI_sd_host);

	if (type == PCI_base_peripheral) {
		if (subType != PCI_sd_host) {
			// Also accept some compiant devices that do not advertise
			// themselves as such.
	...
	TRACE("Not the right device class\n");
