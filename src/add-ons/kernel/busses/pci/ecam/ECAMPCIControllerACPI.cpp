...
acpi_status
ECAMPCIControllerACPI::AcpiCrsScanCallback(uacpi_resource *res, void *context)
{
...
{
	const auto& acpiRange = resource.common;
	dprintf("PCI: range from ACPI [%lx(%d),%lx(%d)] with length %lx\n",
		(unsigned long)resource.minimum, acpiRange.fixed_min_address,
		(unsigned long)resource.maximum, acpiRange.fixed_max_address,
		(unsigned long)resource.address_length);

	// If address_length isn't set, compute it from minimum and maximum
	// If maximum isn't set, compute it from minimum and length
	auto addressLength = resource.address_length;
	phys_addr_t addressMaximum = resource.maximum;

	if (addressLength == 0 && addressMaximum <= resource.minimum) {
		// There's nothing we can do with that...
		dprintf("PCI: Ignore empty ACPI range\n");
		return false;
	} else if (!acpiRange.fixed_max_address) {
		if (addressLength == 0) {
		...
		addressMaximum = resource.minimum + addressLength - 1;
	} else if (addressLength != addressMaximum - resource.minimum + 1) {
		dprintf("PCI: Fixup invalid length from ACPI!\n");
		addressLength = addressMaximum - resource.minimum + 1;
	}

	range.host_address = resource.minimum + resource.translation_offset;
	range.pci_address  = resource.minimum;
...
acpi_status
ECAMPCIControllerACPI::AcpiCrsScanCallbackInt(uacpi_resource *res)
{
	pci_resource_range range = {};

	switch (res->type) {
		case UACPI_RESOURCE_TYPE_ADDRESS16: {
			const auto& address = res->address16;
			if (!DecodeAddress(address, range))
				return B_OK;
			break;
		}
		case UACPI_RESOURCE_TYPE_ADDRESS32: {
			const auto& address = res->address32;
			if (!DecodeAddress(address, range))
				return B_OK;
		...
		case UACPI_RESOURCE_TYPE_ADDRESS64: {
			const auto& address = res->address64;
			if (!DecodeAddress(address, range))
				return B_OK;
	...
	switch (res->address32.common.type) {
		case 0: // ACPI_MEMORY_RANGE
			range.type = B_IO_MEMORY;
			if (res->address32.common.attribute.memory.caching == 3 /*ACPI_PREFETCHABLE_MEMORY*/)
