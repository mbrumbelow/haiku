...
template<typename T> void DecodeAddress(const T& address, pci_resource_range& range)
{
	// If address_length isn't set, compute it from minimum and maximum
	// If maximum isn't set, compute it from minimum and length
	auto address_length = address.address.address_length;
	phys_addr_t address_maximum = address.address.maximum;
	dprintf("PCI: range from ACPI [%lx(%d),%lx(%d)] with length %lx\n",
		(unsigned long)address.address.minimum, address.minAddress_fixed,
		(unsigned long)address.address.maximum, address.maxAddress_fixed,
		(unsigned long)address.address.address_length);
	if (address_length == 0)
		address_length = address.address.maximum - address.address.minimum + 1;
	else if (!address.maxAddress_fixed)
		address_maximum = address.address.minimum + address_length + 1;
	ASSERT((phys_addr_t)(address.address.minimum + address_length - 1) == address_maximum);
	range.host_addr = address.address.minimum + address.address.translation_offset;
	range.pci_addr  = address.address.minimum;
