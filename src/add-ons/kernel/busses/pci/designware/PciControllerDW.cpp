...
float
PciControllerDW::SupportsDevice(device_node* parent)
{
	const char* bus;
...
status_t
PciControllerDW::InitDriver(device_node* node, PciControllerDW*& outDriver)
{
	ObjectDeleter<PciControllerDW> driver(new(std::nothrow) PciControllerDW());
	if (!driver.IsSet())
		return B_NO_MEMORY;
...
status_t
PciControllerDW::ReadConfig(uint8 bus, uint8 device, uint8 function,
...
status_t
PciControllerDW::WriteConfig(uint8 bus, uint8 device, uint8 function,
	uint16 offset, uint8 size, uint32 value)
{
	InterruptsSpinLocker lock(fLock);
...
status_t
PciControllerDW::GetMaxBusDevices(int32& count)
{
	count = 32;
	return B_OK;
}
...
status_t
PciControllerDW::ReadIrq(uint8 bus, uint8 device, uint8 function,
	uint8 pin, uint8& irq)
{
...
status_t
PciControllerDW::WriteIrq(uint8 bus, uint8 device, uint8 function,
	uint8 pin, uint8 irq)
{
	return B_UNSUPPORTED;
}
...
//#pragma mark - PciControllerDW


status_t
PciControllerDW::AtuMap(uint32 index, uint32 direction, uint32 type, uint64 parentAdr, uint64 childAdr,
	uint32 size)
{
	/*
