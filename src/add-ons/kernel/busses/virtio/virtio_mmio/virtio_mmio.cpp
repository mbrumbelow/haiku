	...
	void* cookie)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		if (desc < 0) {
			...
			desc = firstDesc;
			while (kVringDescFlagsNext & desc) {
...
*/
		...
		if (i == 0) {
			...
			queue->fDescs[lastDesc].flags |= kVringDescFlagsNext;
		...
		if (i >= readVectorCount)
			queue->fDescs[desc].flags |= kVringDescFlagsWrite;
	...
	uint32* _usedLength)
{
	...
	while (kVringDescFlagsNext & queue->fDescs[desc].flags) {
		int32_t nextDesc = queue->fDescs[desc].next;
		// dprintf("%p.FreeDesc(): %" B_PRId32 "\n", queue, desc);
		queue->FreeDesc(desc);
		desc = nextDesc;
