...
/*
static acpi_status
virtio_crs_callback(ACPI_RESOURCE *res, void *context)
{
	if (res->Type == ACPI_RESOURCE_TYPE_FIXED_MEMORY32)
		dprintf("address %" B_PRIx32 "\n",
			((ACPI_RESOURCE_FIXED_MEMORY32 *)&res->Data)->Address);
	else if (res->Type == ACPI_RESOURCE_TYPE_EXTENDED_IRQ)
		dprintf("irq %" B_PRIx32 "\n",
			((ACPI_RESOURCE_EXTENDED_IRQ *)&res->Data)->Interrupts[0]);
...
virtio_crs_find_address(ACPI_RESOURCE *res, void *context)
{
	...
	if (res->Type == ACPI_RESOURCE_TYPE_FIXED_MEMORY32) {
		range.base = (uint64)((ACPI_RESOURCE_FIXED_MEMORY32 *)&res->Data)->Address;
		range.length = (uint64)((ACPI_RESOURCE_FIXED_MEMORY32 *)&res->Data)->AddressLength;
	}
...
virtio_crs_find_interrupt(ACPI_RESOURCE *res, void *context)
{
	...
	if (res->Type == ACPI_RESOURCE_TYPE_EXTENDED_IRQ)
		interrupt = (uint64)((ACPI_RESOURCE_EXTENDED_IRQ *)&res->Data)->Interrupts[0];
...
virtio_device_register_device(device_node* parent)
{
	...
	if (strcmp(bus, "acpi") == 0) {
		...
		parentModule->walk_resources(parentDev, (ACPI_STRING)METHOD_NAME__CRS,
...
virtio_device_init_device(device_node* node, void** cookie)
{
	...
	reserve_io_interrupt_vectors(1, interrupt, INTERRUPT_TYPE_IRQ);
