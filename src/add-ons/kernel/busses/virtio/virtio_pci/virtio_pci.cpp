...
init_bus(device_node* node, void** bus_cookie)
{
	...
	if (bus->virtio1) {
		...
		if (virtio_pci_find_capability(bus, VIRTIO_PCI_CAP_COMMON_CFG, &common, sizeof(common)) != B_OK)
			return B_DEVICE_NOT_FOUND;
		if (virtio_pci_find_capability(bus, VIRTIO_PCI_CAP_ISR_CFG, &isr, sizeof(isr)) != B_OK)
			return B_DEVICE_NOT_FOUND;
		if (virtio_pci_find_capability(bus, VIRTIO_PCI_CAP_DEVICE_CFG, &deviceCap, sizeof(deviceCap)) != B_OK)
			memset(&deviceCap, 0, sizeof(deviceCap));
		else {
			deviceCfgFound = true;
		}
		if (virtio_pci_find_capability(bus, VIRTIO_PCI_CAP_NOTIFY_CFG, &notify, sizeof(notify)) != B_OK)
			return B_DEVICE_NOT_FOUND;
		...
		if (notify.cap.length > 0)
			bars[notify.cap.bar] = max_c(bars[notify.cap.bar], notify.cap.offset + notify.cap.length);
