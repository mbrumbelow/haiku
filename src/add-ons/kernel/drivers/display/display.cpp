...
display_read(void *cookie, off_t position, void *buf, size_t* num_bytes)
{
	...
	status_t status = gAcpi->evaluate_method(device->acpi_device, "_BCQ", NULL, &buffer);
	if (status == B_OK) {
		acpi_object_type* backlightLevel = (acpi_object_type*)buffer.pointer;
		dprintf("display: Current backlight level is %d\n", backlightLevel->integer.integer);
	} else {
		dprintf("display: Failed to get current backlight level: %d\n", status);
	}
	...
	size_t* num_bytes)
{
	...
	dprintf("display: Call _BCM(%ld) on %p results in %d\n", *num_bytes, device->acpi_device,
		status);
