...
get_settings_from_file()
{
	...
	if (item) {
		...
		if (*end == '\0')
			requested_settings.play_sample_rate =
				value ? fps_to_ratecode(value) : 0;
	}

	item = get_driver_parameter (settings_handle, "play_buffer_frames", "0",
		"10240");
	if (item) {
		value = strtoul (item, &end, 0);
		if (*end == '\0')
	...
	item = get_driver_parameter (settings_handle, "record_sample_rate", "0",
		"192000");
	value = strtoul (item, &end, 0);
	if (*end == '\0')
		requested_settings.record_sample_rate =
			value ? fps_to_ratecode(value) : 0;

	item = get_driver_parameter (settings_handle, "record_buffer_frames", "0",
		"10240");
	value = strtoul (item, &end, 0);
	if (*end == '\0')
		requested_settings.record_buffer_frames = value;

	item = get_driver_parameter (settings_handle, "record_buffer_count", "0",
		"4");
	value = strtoul (item, &end, 0);
	if (*end == '\0')
