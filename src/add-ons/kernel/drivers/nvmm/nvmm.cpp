...
vmx_check_ctls(uint64_t msr_ctls, uint64_t msr_true_ctls, uint64_t set_one)
{
	...
	basic = x86_read_msr(MSR_IA32_VMX_BASIC);
	has_true = (basic & IA32_VMX_BASIC_TRUE_CTLS) != 0;

	val = x86_read_msr(msr_ctls);
	if (has_true) {
		true_val = x86_read_msr(msr_true_ctls);
	} else {
		true_val = val;
	}
...
vmx_ident(void)
{
	...
	if (!(cpu->arch.feature[FEATURE_EXT] & IA32_FEATURE_EXT_VMX)) {
		TRACE(("NVMM: VMX not supported\n"));
		return false;
	}

	msr = x86_read_msr(MSR_IA32_FEATURE_CONTROL);
	if ((msr & IA32_FEATURE_CONTROL_LOCK) != 0 &&
	    (msr & IA32_FEATURE_CONTROL_OUT_SMX) == 0) {
		TRACE(("NVMM: VMX disabled in BIOS\n"));
		return false;
	}

	msr = x86_read_msr(MSR_IA32_VMX_BASIC);
	if ((msr & IA32_VMX_BASIC_IO_REPORT) == 0) {
		TRACE(("NVMM: I/O reporting not supported\n"));
		return false;
	}
	if (__SHIFTOUT(msr, IA32_VMX_BASIC_MEM_TYPE) != MEM_TYPE_WB) {
		TRACE(("NVMM: WB memory not supported\n"));
		return false;
	}

	/* PG and PE are reported, even if Unrestricted Guests is supported. */
	vmx_cr0_fixed0 = x86_read_msr(MSR_IA32_VMX_CR0_FIXED0) & ~(CR0_PAGING_ENABLE|CR0_PROTECTED_MODE_ENABLE);
	vmx_cr0_fixed1 = x86_read_msr(MSR_IA32_VMX_CR0_FIXED1) | (CR0_PAGING_ENABLE|CR0_PROTECTED_MODE_ENABLE);
	ret = vmx_check_cr(x86_read_cr0(), vmx_cr0_fixed0, vmx_cr0_fixed1);
	if (ret == -1) {
		TRACE(("NVMM: CR0 requirements not satisfied\n"));
		return false;
	}

	vmx_cr4_fixed0 = x86_read_msr(MSR_IA32_VMX_CR4_FIXED0);
	vmx_cr4_fixed1 = x86_read_msr(MSR_IA32_VMX_CR4_FIXED1);
	ret = vmx_check_cr(x86_read_cr4() | IA32_CR4_VMXE, vmx_cr4_fixed0, vmx_cr4_fixed1);
	if (ret == -1) {
		TRACE(("NVMM: CR4 requirements not satisfied\n"));
		return false;
	}
	...
	ret = vmx_init_ctls(
	    ...
	    &vmx_pinbased_ctls);
	if (ret == -1) {
		TRACE(("NVMM: pin-based-ctls requirements not satisfied\n"));
		return false;
	}
	ret = vmx_init_ctls(
	    ...
	    &vmx_procbased_ctls);
	if (ret == -1) {
		TRACE(("NVMM: proc-based-ctls requirements not satisfied\n"));
		return false;
	}
	ret = vmx_init_ctls(
	    ...
	    &vmx_procbased_ctls2);
	if (ret == -1) {
		TRACE(("NVMM: proc-based-ctls2 requirements not satisfied\n"));
		return false;
	}
	ret = vmx_check_ctls(
	    MSR_IA32_VMX_PROCBASED_CTLS2, MSR_IA32_VMX_PROCBASED_CTLS2,
	...
	ret = vmx_init_ctls(
	    ...
	    &vmx_entry_ctls);
	if (ret == -1) {
		TRACE(("NVMM: entry-ctls requirements not satisfied\n"));
		return false;
	}
	ret = vmx_init_ctls(
	    ...
	    &vmx_exit_ctls);
	if (ret == -1) {
		TRACE(("NVMM: exit-ctls requirements not satisfied\n"));
		return false;
	}

	msr = x86_read_msr(MSR_IA32_VMX_EPT_VPID_CAP);
	if ((msr & IA32_VMX_EPT_VPID_WALKLENGTH_4) == 0) {
		TRACE(("NVMM: 4-level page tree not supported\n"));
		return false;
	}
	if ((msr & IA32_VMX_EPT_VPID_INVEPT) == 0) {
		TRACE(("NVMM: INVEPT not supported\n"));
		return false;
	}
	if ((msr & IA32_VMX_EPT_VPID_INVVPID) == 0) {
		TRACE(("NVMM: INVVPID not supported\n"));
		return false;
	}
	...
	if (!(msr & IA32_VMX_EPT_VPID_UC) && !(msr & IA32_VMX_EPT_VPID_WB)) {
		TRACE(("NVMM: EPT UC/WB memory types not supported\n"));
		return false;
	}
