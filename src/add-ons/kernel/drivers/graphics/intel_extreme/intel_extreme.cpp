			...
			if (status == B_OK) {
				...
				if (info.device_type.Generation() >= 11) {
					...
					if (info.shared_info->pch_info >= INTEL_PCH_ICP) {
						...
						// we enable everything, should come from the VBT
						ctl |= SHOTPLUG_CTL_DDI_HPD_ENABLE(HPD_PORT_TC1)
							| SHOTPLUG_CTL_DDI_HPD_ENABLE(HPD_PORT_TC2)
							| SHOTPLUG_CTL_DDI_HPD_ENABLE(HPD_PORT_TC3)
							| SHOTPLUG_CTL_DDI_HPD_ENABLE(HPD_PORT_TC4)
							| SHOTPLUG_CTL_DDI_HPD_ENABLE(HPD_PORT_TC5)
							| SHOTPLUG_CTL_DDI_HPD_ENABLE(HPD_PORT_TC6);
						write32(info, SHOTPLUG_CTL_TC, 0);
					}
