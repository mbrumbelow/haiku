...
{
	if (info.device_type.Generation() > 7)
		return;
...
{
	if (info.device_type.Generation() < 8) {
		// This function is a check that is called in generic logic using
		// && operator, so to not change the result of check for other
		// platforms, return true if there is nothing to do.
		return true;
	}
...
intel_interrupt_handler(void* data)
{
	...
	while (shouldHandle) {
		...
		if ((which & INTEL_PIPE_C) != 0 && bdw_check_pipe_interrupt(info, INTEL_PIPE_C)) {
			handled = release_vblank_sem(info);
...
init_interrupt_handler(intel_info &info)
{
	...
	if (status == B_OK && info.irq != 0xff) {
		...
		if (status == B_OK) {
			...
			intel_enable_interrupts(info, INTEL_PIPE_A | INTEL_PIPE_B, true);
