...
bdw_enable_interrupts(intel_info& info, pipe_index pipe, bool enable)
{
	...
	const uint32 reg_mask = PCH_INTERRUPT_PIPE_MASK_BDW(pipe);
	const uint32 reg_enabled = PCH_INTERRUPT_PIPE_ENABLED_BDW(pipe);
	const uint32 reg_identity = PCH_INTERRUPT_PIPE_IDENTITY_BDW(pipe);
