	...
	for (int bdbBlockOffset = bdb->header_size; bdbBlockOffset < bdb->bdb_size;
		...
		switch (id) {
			case BDB_GENERAL_DEFINITIONS:
			{
				...
				uint32 device_config_count = min_c(10, (blockSize - sizeof(*defs)) / childDeviceSize);
				for (uint32 i = 0; i < device_config_count; i++) {
					child_device_config* config = (child_device_config*)(&defs->devices[i * childDeviceSize]);
