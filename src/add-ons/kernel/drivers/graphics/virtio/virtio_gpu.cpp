...
virtio_gpu_get_display_info(virtio_gpu_driver_info* info)
{
	...
	struct virtio_gpu_resp_display_info displayInfo = { 0 };
	...
	info->framebufferWidth = displayInfo.pmodes[0].r.width;
	info->framebufferHeight = displayInfo.pmodes[0].r.height;
...
{
	CALLED();
	struct virtio_gpu_cmd_get_edid getEdid = { 0 };
	struct virtio_gpu_resp_edid response = { 0 };
...
{
	CALLED();
	struct virtio_gpu_resource_create_2d resource = { 0 };
...
{
	CALLED();
	struct virtio_gpu_resource_unref resource = { 0 };
	struct virtio_gpu_ctrl_hdr response = { 0 };
...
virtio_gpu_transfer_to_host_2d(virtio_gpu_driver_info* info, int resourceId,
    uint32 width, uint32 height)
{
	struct virtio_gpu_transfer_to_host_2d transferToHost = { 0 };
	struct virtio_gpu_ctrl_hdr response = { 0 };
...
virtio_gpu_flush_resource(virtio_gpu_driver_info* info, int resourceId, uint32 width,
    uint32 height)
{
	struct virtio_gpu_resource_flush resourceFlush = { 0 };
	struct virtio_gpu_ctrl_hdr response = { 0 };
...
virtio_update_thread(void *arg)
{
	...
	while (info->updateThreadRunning) {
		...
		virtio_gpu_transfer_to_host_2d(info, info->displayResourceId, info->framebufferWidth,
			info->framebufferHeight);
		virtio_gpu_flush_resource(info, info->displayResourceId, info->framebufferWidth, info->framebufferHeight);
...
virtio_gpu_init_device(void* _info, void** _cookie)
{
	...
	info->virtio->negotiate_features(info->virtio_device, /*VIRTIO_GPU_F_EDID*/0,
...
virtio_gpu_open(void* _info, const char* path, int openMode, void** _cookie)
{
	...
	status = virtio_gpu_create_2d(info, info->displayResourceId, info->framebufferWidth,
		info->framebufferHeight);
	if (status != B_OK)
		goto error2;
	...
	status = virtio_gpu_set_scanout(info, 0, info->displayResourceId, info->framebufferWidth,
		info->framebufferHeight);
	if (status != B_OK)
		goto error2;
...

	{
		...
		sharedInfo.bytes_per_row = info->framebufferWidth * 4;

		sharedInfo.current_mode.virtual_width = info->framebufferWidth;
		sharedInfo.current_mode.virtual_height = info->framebufferHeight;
