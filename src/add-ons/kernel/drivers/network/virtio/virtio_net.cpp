...
static status_t
rx_enqueue_buf(virtio_net_driver_info* info, BufInfo* buf)
{
...
virtio_net_init_device(void* _info, void** _cookie)
{
	...
	char* rxBuffer;
	char* txBuffer;
	size_t rxBufferSize = BUFFER_SIZE + sizeof(virtio_net_rx_hdr);
	size_t txBufferSize = BUFFER_SIZE + sizeof(virtio_net_tx_hdr);
...
virtio_net_open(void* _info, const char* path, int openMode, void** _cookie)
{
	...
	for (int i = 0; i < info->rxSizes[0]; i++) {
		rx_enqueue_buf(info, info->rxBufInfos[i]);
	}
