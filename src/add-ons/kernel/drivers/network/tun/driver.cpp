...
tun_read(void *cookie, off_t position, void *data, size_t *numbytes)
{
    ...
    status_t result = acquire_sem_etc(readSem, 1, B_CAN_INTERRUPT, 0);
    if (result < B_OK) {
        *numbytes = 0;
        return result;
    }
    dprintf("Acquired Semaphore\n");
    status_t status;
    net_buffer* buffer;
    // dprintf("TUN: Reading %li bytes of data\n", *numbytes);
    if ((strcmp((char*)cookie, "tun") == 0) && (interfaceQ.Available() != 0)) {
        ...
        // }
        return B_OK;
    } else if ((cookie == NULL) && (appQ.Available() != 0)) {
