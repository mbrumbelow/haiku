...
tun_notify(tun_struct* tun)
{
	...
	if (!tun->readable && readable) {
		dprintf("notify:read\n");
        notify_select_event_pool(tun->select_pool, B_SELECT_READ);
    }
	tun->readable = readable;

    bool writable = tun->sendQueue->Used() != 0;
	if (!tun->writable && writable) {
		dprintf("notify:write\n");
        notify_select_event_pool(tun->select_pool, B_SELECT_WRITE);
    }
	tun->writable = writable;
}
...
tun_open(const char* name, uint32 flags, void** cookie)
{
    ...
    tun->flags = 0;
    tun->select_pool = NULL;
    tun->readable = tun->recvQueue->Used() != 0;
    tun->writable = tun->sendQueue->Used() != 0;
...
tun_ioctl(void* cookie, uint32 op, void* data, size_t len)
{
    /* IOCTL for driver */
    tun_struct *tun = static_cast<tun_struct*>(cookie);
	dprintf("%s: tun_ioctl | op = %" B_PRIu32 "\n", tun->name, op);
	switch (op) {
	case TUNSETIFF: 
        // TODO: Implement tun_set_iff
		return B_OK;
	case B_SET_NONBLOCKING_IO:
        tun->flags |= B_SET_NONBLOCKING_IO;
        return B_OK;
    default:
        dprintf("%s: IOCTL Option not Availvable\n", tun->name);
...
{
    /* Blocks but doesn't work */ 
    tun_struct* tun = static_cast<tun_struct*>(cookie);
    dprintf("%s:select() | %u\n", tun->name, event);
	size_t recvUsed = tun->recvQueue->Used();
    size_t sendUsed = tun->sendQueue->Used();
    bool wasReadable = recvUsed != 0;
    bool wasWritable = sendUsed != 0;

	if (event != B_SELECT_READ && event != B_SELECT_WRITE)
        return B_BAD_VALUE;
	
    dprintf("select(): Adding select sync pool entry\n");
	status_t status = add_select_sync_pool_entry(&tun->select_pool, sync, event);
	if (status != B_OK) {
        dprintf("add_select_sync_pool_entry() failed: %" B_PRIx32 "\n", status);
		return status;
    }
...
tun_deselect(void* cookie, uint8 event, selectsync* sync)
{
    ...
    dprintf("deselect(): Removing select sync pool entry %u\n", event);
	return remove_select_sync_pool_entry(&tun->select_pool, sync, event);
}
