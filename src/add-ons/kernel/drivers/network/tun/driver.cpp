 ...
 * (c) 2023, Swangeon, swangeon@gmail.com
 *
...
tun_read(void* cookie, off_t position, void* data, size_t* numbytes)
{
	tun_struct* tun = static_cast<tun_struct*>(cookie);
	status_t status;
	MutexLocker lock(tun->readLock); // released on exit
	while (tun->recvQueue->Used() == 0) {
		/* APP side will always want non-blocking I/O */
		if ((tun->flags & O_NONBLOCK) != 0)
			/* Is a workaround due to select implementation problems at the time of writing */
	...
	return retrieve_packet(tun->recvQueue, data, numbytes);
}
