...
RNDISDevice::Read(uint8 *buffer, size_t *numBytes)
{
	...
	if (header[0] != REMOTE_NDIS_PACKET_MSG) {
		TRACE_ALWAYS("Received unexpected packet type %08X on data link\n", header[0]);
		*numBytes = 0;
		return B_BAD_VALUE;
	}

	if (header[1] != fActualLengthRead) {
		TRACE_ALWAYS("Received frame length %08x but USB transfer length is %08x\n",
			header[1], fActualLengthRead);
	}

	if (header[4] != 0 || header[5] != 0 || header[6] != 0) {
		TRACE_ALWAYS("Received frame has out of bound data: off %08x len %08x count %08x\n",
			header[4], header[5], header[6]);
	}

	if (header[7] != 0 || header[8] != 0) {
		TRACE_ALWAYS("Received frame has per-packet info: off %08x len %08x\n",
			header[7], header[8]);
	}
