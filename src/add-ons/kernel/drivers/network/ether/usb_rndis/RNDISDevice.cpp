...
RNDISDevice::Open()
{
	...
	status_t status = _EnableBroadcast(fDevice);
...
RNDISDevice::Read(uint8 *buffer, size_t *numBytes)
{
	...
	// TODO decapsulate REMOTE_NDIS_PACKET_MSG to get each packet
	...
	TRACE_ALWAYS("FIXME: RNDIS packet decapsulation\n");
...
RNDISDevice::Write(const uint8 *buffer, size_t *numBytes)
{
	...
	// FIXME encapsulate the data in a REMOTE_NDIS_PACKET_MSG
	TRACE_ALWAYS("FIXME: RNDIS packet encapsulation\n");
...
RNDISDevice::_ReadMediaState(usb_device device)
{
	...
	TRACE_ALWAYS("media connect state: %d\n", fMediaConnectState);
	return B_OK;
}
...
RNDISDevice::_ReadLinkSpeed(usb_device device)
{
	...
	TRACE("Read OID command results in %s\n", strerror(result));
	if (result != B_OK)
		return result;

	TRACE_ALWAYS("link speed: %d * 100bps\n", fDownstreamSpeed);
	return B_OK;
}
...
RNDISDevice::_EnableBroadcast(usb_device device)
{
	...
	switch (response[4])
	{
		...
		case RNDIS_STATUS_MEDIA_DISCONNECT:
			return ENOTCONN;
		default:
			TRACE_ALWAYS("Unexpected error code %x\n", response[4]);
			return B_IO_ERROR;
	}
}
	...
	size_t actualLength)
{
	...
	if (status != B_OK) {
		...
		// FIXME this just tells us that something is available to read from the control endpoint.
		// Should we read it now, or can we let other parts of the code handle it? In most cases
		// it will be a response to a query from the host, but maybe not always.
