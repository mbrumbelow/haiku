...
do_nvme_io_operation(nvme_disk_driver_info* info, nvme_io_operation* op)
{
	...
	if (op->status != B_OK) {
		TRACE_ERROR("%s at %" B_PRIdOFF " of %" B_PRIuGENADDR
			" bytes failed!\n", op->operation.IsWrite() ? "write" : "read",
			op->operation.Offset(), op->operation.Length());
	}
	return op->status;
...
nvme_disk_io(void* cookie, io_request* request)
{
	...
	while (request->RemainingBytes() > 0) {
		...
		do {
			TRACE("%p: IOO Offset: %" B_PRIdOFF ", Length: %" B_PRIuGENADDR
				", Write: %s\n", request, nvme_op.operation.Offset(),
				nvme_op.operation.Length(),
				nvme_op.operation.IsWrite() ? "yes" : "no");

			status = do_nvme_io_operation(info, &nvme_op);
			if (status == B_OK
				&& nvme_op.operation.IsWrite() == request->IsWrite()) {
				transferredBytes += nvme_op.operation.OriginalLength();
			}
		...
		request->OperationFinished(&nvme_op.operation, status, status != B_OK,
			nvme_op.operation.OriginalOffset() + transferredBytes);
	...
	ASSERT(status != B_OK || request->TransferredBytes() == request->Length());
