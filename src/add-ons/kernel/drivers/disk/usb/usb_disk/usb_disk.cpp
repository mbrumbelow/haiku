...
usb_disk_request_sense(device_lun *lun, err_act *_action)
{
	...
	for (uint32 tries = 0; tries < 3; tries++) {
		result = usb_disk_operation(lun, commandBlock, 6, &parameter, &dataLength,
			true);
		if (result != B_TIMED_OUT)
			break;
		snooze(100000);
...
usb_disk_update_capacity(device_lun *lun)
{
	...
	// reads.
	for (int32 i = 0; i < 5; i++) {
		result = usb_disk_operation(lun, commandBlock, 10, &parameter, &dataLength,
			true, &action);
...
usb_disk_ioctl(void *cookie, uint32 op, void *buffer, size_t length)
{
	...
	switch (op) {
		...
		case B_GET_VECTOR_ICON:
		{
			...
			if (device->is_ufi) {
				// Only UFI device (floppy drives) use interrupt mode, because
				// they are slow and deserve an asyncronous protocol.
