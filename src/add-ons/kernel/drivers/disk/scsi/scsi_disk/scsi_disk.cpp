...
trim_device(das_driver_info* device, fs_trim_data* trimData)
{
	...
	for (uint32 i = 0; i < trimData->range_count; i++) {
		off_t startBytes = trimData->ranges[i].offset;
		off_t sizeBytes = trimData->ranges[i].size;
		uint32 blockSize = device->block_size;

		// Check that signed off_t can be converted to unsigned uint64
		if (startBytes < 0 || sizeBytes < 0
			|| (off_t)(uint64)startBytes != trimData->ranges[i].offset
			|| (off_t)(uint64)sizeBytes != trimData->ranges[i].size)
			return B_BAD_VALUE;
