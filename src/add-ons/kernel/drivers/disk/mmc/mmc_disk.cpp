...
mmc_block_get_geometry(mmc_disk_driver_info* info, device_geometry* geometry)
{
	...
	TRACE("Ready to execute %p\n", info->mmc->execute_command);
	info->mmc->execute_command(info->parent, SD_SEND_CSD,
...
mmc_disk_init_driver(device_node* node, void** cookie)
{
	...
	error = mmc_block_get_geometry(info, &info->geometry);
	if (error != B_OK) {
...
mmc_block_ioctl(void* cookie, uint32 op, void* buffer, size_t length)
{
	...
	switch (op) {
		...
		case B_GET_GEOMETRY:
		{
			...
			return user_memcpy(buffer, &info->geometry, sizeof(device_geometry));
		}
