		...
		fBusyCount(0)
{
	...
	if (_ReadReg(IETP_MAX_X_AXIS, sizeof(buf), &buf) != B_OK) {
		TRACE_ALWAYS("failed reading max x\n");
		return;
	}
	fHardwareSpecs.areaEndX = buf;

	if (_ReadReg(IETP_MAX_Y_AXIS, sizeof(buf), &buf) != B_OK) {
	...
	size_t length)
{
	switch (op) {
		...
		case MS_READ_TOUCHPAD:
		{
			...
			while (true) {
				status_t result = _ReadAndParseReport(&read.u.touchpad, read.timeout, zero_report_count);
...
{
	return _WriteReg(IETP_CONTROL, enable ? IETP_CTRL_ABSOLUTE : IETP_CTRL_STANDARD);
}
...
ELANDevice::_WaitForReport(bigtime_t timeout)
{
	...
	status_t result = _MaybeScheduleTransfer(HID_REPORT_TYPE_INPUT, fReportID, fHighPrecision ? HI_ELAN_REPORT_SIZE : LO_ELAN_REPORT_SIZE);
	if (result != B_OK) {
...
ELANDevice::_ReadAndParseReport(touchpad_movement *info, bigtime_t timeout, int &zero_report_count)
{
	...
	if (finger_cnt) {
		info->xPosition = sumx / finger_cnt;
		info->yPosition = sumy / finger_cnt;
		info->zPressure = sumz / finger_cnt;
		info->fingerWidth = MIN(12, sumw / finger_cnt);
	}

	TRACE("REsulting position=[%d, %d, %d, %d]\n", info->xPosition, info->yPosition, info->zPressure, info->fingerWidth);
...
status_t
ELANDevice::_ReadReg(uint16_t reg, size_t len, void *val)
{
	uint8_t cmd[2] = {
		(uint8_t) (reg & 0xff), (uint8_t) ((reg >> 8) & 0xff) };
	status_t status = _FetchBuffer(cmd, sizeof(cmd), fTransferBuffer,
		len);
	TRACE("Read reg 0x%04x with value 0x%02x 0x%02x status=%d\n", reg, fTransferBuffer[0], fTransferBuffer[1], status);
	if (status != B_OK)
		return status;
	memcpy(val, fTransferBuffer, len);
	return B_OK;
}

status_t
ELANDevice::_WriteReg(uint16_t reg, uint16_t val)
{
	uint8_t cmd[4] = { (uint8_t) (reg & 0xff),
		(uint8_t) ((reg >> 8) & 0xff),
		(uint8_t) (val & 0xff),
		(uint8_t) ((val >> 8) & 0xff) };
	TRACE("Write reg 0x%04x with value 0x%04x\n", reg, val);
