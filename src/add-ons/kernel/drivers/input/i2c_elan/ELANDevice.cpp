...
ELANDevice::_WaitForReport(bigtime_t timeout)
{
	...
	status_t result = _MaybeScheduleTransfer(
		0x01, fReportID, fHighPrecision ? HI_ELAN_REPORT_SIZE : LO_ELAN_REPORT_SIZE);
	if (result != B_OK) {
...
status_t
ELANDevice::_ReadReg(uint16_t reg, size_t length, void *value)
{
	uint8_t cmd[2] = {
...
status_t
ELANDevice::_WriteReg(uint16_t reg, uint16_t value)
{
