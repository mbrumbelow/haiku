...
static bool is_elan_name(const char *name) {
	if (!name)
		return false;
	for (unsigned i = 0; i < sizeof(elan_iic_devs) / sizeof(elan_iic_devs[0]); i++)
...
i2c_elan_init_driver(device_node *node, void **driverCookie)
{
	...
	*driverCookie = device;
	...
	mutex_lock(&sDriverLock);
	ELANDevice *elanDevice
		= new(std::nothrow) ELANDevice(node, i2c, i2c_cookie);
	...
	return device->elanDevice ? B_OK : B_IO_ERROR;
}
