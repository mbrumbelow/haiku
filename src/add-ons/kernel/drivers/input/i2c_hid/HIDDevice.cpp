...
HIDDevice::_Reset()
{
	...
	status = fI2C->exec_command(fI2CCookie, I2C_OP_WRITE_STOP, cmd, sizeof(cmd),
		NULL, 0);
	if (status != B_OK) {
		_SetPower(I2C_HID_POWER_OFF);
		return status;
...
HIDDevice::_SetPower(uint8 power)
{
	...
	return fI2C->exec_command(fI2CCookie, I2C_OP_WRITE_STOP, cmd, sizeof(cmd),
		NULL, 0);
}
...
HIDDevice::_FetchBuffer(uint8* cmd, size_t cmdLength, void* buffer,
	size_t bufferLength)
{
	return fI2C->exec_command(fI2CCookie, I2C_OP_READ_STOP, cmd, cmdLength,
		buffer, bufferLength);
}
