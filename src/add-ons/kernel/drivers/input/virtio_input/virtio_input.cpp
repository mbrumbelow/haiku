...
{
	switch (pkt.type) {
		case virtioInputEvSyn: dprintf("syn"); break;
		case virtioInputEvKey: dprintf("key");
			dprintf(", ");
			switch (pkt.code) {
				case virtioInputBtnLeft:     dprintf("left");     break;
				case virtioInputBtnRight:    dprintf("middle");   break;
				case virtioInputBtnMiddle:   dprintf("right");    break;
				case virtioInputBtnGearDown: dprintf("gearDown"); break;
				case virtioInputBtnGearUp:   dprintf("gearUp");   break;
				default: dprintf("%d", pkt.code);
			}
			break;
		case virtioInputEvRel: dprintf("rel");
			dprintf(", ");
			switch (pkt.code) {
				case virtioInputRelX:     dprintf("relX");     break;
				case virtioInputRelY:     dprintf("relY");     break;
				case virtioInputRelZ:     dprintf("relZ");     break;
				case virtioInputRelWheel: dprintf("relWheel"); break;
				default: dprintf("%d", pkt.code);
			}
			break;
		case virtioInputEvAbs: dprintf("abs");
			dprintf(", ");
			switch (pkt.code) {
				case virtioInputAbsX: dprintf("absX"); break;
				case virtioInputAbsY: dprintf("absY"); break;
				case virtioInputAbsZ: dprintf("absZ"); break;
				default: dprintf("%d", pkt.code);
			}
			break;
		case virtioInputEvRep: dprintf("rep"); break;
		default: dprintf("?(%d)", pkt.type);
	}
	switch (pkt.type) {
		case virtioInputEvSyn: break;
		case virtioInputEvKey: dprintf(", "); if (pkt.value == 0) dprintf("up"); else if (pkt.value == 1) dprintf("down"); else dprintf("%d", pkt.value); break;
...
virtio_input_supports_device(device_node *parent)
{
	...
	if (sDeviceManager->get_attr_uint16(parent, VIRTIO_DEVICE_TYPE_ITEM,
			&deviceType, true) != B_OK || deviceType != virtioDevInput)
		return 0.0;
