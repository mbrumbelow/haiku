...
virtio_input_ioctl(void* cookie, uint32 op, void* buffer, size_t length)
{
	...
	switch (op) {
		...
		case virtioInputGetConfig: {
			...
			res = info->virtio->write_device_config(info->virtio_device, 0, &config, offsetof(VirtioInputConfig, size));
			if (res < B_OK)
				return res;

			res = info->virtio->read_device_config(info->virtio_device, 0, &config, sizeof(VirtioInputConfig));
			if (res < B_OK)
				return res;
