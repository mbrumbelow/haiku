...
amd_thermal_read(void* _cookie, off_t position, void *buf, size_t* num_bytes)
{
	...
	if (position == 0) {
		...
		int32 copied = snprintf(buffer, sizeof(buffer), "  Current Temperature: %" B_PRIu32 ".%"
...
amd_thermal_support(device_node *parent)
{
	...
	const uint16 devices[] = { 0x1450, 0x1480, 0x15d0, 0x1630, 0x14a4, 0x14b5, 0x14d8, 0x14e8,
		0x1507, 0 };
...
amd_thermal_register_device(device_node *node)
{
	...
	return sDeviceManager->register_node(node, AMD_THERMAL_MODULE_NAME, attrs,
		NULL, NULL);
}
