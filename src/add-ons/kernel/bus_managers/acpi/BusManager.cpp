...
get_name(uacpi_namespace_node* handle, uint32 nameType, char* returnedName,
	size_t bufferLength)
{
	// TODO handle nameType (ACPI_FULL_PATHNAME or ACPI_FULL_PATHNAME_NO_TRAILING or just the name)
...
{
	// TODO uacpi API has no "handlerType"?
...
update_all_gpes()
{
	return AcpiUpdateAllGpes() == UACPI_STATUS_OK ? B_OK : B_ERROR;
}
...
{
	return AcpiSetGpe(handle, gpeNumber, action) == UACPI_STATUS_OK ? B_OK : B_ERROR;
}
...
{
	// TODO uacpi can have only one handler per address space, is that OK?
	return uacpi_install_address_space_handler(handle, spaceId, handler, data)
		== UACPI_STATUS_OK ? B_OK : B_ERROR;
}


// TODO passing the handler back is not needed
status_t
...
{
	return uacpi_read_register(regid, val);
}
...
{
	return uacpi_write_register(regid, val);
}
