...
status_t
get_handle(uacpi_namespace_node* parent, const char *pathname, acpi_handle *retHandle)
{
	return uacpi_namespace_node_find(parent, pathname, (uacpi_namespace_node**)&retHandle) == UACPI_STATUS_OK
		? B_OK : B_ERROR;
}
	...
	acpi_notify_handler handler, void *context)
{
	...
	return uacpi_install_notify_handler(device, (uacpi_notify_handler)handler, context)
	...
	acpi_notify_handler handler)
{
	...
	return uacpi_uninstall_notify_handler(device, (uacpi_notify_handler)handler)
		== UACPI_STATUS_OK ? B_OK : B_ERROR;
}
...
status_t
install_gpe_handler(uacpi_namespace_node* handle, uint32 gpeNumber, uint32 type,
	acpi_gpe_handler handler, void *data)
{
	return uacpi_install_gpe_handler(handle, gpeNumber, (uacpi_gpe_triggering)type,
		(uacpi_gpe_handler)handler, data) == UACPI_STATUS_OK ? B_OK : B_ERROR;
}
...
{
	return uacpi_uninstall_gpe_handler(handle, gpeNumber, (uacpi_gpe_handler)address)
		== UACPI_STATUS_OK ? B_OK : B_ERROR;
}


status_t
install_address_space_handler(uacpi_namespace_node* handle, uint32 spaceId,
	acpi_adr_space_handler handler, void *data)
{
	// FIXME not a valid conversion, the address space handler has a different prototype, change
	// the definition of acpi_adr_space_handler to match uacpi_region_handler
	return uacpi_install_address_space_handler(handle, (uacpi_address_space)spaceId, (uacpi_region_handler)handler, data)
		== UACPI_STATUS_OK ? B_OK : B_ERROR;
}


status_t
remove_address_space_handler(uacpi_namespace_node* handle, uint32 spaceId,
	acpi_adr_space_handler handler)
{
	// TODO passing the handler back is not needed (ACPICA used it to sanity check that the handler
	// was indeed the installed one). Remove it from the parameters or mark it as unused.
	return uacpi_uninstall_address_space_handler(handle, (uacpi_address_space)spaceId)
		== UACPI_STATUS_OK ? B_OK : B_ERROR;
}
...
{
	uacpi_enable_fixed_event((uacpi_fixed_event)event);
}
...
{
	uacpi_disable_fixed_event((uacpi_fixed_event)event);
}
...
fixed_event_status(uint32 event)
{
	...
	if (uacpi_fixed_event_info((uacpi_fixed_event)event, &status) != UACPI_STATUS_OK)
...
{
	uacpi_clear_fixed_event((uacpi_fixed_event)event);
}


status_t
install_fixed_event_handler(uint32 event, acpi_event_handler handler, void *data)
{
	return uacpi_install_fixed_event_handler((uacpi_fixed_event)event, (uacpi_interrupt_handler)handler, data) == UACPI_STATUS_OK
		? B_OK : B_ERROR;
}
...
remove_fixed_event_handler(uint32 event, acpi_event_handler handler)
{
	...
	return uacpi_uninstall_fixed_event_handler((uacpi_fixed_event)event) == UACPI_STATUS_OK
		? B_OK : B_ERROR;
}
...
{
	uacpi_namespace_node* parent, *newChild;
	uacpi_status status;
	...
	newChild = (uacpi_namespace_node*)*counter;

	status = uacpi_namespace_node_next_typed(parent, &newChild, (uacpi_object_type_bits)objectType);
	if (status != UACPI_STATUS_OK)
		return B_ENTRY_NOT_FOUND;
...
get_next_object(uint32 objectType, acpi_handle parent,
	acpi_handle* child)
{
	return uacpi_namespace_node_next_typed(parent, child, (uacpi_object_type_bits)objectType) == UACPI_STATUS_OK
		? B_OK : B_ERROR;
}
...
status_t
get_device_addr(const char *path, uint64 *addr)
{
	uacpi_namespace_node* handle;
	...
	uacpi_object* object;
	if (addr != NULL
		&& evaluate_method(handle, "_ADR", NULL, &object) == B_OK
		&& uacpi_object_is(object, UACPI_OBJECT_INTEGER)) {
		status = B_OK;
		uacpi_object_get_integer(object, addr);
	}
	return status;
}
...
get_object(const char* path, acpi_object_type** _returnValue)
{
	...
	status = uacpi_eval_simple(NULL, path, (uacpi_object**)_returnValue);
	...
	uint32 objectType)
{
	...
	status = uacpi_eval_simple_typed(NULL, path, (uacpi_object_type_bits)objectType, (uacpi_object**)_returnValue);
...
status_t
ns_handle_to_pathname(uacpi_namespace_node* targetHandle, const char** buffer)
{
...
status_t
evaluate_object(uacpi_namespace_node* handle, const char* object, acpi_objects *args,
	acpi_object_type* returnValue, size_t bufferLength)
{
	uacpi_status status;

	status = uacpi_eval(handle, object, (const uacpi_object_array*)args, (uacpi_object**)&returnValue);
	if (uacpi_unlikely_error(status))
		dprintf("acpi evaluate_object: %s\n", uacpi_status_to_string(status));
	...
	const uacpi_object_array *args, uacpi_object **returnValue)
{
	...
	status = uacpi_eval(handle, method, (const uacpi_object_array*)args, returnValue);
	if (uacpi_unlikely_error(status))
		dprintf("acpi evaluate_method: %s\n", uacpi_status_to_string(status));
...
get_irq_routing_table(uacpi_namespace_node* busDeviceHandle, uacpi_pci_routing_table **retBuffer)
{
	...
	if (uacpi_unlikely_error(status))
		dprintf("acpi get_irq_routing_table: %s\n", uacpi_status_to_string(status));
	...
	acpi_walk_resources_callback callback, void* context)
{
	return uacpi_for_each_device_resource(busDeviceHandle, method,
		(uacpi_resource_iteration_callback)callback, context);
}


status_t
walk_namespace(uacpi_namespace_node* busDeviceHandle, uint32 objectType,
	...
	acpi_walk_callback ascendingCallback, void* context)
{
	return uacpi_namespace_for_each_child(busDeviceHandle,
		(uacpi_iteration_callback)descendingCallback,
		(uacpi_iteration_callback)ascendingCallback,
		(uacpi_object_type_bits)objectType, maxDepth, context);
}
...
#	endif
	...
	acpiStatus = uacpi_prepare_for_sleep_state((uacpi_sleep_state)state);
	if (acpiStatus != UACPI_STATUS_OK)
		return B_ERROR;
...
enter_sleep_state(uint8 state)
{
	...
	status = uacpi_enter_sleep_state((uacpi_sleep_state)state);
...
{
	uacpi_status status = uacpi_table_find_by_signature((char*)signature, (uacpi_table*)tableHeader);
	if (uacpi_unlikely_error(status))
		return B_ERROR;

	for (uint32 i = 1; i < instance; i++) {
		status = uacpi_table_find_next_with_same_signature((uacpi_table*)tableHeader);
		if (uacpi_unlikely_error(status))
			return B_ERROR;
	}
