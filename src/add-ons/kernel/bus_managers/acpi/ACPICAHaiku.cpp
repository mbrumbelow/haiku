...
{
	status_t result;
	DEBUG_FUNCTION_F("vector: %" B_PRId32 "; handler: %p context %p",
	...
	DEBUG_FUNCTION_F("vector: %" B_PRId32 "; handler: %p context %p returned %" B_PRId32,
...
{
	DEBUG_FUNCTION_F("vector: %" B_PRId32 "; handler: %p", (uint32)interruptNumber,
		serviceRoutine);
#ifdef _KERNEL_MODE
...
{
	DEBUG_FUNCTION_F("microseconds: %" B_PRId32, (uint32)microseconds);
	if (microseconds)
		spin(microseconds);
}
...
{
	DEBUG_FUNCTION_F("milliseconds: %" B_PRId32, (uint32)milliseconds);
	if (gKernelStartup)
...
{
#ifdef _KERNEL_MODE
	DEBUG_FUNCTION_F("addr: 0x%08lx; width: %" B_PRId32, (addr_t)address, (uint32)width);
	switch (width) {
		case 8:
...
{
#ifdef _KERNEL_MODE
	DEBUG_FUNCTION_F("addr: 0x%08lx; value: %" B_PRId32 "; width: %" B_PRId32,
