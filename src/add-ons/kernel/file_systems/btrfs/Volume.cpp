...
btrfs_super_block::IsValid() const
{
	...
	if (strncmp(magic, BTRFS_SUPER_BLOCK_MAGIC, sizeof(magic)) != 0 &&
		strncmp(magic, BTRFS_SUPER_BLOCK_MAGIC_TEMPORARY, sizeof(magic)) != 0)
		return false;
		...
		uint32 blockSize, uint32 sectorSize)
{
	...
	uuid_generate(fsid);
	blocknum = B_LENDIAN_TO_HOST_INT64(BTRFS_RESERVED_SPACE_OFFSET);
	...
	root = B_HOST_TO_LENDIAN_INT64(blocknum + blockSize);
	...
	uint32 sectorSize)
{
	...
	ino_t id = GetNextInodeID();
	Inode* root_inode = Inode::Create(transaction, id, NULL,
		S_DIRECTORY | 0755);
	if (root_inode == NULL)
		return B_NO_MEMORY;

	// TODO(lesderid): Initialize remaining structures

	status_t status = _EraseUnusedBootBlock();
	if (status < B_OK)
		return status;
