...
Inode::GetFromDisk()
{
	...
	agNo = INO_TO_AGNO(fId, fVolume);	// Get the AG number from the inode
	agInodeNo = INO_TO_AGINO(fId, fVolume->AgInodeBits()); // AG relative ino #
	agBlock = INO_TO_AGBLOCK(agInodeNo, fVolume);
	offset = INO_TO_OFFSET(fId, fVolume);
	len = fVolume->InodeSize();
	fd = fVolume->Device();
