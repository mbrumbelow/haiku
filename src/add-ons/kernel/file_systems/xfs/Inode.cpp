...
Inode::GetPtrOffsetIntoNode(int pos)
{
	...
	uint32 longBlockSize = 0;
	if(Version() == 3)
		longBlockSize = sizeof(LongBlock);
	else
		longBlockSize = offsetof(struct LongBlock, bb_blkno);
	return longBlockSize + maxRecords * XFS_KEY_SIZE
		+ (pos - 1) * XFS_PTR_SIZE;
}
...
Inode::ReadExtentsFromTreeInode()
{
	...
	while (1) {
		...
		uint32 offset = 0;
		if(Version() == 3)
			offset = sizeof(LongBlock);
		else
			offset = offsetof(struct LongBlock, bb_blkno);
