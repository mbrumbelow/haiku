...
LeafDirectory::Init()
{
	...
	ASSERT(LeafType() == true);
	FillMapEntry(DIR_DFORK_PTR(fInode->Buffer()),
		fInode->NoOfDataExtents()-1, fLeafMap);
	FillMapEntry(DIR_DFORK_PTR(fInode->Buffer()), 0, fDataMap);
	return B_OK;
}

bool
LeafDirectory::LeafType()
{
	bool status = true;
	if (fInode->NoOfBlocks() == 1)
		status = false;
	if (fInode->Size() != (fInode->NoOfBlocks() - 1) * fInode->DirBlockSize())
		status = false;
	if (fInode->NoOfDataExtents() == 1)
		status = false;
	return status;
...
void
LeafDirectory::FillMapEntry(void* directoryFork, int num, ExtentMapEntry* fMap)
{
...
LeafDirectory::FillBuffer(int type, char* blockBuffer)
{
	...
	if (type == DATA) {
		...
		if (B_BENDIAN_TO_HOST_INT32(header->magic) == 0x58443244) {
...
LeafDirectory::GetNext(char* name, size_t* length, xfs_ino_t* ino)
{
	...
	void* endPointer = (void*)(fDataBuffer + fInode->DirBlockSize());

	while (entry != endPointer || fOffset == -1) {
		TRACE("fOffset:(%d)\n", fOffset);
		if (fOffset == -1) {
			fCurBlockNumber++;
			FillMapEntry((void*)DIR_DFORK_PTR(fInode->Buffer()), fCurBlockNumber, fDataMap);
			status = FillBuffer(DATA, fDataBuffer);
			if (status != B_OK)
				return status;
		...
		if (B_BENDIAN_TO_HOST_INT16(unusedEntry->freetag) == DIR2_FREE_TAG) {
			...
			if (entry == endPointer
				&& fCurBlockNumber < fInode->NoOfDataExtents()) fOffset = -1;
			continue;
		}
	...
	*/
	...
	while (B_BENDIAN_TO_HOST_INT32(leafEntry[left].hashval)
		...
		if (dataBlockNumber != fCurBlockNumber) {
			...
			FillMapEntry(DIR_DFORK_PTR(fInode->Buffer()), dataBlockNumber, fDataMap);
			status = FillBuffer(DATA, fDataBuffer);
			if (status != B_OK)
				return B_OK;
		}
