...
LeafDirectory::Init()
{
	...
	FillMapEntry(fInode->NoOfDataExtents()-1, fLeafMap);
...
LeafDirectory::IsLeafType()
{
	...
	if (fInode->NoOfBlocks() == 1
		...
		|| fInode->Size() != (fInode->NoOfBlocks() - 1) * fInode->DirBlockSize())
		status = false;
	...
	if (fLeafMap->br_startoff != LEAF_STARTOFFSET(fInode->GetVolume()->BlockLog()))
		status = false;
...
LeafDirectory::FillBuffer(int type, char* blockBuffer, int howManyBlocksFurthur)
{
	...
	uint64 agNo = FSBLOCKS_TO_AGNO(map->br_startblock + howManyBlocksFurthur, volume);
	uint64 agBlockNo = FSBLOCKS_TO_AGBLOCKNO(map->br_startblock + howManyBlocksFurthur, volume);
...
LeafDirectory::GetNext(char* name, size_t* length, xfs_ino_t* ino)
{
	...
	while (fOffset != curDirectorySize) {
		...
		if (fCurBlockNumber != blockNoFromAddress
			...
			&& blockNoFromAddress <= fDataMap->br_startoff + fDataMap->br_blockcount - 1) {
			// When the block is mapped in the same data map entry but is not the first block
			status = FillBuffer(DATA, fDataBuffer, blockNoFromAddress - fDataMap->br_startoff);
			if (status != B_OK)
				return status;
	...
	*/
	...
	while (B_BENDIAN_TO_HOST_INT32(leafEntry[left].hashval)
		...
		if (dataBlockNumber != fCurBlockNumber) {
			...
			status = FillBuffer(DATA, fDataBuffer, dataBlockNumber - fDataMap->br_startoff);
			if (status != B_OK)
				return B_OK;
		}
