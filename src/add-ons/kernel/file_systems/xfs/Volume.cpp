...
Volume::Identify(int fd, xfs_sb *superBlock)
{
    ...
    char *buffer;
    buffer = (char *)malloc(sizeof(char) * XFS_SB_MAXSIZE);

    if (read_pos(fd, 0, buffer, sizeof(buffer)) != sizeof(buffer))
        return B_IO_ERROR;

    /* Initialize and fill the SuperBlockObject */
    xfs_sb superBlockObject;
    superBlockObject.FillSuperBlock(buffer);
