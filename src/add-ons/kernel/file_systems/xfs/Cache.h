...
public:
	...
	inline status_t			SetToOffset(off_t block,off_t base, off_t length);
...
{
	status_t status=SetToOffset(block,block,1);
	if(status != B_OK) {
...
CachedBlock::SetToOffset(off_t blockNum, off_t base, off_t length) {
	...
	status_t status = block_cache_get_etc(fVolume->BlockCache(), blockNum, base, length,
										  (const void**)&fBlock);
	if (status != B_OK) {
		ERROR("CACHE IO ERROR");
		return status;
