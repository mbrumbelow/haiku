...
TreeDirectory::GetAllExtents()
{
	...
	while (1) {
		...
		TRACE("Next leaf is at: (%" B_PRIdOFF ")\n", fileSystemBlockNo);
 ...
 */
status_t
TreeDirectory::SearchMapInAllExtent(off_t blockNo, uint32& mapIndex)
{
	ExtentMapEntry map;
...
void
TreeDirectory::SearchForMapInDirectoryBlock(off_t blockNo,
	int entries, ExtentMapEntry** map, int type, int pathIndex)
{
	TRACE("SearchForMapInDirectoryBlock: blockNo:(%" B_PRIdOFF ")\n", blockNo);
