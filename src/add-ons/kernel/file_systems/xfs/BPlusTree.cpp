	...
	for (int i = maxRecords - 1; i >= 0; i--) {
		...
		if (B_BENDIAN_TO_HOST_INT64(*offsetKey) <= offset) {
			*pointer = GetPtrFromNode(i + 1, (void*)
				pathForLeaves[pathIndex].blockData);
...
TreeDirectory::GetAllExtents()
{
	...
	//TODO: Consider using AutoDeleter to not manually handle memory leak issues
 ...
 */
uint64
TreeDirectory::SearchMapInAllExtent(int blockNo)
{
	ExtentMapEntry map;
	...
	// since that'd contain all extent maps of the directory.
	
	// if we find a map it'd be in the range of uint32
	// 2^32 is out of that range.
	return (1<<32);
}
...
TreeDirectory::GetNext(char* name, size_t* length, xfs_ino_t* ino)
{
	...
	while (fOffset != curDirectorySize) {
		...
		if (fCurBlockNumber != blockNoFromAddress
			...
			uint64 curMapIndex = SearchMapInAllExtent(blockNoFromAddress);
			if (curMapIndex == (1<<32)) {
				return B_BAD_DATA;
			} else
	...
	int type, int pathIndex)
{
	void* pointerToMap;
	if (type == DATA) {
		char* base = pathForData[pathIndex].blockData + BlockLen();
		off_t offset = num * EXTENT_SIZE;
		pointerToMap = (void*)(base + offset);
	} else {
		char* base = pathForLeaves[pathIndex].blockData + BlockLen();
		off_t offset = num * EXTENT_SIZE;
		pointerToMap = (void*)(base + offset);
	}
