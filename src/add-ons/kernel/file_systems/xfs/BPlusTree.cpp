	...
	for (int i = maxRecords - 1; i >= 0; i--) {
		...
		if (B_BENDIAN_TO_HOST_INT64(*offsetKey) <= offset) {
			*pointer = GetPtrFromNode(i+1,
						(void*)fPathForLeaves[pathIndex].blockData);
			break;
		}
	}
	...
	ExtentMapEntry* targetMap)
{
	...
	if (targetMap == NULL) {
		...
		if (B_BENDIAN_TO_HOST_INT32(header->magic) 
			== DATA_HEADER_MAGIC) {
	...
	int type, int pathIndex)
{
	...
	if (type == DATA) {
		pointerToMap = (void*)
			(fPathForData[pathIndex].blockData 
				+ BlockLen() + num * EXTENT_SIZE);
	} else {
		pointerToMap = (void*)
...
TreeDirectory::Lookup(const char* name, size_t length, xfs_ino_t* ino)
{
	...
	while (1) {
		...
		uint32 lastHashVal
			= B_BENDIAN_TO_HOST_INT32(leafEntry[numberOfLeafEntries - 1].hashval);
		...
		if (lastHashVal == hashValueOfRequest && nextLeaf != -1) {
			...
			FillBuffer(fSingleDirBlock, nextLeaf 
				- targetMap->br_startoff, targetMap);
