	...
	ExtentMapEntry* targetMap)
{
	...
	if (targetMap == NULL) {
		...
		if (header == NULL)
			return B_NO_MEMORY;
		if (!VerifyHeader1<ExtentDataHeader>(header, fSingleDirBlock, fInode,
				howManyBlocksFurther, &map)) {
	...
	if (targetMap != NULL) {
		...
		*/
		...
		if ((leaf->Magic() == XFS_DIR2_LEAFN_MAGIC
			...
			&& !VerifyHeader1<ExtentLeafHeader>(leaf, fSingleDirBlock, fInode,
				howManyBlocksFurther, &map)) {
		...
		if ((node->Magic() == XFS_DA_NODE_MAGIC
			...
			&& !VerifyHeader1<NodeHeader>(node, fSingleDirBlock, fInode,
				howManyBlocksFurther, &map)) {
