...
bool
NodeDirectory::VerifyDataHeader(ExtentDataHeader* header, int howManyBlocksFurther)
{
	...
	uint64 actualBlockToRead =
		fInode->FileSystemBlockToAddr(fDataMap->br_startblock
...
bool
NodeDirectory::VerifyLeafHeader(ExtentLeafHeader* header, int howManyBlocksFurther)
{
	...
	uint64 actualBlockToRead =
		fInode->FileSystemBlockToAddr(fLeafMap->br_startblock
					+ howManyBlocksFurther) / XFS_MIN_BLOCKSIZE;
...
bool
NodeDirectory::VerifyNodeHeader(NodeHeader* header, int howManyBlocksFurther)
{
	TRACE("VerifyNodeHeader\n");
...
NodeDirectory::FillBuffer(int type, char* blockBuffer, int howManyBlocksFurthur)
{
	...
	if (type == DATA) {
		...
		if(header == NULL)
			return B_NO_MEMORY;
		if (!VerifyDataHeader(header, howManyBlocksFurthur)) {
		...
		*/
		...
		if ((leaf->Magic() == XFS_DIR2_LEAFN_MAGIC
			...
			&& !VerifyLeafHeader(leaf, howManyBlocksFurthur)) {
		...
		if ((node->Magic() == XFS_DA_NODE_MAGIC
			...
			&& !VerifyNodeHeader(node, howManyBlocksFurthur)) {
