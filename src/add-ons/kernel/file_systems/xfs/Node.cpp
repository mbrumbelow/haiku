...
NodeDirectory::FillBuffer(int type, char* blockBuffer, int howManyBlocksFurthur)
{
	...
	if (type == DATA) {
		...
		if(header == NULL)
			return B_NO_MEMORY;
		if (!VerifyHeader1<ExtentDataHeader>(header, fDataBuffer, fInode,
				howManyBlocksFurthur, fDataMap)) {
		...
		*/
		...
		if ((leaf->Magic() == XFS_DIR2_LEAFN_MAGIC
			...
			&& !VerifyHeader1<ExtentLeafHeader>(leaf, fLeafBuffer, fInode,
				howManyBlocksFurthur, fLeafMap)) {
		...
		if ((node->Magic() == XFS_DA_NODE_MAGIC
			...
			&& !VerifyHeader1<NodeHeader>(node, fLeafBuffer, fInode,
				howManyBlocksFurthur, fLeafMap)) {
