		...
		if (bplustree_node::LinkType(oldValue)
			...
			if (arrayCount < NUM_FRAGMENT_VALUES) {
				...
				node->SetValueAt(index, HOST_ENDIAN_TO_BFS_INT64(
					bplustree_node::MakeLink(BPLUSTREE_DUPLICATE_NODE,
						offset)));
			}
	...
	uint16* _keyLength, off_t* _value)
{
	...
	for (in = out = 0; in < node->NumKeys() + 1;) {
		keyLengths = BFS_ENDIAN_TO_HOST_INT16(node->KeyLengthAt(in));
...
BPlusTree::_RemoveKey(bplustree_node* node, uint16 index)
{
	...
	for (uint16 i = index; i < node->NumKeys(); i++) {
		node->SetKeyLengthAt(i, HOST_ENDIAN_TO_BFS_INT16(
			BFS_ENDIAN_TO_HOST_INT16(node->KeyLengthAt(i + 1)) - length));
	}
