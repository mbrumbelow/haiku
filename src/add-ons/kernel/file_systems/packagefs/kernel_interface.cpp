...
lock_directory_for_node(Volume* volume, Node* node, DirectoryReadLocker& locker)
{
	if (Directory* directory = dynamic_cast<Directory*>(node)) {
		...
		// We need to be sure the parent won't be changed or deleted from under us.
		// TODO: It should be possible to atomically synchronize Node and Directory
		// and avoid acquiring the global Volume lock here.
		volume->ReadLock();
		locker.SetTo(node->Parent(), false, true);
		volume->ReadUnlock();
		return locker.IsLocked();
	}
}
	...
	int* _type, uint32* _flags, bool reenter)
{
	...
	if (Directory* directory = dynamic_cast<Directory*>(node)) {
		...
		dirLocker.SetTo(node->Parent(), false, true);
		if (!dirLocker.IsLocked() || node->Parent() != dirLocker.Get())
			return B_NO_INIT;
	}
	volumeLocker.Unlock();
	...
	size_t* _bufferSize)
{
	...
	DirectoryReadLocker dirLocker(node->Parent());
	if (!dirLocker.IsLocked() || node->Parent() != dirLocker.Get())
		return B_NO_INIT;
