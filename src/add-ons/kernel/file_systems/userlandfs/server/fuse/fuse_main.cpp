...
struct fuse_session *fuse_lowlevel_new(struct fuse_args *args,
				       const struct fuse_lowlevel_ops *lowLevelOps,
				       size_t lowLevelOpSize, void *userData)
{
	// parse args
	fuse_config config;
	memset(&config, 0, sizeof(config));
	...
	struct fuse_operations* op = FUSELowLevelWrapper(lowLevelOps, lowLevelOpSize);

	// run the main loop
	status_t error = FUSEFileSystem::GetInstance()->FinishInitClientFS(&config,
		op, sizeof(struct fuse_operations), userData);
