...
FUSEVolume::WriteStat(void* _node, const struct stat* st, uint32 mask)
{
	...
	if (fOps != NULL) {
		// FIXME do we need to convert the mask, or are userlandfs and fuse access masks
		// compatible?
	...
	if (cookie->entryCache == NULL) {
		...
		if (fOps != NULL) {
			...
			if (fuseError > 0) {
				...
				while (countRead < count && (char*)dirent + dirent->d_reclen <= (char*)buffer + fuseError) {
					countRead++;
				...
				fprintf(stderr, "... read %u entries, buffer size %d\n", countRead, fuseError);
