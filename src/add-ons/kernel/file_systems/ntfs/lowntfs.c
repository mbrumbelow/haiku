...
#else
	const char* ghost_prefix = ".ghost-ntfs-3g-";
	if (!no_check_open && strncmp(ghost_prefix, name, strlen(ghost_prefix)) != 0) {
		int* close_state = ntfs_haiku_get_close_state(ctx, ino);
			...
			const char *tmp)
{
	...
	if (!ret) {
		...
		ret = ntfs_fuse_rm(ctx, parent, name, RM_ANY);
		if (ret) {
