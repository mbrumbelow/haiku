	...
	off_t pos, const iovec* vecs, size_t count, size_t* _numBytes)
{
	...
	size_t bytesLeft = *_numBytes = min_c(*_numBytes, ni->data_size - pos);
	for (size_t i = 0; i < count && bytesLeft > 0; i++) {
		const size_t ioSize = min_c(bytesLeft, vecs[i].iov_len);
		if (ntfs_fuse_read(ni, pos, (char*)vecs[i].iov_base, ioSize) != ioSize) {
			*_numBytes = *_numBytes - bytesLeft;
			return errno;
		}
		pos += ioSize;
		_numBytes += ioSize;
	...
	off_t pos, const iovec* vecs, size_t count, size_t* _numBytes)
{
	...
	for (size_t i = 0; i < count && bytesLeft > 0; i++) {
		const size_t ioSize = min_c(bytesLeft, vecs[i].iov_len);
		if (ntfs_fuse_write(&volume->lowntfs, ni, (char*)vecs[i].iov_base, ioSize, pos) != ioSize) {
			*_numBytes = *_numBytes - bytesLeft;
			return errno;
		}
		pos += ioSize;
