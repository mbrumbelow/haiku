	...
	off_t pos, const iovec* vecs, size_t count, size_t* _numBytes)
{
	...
	for (size_t i = 0; i < count; i++) {
		if (ntfs_fuse_read(ni, pos, (char*)vecs[i].iov_base, vecs[i].iov_len) != 0)
			return errno;
		pos += vecs[i].iov_len;
...
fs_lookup(fs_volume* _volume, fs_vnode* _directory, const char* name,
	ino_t* _vnodeID)
{
	TRACE("fs_lookup: name address: %p (%s)", name, name);
	volume* volume = (struct volume*)_volume->private_volume;
	MutexLocker lock(volume->lock);
	...
	TRACE("fs_lookup: ID %" B_PRIdIND, *_vnodeID);
	return B_OK;
}
...
fs_read_stat(fs_volume* _volume, fs_vnode* _node, struct stat* stat)
{
	...
	int result = ntfs_fuse_getstat(&volume->lowntfs, NULL, ni, stat);
	return result;
}
...
fs_open_dir(fs_volume* _volume, fs_vnode* _node, void** _cookie)
{
	...
	if ((ni->mrec->flags & MFT_RECORD_IS_DIRECTORY) == 0)
...
static int
_directory_list_builder(void* _cookie, const ntfschar* ntfs_name, const int ntfs_name_len,
	...
	struct dirent* dirent, size_t bufferSize, uint32* _num)
{
	...
	// TODO: It would probably be better for larger directories if we did *not*
	// read everything at once into memory, but that is a bit more difficult.
	...
	if (cookie->first == NULL) {
		...
		s64 pos = 0;
		if (ntfs_readdir(ni, &pos, cookie, _directory_list_builder) != 0)
			return errno;
