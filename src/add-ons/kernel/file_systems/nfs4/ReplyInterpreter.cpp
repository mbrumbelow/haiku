...
static status_t
process_stream(RPC::Reply* reply, const char* callname)
{
...
ReplyInterpreter::Access(uint32* supported, uint32* allowed)
{
	...
	return process_stream(fReply, __func__);
}
...
ReplyInterpreter::_OperationError(Opcode op)
{
	...
	if (fReply->Error() != B_OK || fReply->Stream().IsEOF()) {
		dprintf("nfs4: Error not B_OK or empty stream!\n");
		fDecodeError = true;
		return fReply->Error();
	}

	if (fReply->Stream().GetInt() != op) {
		dprintf("nfs4: Stream GetInt != op!\n");
		fDecodeError = true;
		return B_BAD_VALUE;
	}
