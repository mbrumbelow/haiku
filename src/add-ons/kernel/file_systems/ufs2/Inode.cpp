...
Inode::ReadAt(off_t file_offset, uint8* buffer, size_t* _length)
{
	...
	int64_t size = Size();
	off_t blockNumber = file_offset / blockSize;
	off_t blockOffset = file_offset % blockSize;
	off_t pos = FindBlock(blockNumber, blockOffset);
	off_t totalBlockSize = (blockNumber + 1) * blockSize;
	ssize_t remainingLength = totalBlockSize - file_offset;
	ssize_t length = 0;
	if (file_offset + *_length > (blockNumber + 1) * blockSize) {
		while (true) {
