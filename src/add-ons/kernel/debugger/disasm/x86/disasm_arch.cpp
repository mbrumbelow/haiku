...
extern "C" void
diasm_arch_assert_fail(const char *assertion, const char *file,
	...
	int backCount)
{
	...
	if (backCount > 0) {
		...
		while (address < where
			...
			&& ZYAN_SUCCESS(ZydisDecoderDecodeInstruction(&sDecoder, (ZydisDecoderContext*)ZYAN_NULL, buffer, sizeof(buffer), &instruction))) {
	...
	for (int i = 0; i < count; i++, offset += instruction.length) {
		if (debug_memcpy(B_CURRENT_TEAM, &buffer, (const void*)(baseAddress + offset), sizeof(buffer)) != B_OK) {
			kprintf("<read fault>\n");
			break;
		}
		ZydisDecodedOperand operands[ZYDIS_MAX_OPERAND_COUNT];
		if (!ZYAN_SUCCESS(ZydisDecoderDecodeFull(&sDecoder, buffer, sizeof(buffer), &instruction, operands)))
			break;
		if (skipCount > 0) {
			skipCount--;
			continue;
		...
		char formatted[1024];
		if (ZYAN_SUCCESS(ZydisFormatterFormatInstruction(&sFormatter, &instruction, operands, instruction.operand_count_visible,
			formatted, sizeof(formatted), baseAddress + offset, NULL))) {
			kprintf("%#16llx: %16.16s\t%s\n", static_cast<unsigned long long>(address), hexString, formatted);
		} else
...
disasm_arch_init()
{
	...
	ZydisFormatterSetProperty(&sFormatter, ZYDIS_FORMATTER_PROP_ADDR_PADDING_ABSOLUTE, ZYDIS_PADDING_DISABLED);
	ZydisFormatterSetProperty(&sFormatter, ZYDIS_FORMATTER_PROP_ADDR_PADDING_RELATIVE, ZYDIS_PADDING_DISABLED);
	ZydisFormatterSetProperty(&sFormatter, ZYDIS_FORMATTER_PROP_DISP_PADDING, ZYDIS_PADDING_DISABLED);
	ZydisFormatterSetProperty(&sFormatter, ZYDIS_FORMATTER_PROP_IMM_PADDING, ZYDIS_PADDING_DISABLED);
