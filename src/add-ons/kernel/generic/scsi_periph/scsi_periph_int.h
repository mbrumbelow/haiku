...
err_res periph_send_start_stop(scsi_periph_device_info* device, scsi_ccb* request, bool start,
	bool with_LoEj);
...
status_t periph_register_device(periph_device_cookie periph_device,
	...
	device_node* node, bool removable, int preferredCcbSize, scsi_periph_device* driver);
...
status_t periph_read_write(scsi_periph_device_info* device, scsi_ccb* request, uint64 offset,
...
status_t periph_io(scsi_periph_device_info* device, io_operation* operation,
	size_t* _bytesTransferred);
status_t periph_ioctl(scsi_periph_handle_info* handle, int op, void* buf, size_t len);
void periph_sync_queue_daemon(void* arg, int iteration);
status_t vpd_page_get(scsi_periph_device_info* device, scsi_ccb* request, uint8 page, void* data,
	uint16 length);
...
status_t periph_simple_exec(scsi_periph_device_info* device, void* cdb, uchar cdb_len, void* data,
	size_t data_len, int ccb_flags);


// sync.c

err_res periph_synchronize_cache(scsi_periph_device_info* device, scsi_ccb* request);
