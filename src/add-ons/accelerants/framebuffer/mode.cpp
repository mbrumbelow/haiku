...
framebuffer_set_display_mode(display_mode* _mode)
{
	...
	uint32 frame_buffer_size = max_mode->virtual_width * max_mode->virtual_height
		* gInfo->bytes_per_pixel;
...
framebuffer_get_frame_buffer_config(frame_buffer_config* config)
{
	...
	config->bytes_per_row = gInfo->current_emulated_mode.virtual_width
		* gInfo->bytes_per_pixel;
