...
radeon_set_brightness(float brightness)
{
	...
	uint32_t backlightRegister = radeon_get_backlight_register();
	uint8_t brightnessRaw = (uint8_t)ceilf(brightness * 255);
	uint32_t level = Read32(OUT, backlightRegister);
	...
	Write32(OUT, backlightRegister, level);
...
radeon_get_brightness(float* brightness)
{
	...
	uint32_t backlightRegister = Read32(OUT, radeon_get_backlight_register());
	uint8_t brightnessRaw = ((backlightRegister & ATOM_S2_CURRENT_BL_LEVEL_MASK) >>
