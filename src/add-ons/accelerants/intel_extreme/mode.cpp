...
{
	// we have to be cautious as refresh rate cannot be controlled directly,
	// so it suffers under rounding errors and hardware restrictions
	return int32(rint(10 * float(mode.timing.pixel_clock * 1000)
...
intel_get_pixel_clock_limits(display_mode* mode, uint32* _low, uint32* _high)
{
	...
	if (_high != NULL) {
		...
		for (uint32 i = 0; i < gInfo->port_count; i++) {
			...
			if (status == B_OK && info.max_frequency != 0 && info.max_frequency < high)
