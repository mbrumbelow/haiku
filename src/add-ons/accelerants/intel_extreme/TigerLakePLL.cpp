...
ProgramPLL(int which, int Pdiv, int Qdiv, int Kdiv, float dco)
{
	...
	uint32 dco_int = floorf(dco / ref);
	uint32 dco_frac = ceilf((dco / ref - dco_int) * (1 << 15));
	...
	TRACE("DFGCR0(%x) = %x, CFGCR1(%x) = %x (int = %d, frac = %d)\n", DPLL_CFGCR0, dco_reg,
