...
vesa_set_brightness(float brightness)
{
	...
	uint8_t rawValues[2];
	status_t result = ioctl(gInfo->device, VESA_GET_BRIGHTNESS, &rawValues, sizeof(rawValues));
	...
	return ioctl(gInfo->device, VESA_SET_BRIGHTNESS, &newValue, sizeof(newValue));
}
