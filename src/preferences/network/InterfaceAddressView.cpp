...
{
	syslog(LOG_DEBUG,"Inside updatefields");
...
InterfaceAddressView::_UpdateSettings()
{
	...
	int otherFamily = (AF_INET ^ AF_INET6) ^ fFamily;
	index = settings.FindFirstAddress(otherFamily);
	if (index < 0) {
		BNetworkInterfaceAddressSettings address;
		address.SetFamily(otherFamily);
		address.SetAutoConfigure(true);
		settings.AddAddress(address);
	}
	interface.MakeEmpty();
