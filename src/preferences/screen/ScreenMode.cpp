 ...
 * if [ $? -eq 0 ]; then
 *	sed -E -e 's:<thead>::g' -e 's:<tr.*="(.+)"><td>:{ ":g' -e 's:<td>[[:digit:]]{2}/[[:digit:]]{2}/[[:digit:]]{4}</td>::g' -e 's:(<\/td><td>):\", \":g' -e 's:<\/td>.<\/tr>:\" },:g' -e 's/"(.*?)", "(.*?)"/\"\2\", \"\1\"/' Vendors_temp.tmp | grep -v '<' | sort > Vendors.h
 * fi
 * rm Vendors_temp.tmp
 */
...
ScreenMode::GetMonitorInfo(monitor_info& info, float* _diagonalInches)
{
	...
	char vendor[5];
	strncpy(vendor, info.vendor, 3);
	vendor[4] = '\0';
	strcpy(info.vendor, GetManufacturerFromID(vendor));
