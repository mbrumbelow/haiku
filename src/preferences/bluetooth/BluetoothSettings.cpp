...
BluetoothSettings::LoadSettings(BluetoothSettingsData& settings) const
{
	bdaddr_t* tmp;
	ssize_t size;
	status_t status = fSettingsMessage.FindData("BDAddress", 'BTAD',
		(const void**)&tmp, &size);
	if (status == B_OK) {
	...
	DeviceClass* dcTmp;
	status = fSettingsMessage.FindData("DeviceClass", 'DECL',
		(const void**)&dcTmp, &size);
	if (status == B_OK) {
...
{
	fSettingsMessage.SetValue("DeviceClass", 'DECL', &settings.LocalDeviceClass
		, sizeof(DeviceClass));
	fSettingsMessage.SetValue("BDAddress", 'BTAD', &settings.PickedDevice,
		sizeof(bdaddr_t));
