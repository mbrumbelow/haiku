...
{
	fSettings.LoadSettings(fBluetoothSettingsData);
	...
	fPolicyMenu->SetValue(fBluetoothSettingsData.Policy);

	BString label(B_TRANSLATE("Default inquiry time:"));
	label <<  " " << fBluetoothSettingsData.InquiryTime;
	...
	fInquiryTimeControl->SetValue(fBluetoothSettingsData.InquiryTime);
	...
	if (ActiveLocalDevice != NULL) {
		...
		if (!rememberedClass.IsUnknownDeviceClass()) {
			fBluetoothSettingsData.LocalDeviceClass = rememberedClass;
		}
	}
...
{
	fSettings.SaveSettings(fBluetoothSettingsData);
}
	...
	switch (message->what) {
		...
		case kMsgSetConnectionPolicy:
		{
			...
			if (message->FindInt32("be:value", (int32*)&policy) == B_OK) {
				fBluetoothSettingsData.Policy = policy;
			}
			break;
		}
		...
		{
			fBluetoothSettingsData.InquiryTime = fInquiryTimeControl->Value();
			BString label(B_TRANSLATE("Default inquiry time:"));
	...
	uint16 service)
{
	...
	fBluetoothSettingsData.LocalDeviceClass.SetRecord(major, minor, service);

	if (ActiveLocalDevice != NULL)
		ActiveLocalDevice->SetDeviceClass(fBluetoothSettingsData.LocalDeviceClass);
	else
		haveRun = false;
	...
	for (uint32 i = 0; i < LocalDevice::GetLocalDeviceCount(); i++) {
		...
		if (lDevice != NULL) {
			...
			if (bdaddrUtils::Compare(lDevice->GetBluetoothAddress(),
				fBluetoothSettingsData.PickedDevice)) {
...
BluetoothSettingsView::_MarkLocalDevice(LocalDevice* lDevice)
{
	...
	fExtDeviceView->SetEnabled(true);
	ActiveLocalDevice = lDevice;
	fBluetoothSettingsData.PickedDevice = lDevice->GetBluetoothAddress();
}
...
{
	int deviceClass = fBluetoothSettingsData.LocalDeviceClass.MajorDeviceClass() +
		fBluetoothSettingsData.LocalDeviceClass.MinorDeviceClass();

	// As of now we only support MajorDeviceClass = 1 and MinorDeviceClass 1-4
	// and MajorDeviceClass = 2 and MinorDeviceClass 3.
	if (fBluetoothSettingsData.LocalDeviceClass.MajorDeviceClass() == 1
		&& (fBluetoothSettingsData.LocalDeviceClass.MinorDeviceClass() > 0
		&& fBluetoothSettingsData.LocalDeviceClass.MinorDeviceClass() < 5)) {
		return deviceClass-1;
	} else if (fBluetoothSettingsData.LocalDeviceClass.MajorDeviceClass() == 2
		&& fBluetoothSettingsData.LocalDeviceClass.MinorDeviceClass() == 3)
		return deviceClass;
	else
		return 0;
}
