...
static void
get_color_components(rgb_color& color, uint8*& primary, uint8*& secondary, uint8*& tertiary)
{
	if (color.blue >= max_c(color.red, color.green)) {
		primary = &color.blue;
	...
	} else if (color.red >= max_c(color.green, color.blue)) {
		primary = &color.red;
	...
	BView(name, B_WILL_DRAW)
{
	...
	fAutoAdjustCheckBox = new BCheckBox(B_TRANSLATE("Automatically adjust most colors"),
		new BMessage(AUTO_ADJUST_CHANGED));
	fAutoAdjustCheckBox->SetValue(true);
...
{
	fAutoAdjustCheckBox->SetTarget(this);
...
APRView::_CreateItems()
{
	...
	const bool autoAdjust = fAutoAdjustCheckBox->Value();
...
APRView::_SetColor(color_which which, rgb_color color)
{
	...
	if (which == B_PANEL_BACKGROUND_COLOR) {
		...
		const float tabColorSaturation = tabColorHSL.saturation != 0 ?
			tabColorHSL.saturation : tabColorHSL.lightness;
