...
BackgroundsView::_LoadSettings()
{
    ...
    int32 countOfRecentFolders = 0;
    if (fSettings.GetInfo("recentfolder", NULL, &countOfRecentFolders) == B_OK) {
    	BPrivate::TitledSeparatorItem* recentFoldersTitle = new BPrivate::TitledSeparatorItem(B_TRANSLATE("Recent folders"));
    	fWorkspaceMenu->AddItem(recentFoldersTitle);
        int32 index = countOfRecentFolders >= fRecentFoldersLimit ? countOfRecentFolders - fRecentFoldersLimit : 0;
        for (; index < countOfRecentFolders &&
               (fSettings.FindString("recentfolder", index, &string) == B_OK); index++) {
