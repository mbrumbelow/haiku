...
{
	int32 count = fInitialSettings.CountItems();
	for (int32 i = 0; i < count; i++) {
		Settings* settings = fInitialSettings.ItemAt(i);
...
BackgroundsView::_SetInitialSettings()
{
	if (_GetInitialSettings(current_workspace()) == NULL) {
		...
		settings->fImage = BackgroundImage::GetBackgroundImage(
			&node, true, this);
		settings->fInfo = settings->fImage->ImageInfoForWorkspace(
			current_workspace());
...
BackgroundsView::_SetControlSettings(int32 workspace)
{
	...
	controls->fPlacementIndex = fPlacementMenu->IndexOf(
		fPlacementMenu->FindMarked());
...
{
	Settings* bgInfo = _GetInitialSettings(current_workspace());
...
BackgroundsView::WorkspaceActivated(uint32 workspace, bool active)
{
	...
	Controls* controls = (Controls*)_GetControlSettings(
