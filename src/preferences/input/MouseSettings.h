...
public:
								...
								MouseSettings(mouse_settings settings, BString
									 name);
								~MouseSettings();
				...
				int32			MouseType() const
	{
		return fSettings.type;
	}
	...
	int32 MouseSpeed() const
	{
		return fSettings.accel.speed;
	}
...
public:
				...
				status_t		Archive(BMessage* into, bool deep = false)
									 const;
		...
		/** Get or create settings for the given mouse */
								MouseSettings*	AddMouseSettings(BString
									 mouse_name);
		/** Get the existing settings, or return NULL */
								MouseSettings*	GetMouseSettings(BString
									 mouse_name);
