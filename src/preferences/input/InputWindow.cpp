...
{
	fInputMouse = new InputMouse("Mouse");
	fTouchpadPrefView = new TouchpadPrefView(B_TRANSLATE("Touchpad"));
	fDeviceListView = new DeviceListView(B_TRANSLATE("Device List"));

	BView* mouseView = fInputMouse;
	BView* touchpadView = fTouchpadPrefView;

	fCardView = new BCardView();
	fCardView->AddChild(mouseView);
	fCardView->AddChild(touchpadView);
