		...
		B_FLOATING_WINDOW, B_NOT_RESIZABLE | B_NOT_ZOOMABLE | B_AUTO_UPDATE_SIZE_LIMITS)
{
	...
	_CreateMenuField(&fCapsMenu, &capsMenuField, MENU_ITEM_CAPS,
		B_TRANSLATE_COMMENT("Caps Lock:", "Caps Lock role name"));
	...
	// Update menus and hide/show status icons so menu fields grow right.
	// PostMessage because we're in the constructor.
	PostMessage(kMsgUpdateStatus);
}
...
void
ModifierKeysWindow::_CreateMenuField(BPopUpMenu** outMenu, BMenuField** outField, uint32 inKey,
	const char* label)
{
...
{
	if (fCapsConflictView->Icon() == NULL) {
		if (!fCapsConflictView->IsHidden())
			fCapsConflictView->Hide();
	} else {
		if (fCapsConflictView->IsHidden())
			fCapsConflictView->Show();
	}

	if (fShiftConflictView->Icon() == NULL) {
		if (!fShiftConflictView->IsHidden())
	...
	if (fControlConflictView->Icon() == NULL) {
		if (!fControlConflictView->IsHidden())
	...
	if (fOptionConflictView->Icon() == NULL) {
		if (!fOptionConflictView->IsHidden())
	...
	if (fCommandConflictView->Icon() == NULL) {
		if (!fCommandConflictView->IsHidden())
