...
TAnalogClock::DrawClock()
{
	...
	// set to control colors, slightly tinted so not b&w
	SetLowUIColor(B_CONTROL_BACKGROUND_COLOR, 1.05);
	SetHighUIColor(B_CONTROL_TEXT_COLOR, 1.95);
	SetPenSize(2.0);

	bool isLight = LowColor().IsLight();

	if (isLight)
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, B_DARKEN_1_TINT);
	else
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, 0.853);
	StrokeEllipse(bounds.OffsetByCopy(-1, -1));

	if (isLight)
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, B_LIGHTEN_2_TINT);
	else
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, 1.615);
	StrokeEllipse(bounds);

	if (isLight)
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, B_DARKEN_3_TINT);
	else
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, 0.593);
	StrokeEllipse(bounds.OffsetByCopy(1, 1));

	FillEllipse(bounds, B_SOLID_LOW);

	if (isLight)
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, B_DARKEN_2_TINT);
	else
		SetHighUIColor(B_CONTROL_BACKGROUND_COLOR, 0.705);
	...
	if (isLight)
		SetHighUIColor(B_CONTROL_TEXT_COLOR, B_DARKEN_1_TINT);
	else
		SetHighUIColor(B_CONTROL_TEXT_COLOR, 0.853);
	...
	rgb_color hourColor;
	if (fHourDragging)
		hourColor = ui_color(B_CONTROL_MARK_COLOR);
	else
		hourColor = HighColor();

	rgb_color minuteColor;
	if (fMinuteDragging)
		minuteColor = ui_color(B_CONTROL_MARK_COLOR);
	else
	...
	_DrawHands(fCenterX, fCenterY, fRadius,
		hourColor, minuteColor, secondsColor, HighColor());
