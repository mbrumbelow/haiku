...
int __wcsncasecmp_l(const wchar_t* wcs1, const wchar_t* wcs2, size_t count, locale_t locale);
...
# define STRNCASECMP(S1, S2, N) \
  strncasecmp_l ((S1), (S2), (N), _nl_C_locobj_ptr)
#endif
...
overflow_value (int negative)
{
  FLOAT result;
  __set_errno (ERANGE);
  result = math_narrow_eval ((negative ? -MAX_VALUE : MAX_VALUE)
				   * MAX_VALUE);
  return result;
}
...
underflow_value (int negative)
{
  FLOAT result;
  __set_errno (ERANGE);
  result = math_narrow_eval ((negative ? -MIN_VALUE : MIN_VALUE)
				   * MIN_VALUE);
  return result;
}
