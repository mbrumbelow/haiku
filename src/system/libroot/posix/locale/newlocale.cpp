...
newlocale(int category_mask, const char* locale, locale_t base)
{
    ...
    LocaleBackendData* locobj = (LocaleBackendData*)base;

    if (locobj == NULL) {
        locobj = new (std::nothrow) LocaleBackendData;
        if (locobj == NULL) {
            errno = ENOMEM;
            return (locale_t)0;
        }
        locobj->magic = LOCALE_T_MAGIC;
        locobj->backend = NULL;
        locobj->databridge = NULL;

        newObject = true;
    }

    LocaleBackend*& backend = locobj->backend;
