...
static pthread_once_t sLocaleInfoKeyInitOnce = PTHREAD_ONCE_INIT;
static pthread_key_t sLocaleInfoKey;
...
MakeLocaleInfoKey()
{
	pthread_key_create(&sLocaleInfoKey, NULL);
}

locale_t
GetCurrentLocaleInfo()
{
	pthread_once(&sLocaleInfoKeyInitOnce, MakeLocaleInfoKey);
	return (locale_t)pthread_getspecific(sLocaleInfoKey);
}

void
SetCurrentLocaleInfo(locale_t newLocale)
{
	pthread_once(&sLocaleInfoKeyInitOnce, MakeLocaleInfoKey);
	pthread_setspecific(sLocaleInfoKey, newLocale);
}


LocaleBackend* GetCurrentLocaleBackend()
{
	locale_t info = GetCurrentLocaleInfo();
	if (info != NULL && info->backend != NULL) {
		return (LocaleBackend*)info->backend;
	}
