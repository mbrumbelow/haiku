	...
	off_t offset)
{
	...
	if (((flags & MAP_ANONYMOUS) != 0) && ((flags & MAP_CLONE) != 0)) {
		fd = -1;
	} else if ((flags & MAP_CLONE) != 0 && fd < 0) {
		__set_errno(EBADF);
		return MAP_FAILED;
	}
	...
	if ((flags & MAP_CLONE) != 0) {
		...
		status_t status = _kern_clone_memory(targetTeam, &address, addressSpec,
