...
static int
crypt_gensalt_r(char *outbuf, size_t bufsize)
{
...
crypt_r(const char* key, const char* setting, struct crypt_data *data)
{
	...
	if (setting == NULL) {
		int res = crypt_gensalt_r(data->gensaltResult, sizeof(data->gensaltResult));

		// crypt_gensalt_r should set errno itself.
		if (res < 0)
			return NULL;

		setting = data->gensaltResult;
	}
	...
	if (strlen(setting) < 16) {
		crypt_legacy(key, setting, data->outBuffer);
		return data->outBuffer;
	}
	...
	snprintf(data->outBuffer, sizeof(data->outBuffer), "$s$%d$%s$%s", nLog2, saltString,
		hexResultBuffer);
