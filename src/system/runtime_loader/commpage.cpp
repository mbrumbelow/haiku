...
status_t
get_nearest_commpage_symbol_at_address_locked(void* address,
	...
	char** _symbolName, int32* _type, void** _location, bool* _exactMatch)
{
	...
	if (wantsSymbol && sCommpageSymbolCount == -1) {
		...
		status = _kern_read_kernel_image_symbols(systemCommpageID, symbols,
			&symbolCount, stringTable, &stringTableSize, NULL);
		if (status < B_OK) {
			free(buffer);
			return status;
		}
