...
static bool
match_symbol(image_t* image, const SymbolLookupInfo& lookupInfo, bool allowLocal, uint32 symIndex,
	match_result& result)
{
	elf_sym* symbol = &image->syms[symIndex];
	if (symbol->st_shndx == SHN_UNDEF)
		return false;
	if (!allowLocal && (symbol->Bind() != STB_GLOBAL && symbol->Bind() != STB_WEAK))
		return false;
	...
	uint32 versionID = image->symbol_versions[symIndex];
...
static elf_sym*
find_symbol_sysv(image_t* image, const SymbolLookupInfo& lookupInfo, bool allowLocal)
{
...
find_symbol(image_t* image, const SymbolLookupInfo& lookupInfo, bool allowLocal)
{
	...
	return find_symbol_sysv(image, lookupInfo, allowLocal);
}
