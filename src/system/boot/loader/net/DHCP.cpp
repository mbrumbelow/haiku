	...
	fSocket(NULL)
{
	...
	// create a transaction identifier. Ideally, this would be randomly
	// generated, but as we don't have a random generator, we'll
	// generate one using the mac address and system time which will
	// hopefully be random enough
	mac_addr_t mac = interface->MACAddress();
	fXid = (((uint32)mac[0] << 8)
			| ((uint32)mac[1])
			| ((uint32)mac[2] << 24)
			| ((uint32)mac[3] << 16)
