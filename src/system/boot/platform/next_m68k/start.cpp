...
inline void dump_mg(struct mon_global *mg)
{
	...
	dprintf("mg_sddp\t%p\n", mg->mg_sddp);
	dprintf("mg_dgp\t%p\n", mg->mg_dgp);	// "dlV3" (disk label signature ?) SCSI boot gives "@dlV3"
	...
	*/
	...
	dprintf("mg_si\t%p\n", mg->mg_si);
	/* XXX:the pointer seems completely random, sadly. Possibly the kernel is supposed to set it? */
	/*
	...
	for (i = 0; i < 5; i++) {
		...
		dprintf("sio[%d] = {%08x, %u, %u, %u, %p, %u, %u, %p, %p, %p}\n", i,
				...
				s->si_dev, s->si_blklen, s->si_lastlba, s->si_sadmem, s->si_protomem, s->si_devmem);
		s++;
	}
	*/
