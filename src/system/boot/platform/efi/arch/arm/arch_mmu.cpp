	...
	uint32_t descriptor_version)
{
	...
	while (mmu_next_region(&cookie, &vaddr, &paddr, &size)) {
		map_range(vaddr, paddr, size, ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | ARM_MMU_L2_FLAG_AP_RW);
	}
	...
	map_range(page_table_area, page_table_area, MAX_PAGE_TABLES*ARM_MMU_L2_COARSE_TABLE_SIZE,
		ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | ARM_MMU_L2_FLAG_AP_RW);
