...
static void dump_page_dir
(void)
{
	dprintf("=== Page Directory ===\n");
	for (uint32_t i = 0; i < ARM_MMU_L1_TABLE_ENTRY_COUNT; i++) {
		uint32 pde = sPageDirectory[i];
		if (pde) {
			dprintf("virt 0x%08x --> page table 0x%08x type 0x%08x\n",
				i<<20, pde&ARM_PDE_ADDRESS_MASK, pde&ARM_PDE_TYPE_MASK);
			uint32_t *pageTable = (uint32_t *)(pde&ARM_PDE_ADDRESS_MASK);
			for (uint32_t j = 0; j < ARM_MMU_L2_COARSE_ENTRY_COUNT; j++) {
				uint32 pte = pageTable[j];
				if (pte) {
					dprintf("virt 0x%08x     --> page 0x%08x type+flags 0x%08x\n",
						(i<<20)|(j<<12), pte&ARM_PTE_ADDRESS_MASK, pte&(~ARM_PTE_ADDRESS_MASK));
				}
			}
		}
	}
	...
	uint32_t descriptor_version)
{
	...
	for (uint32_t i = 0; i < gKernelArgs.num_physical_memory_ranges; i++) {
		dprintf("    0x%08x-0x%08x, length 0x%08x\n",
			(uint32_t)gKernelArgs.physical_memory_range[i].start,
			(uint32_t)(gKernelArgs.physical_memory_range[i].start + gKernelArgs.physical_memory_range[i].size),
			(uint32_t)gKernelArgs.physical_memory_range[i].size);
	}

	dprintf("allocated phys memory ranges:\n");
	for (uint32_t i = 0; i < gKernelArgs.num_physical_allocated_ranges; i++) {
		dprintf("    0x%08x-0x%08x, length 0x%08x\n",
			(uint32_t)gKernelArgs.physical_allocated_range[i].start,
			(uint32_t)(gKernelArgs.physical_allocated_range[i].start + gKernelArgs.physical_allocated_range[i].size),
			(uint32_t)gKernelArgs.physical_allocated_range[i].size);
	}

	dprintf("allocated virt memory ranges:\n");
	for (uint32_t i = 0; i < gKernelArgs.num_virtual_allocated_ranges; i++) {
		dprintf("    0x%08x-0x%08x, length 0x%08x\n",
			(uint32_t)gKernelArgs.virtual_allocated_range[i].start,
			(uint32_t)(gKernelArgs.virtual_allocated_range[i].start + gKernelArgs.virtual_allocated_range[i].size),
			(uint32_t)gKernelArgs.virtual_allocated_range[i].size);
	}
}
...
{
	if (platform_allocate_region((void **)&sPageDirectory, ARM_MMU_L1_TABLE_SIZE + ALIGN_PAGEDIR + MAX_PAGE_TABLES*ARM_MMU_L2_COARSE_TABLE_SIZE, 0, false) != B_OK)
	...
	sLastPageTable = (uint32*)((uint32)sFirstPageTable + MAX_PAGE_TABLES*ARM_MMU_L2_COARSE_TABLE_SIZE);

	memset(sFirstPageTable, 0, MAX_PAGE_TABLES*ARM_MMU_L2_COARSE_TABLE_SIZE);
	...
	uint32_t descriptor_version)
{
	...
	for (size_t i = 0; i < memory_map_size / descriptor_size; ++i) {
		...
		case EfiLoaderCode:
		case EfiLoaderData:
			map_range(entry->VirtualStart, entry->PhysicalStart, entry->NumberOfPages * B_PAGE_SIZE,
				ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | 0x30 | 0x0400 | 0x400);
			break;
		default:
			;
		}
	...
	while (mmu_next_region(&cookie, &vaddr, &paddr, &size)) {
		map_range(vaddr, paddr, size, ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | 0x30 | 0x0400 | 0x400);
	}
	...
	map_range(page_table_area, page_table_area, MAX_PAGE_TABLES*ARM_MMU_L2_COARSE_TABLE_SIZE, ARM_MMU_L2_FLAG_B | ARM_MMU_L2_FLAG_C | 0x30/*ap*/ | 0x040 /*tex*/ | 0x400 /*S*/);
