...
allocate_trampoline_page(void)
{
	...
	if (platform_allocate_lomem(&trampoline_page, B_PAGE_SIZE) == B_OK)
		return trampoline_page;

	trampoline_page = (void *)get_next_virtual_address(B_PAGE_SIZE);
	if (platform_allocate_region(&trampoline_page, B_PAGE_SIZE, 0, true) == B_OK)
		return trampoline_page;

	trampoline_page = NULL;
	if (platform_allocate_region(&trampoline_page, B_PAGE_SIZE, 0, false) != B_OK)
		return NULL;

	if (platform_free_region(trampoline_page, B_PAGE_SIZE) != B_OK)
		return NULL;

	if (platform_allocate_region(&trampoline_page, B_PAGE_SIZE, 0, true) != B_OK)
		return NULL;

	ASSERT_ALWAYS((uint32_t)trampoline_page >= 0x88000000);
	return trampoline_page;
}
...
arch_start_kernel(addr_t kernelEntry)
{
	...
	void *trampoline_page = allocate_trampoline_page();
	if (trampoline_page == NULL)
		panic("Failed to allocate trampoline page.");

	memcpy(trampoline_page, (void *)arch_enter_kernel, B_PAGE_SIZE);
	arch_enter_kernel_t enter_kernel = (arch_enter_kernel_t)trampoline_page;
