...
arch_smp_boot_other_cpus(uint64 pagedir, uint64 kernelEntry, addr_t virtKernelArgs)
{
	...
	for (uint32 i = 1; i < gKernelArgs.num_cpus; i++) {
		...
		apic_set_interrupt_command(gKernelArgs.arch_args.cpu_apic_id[i], APIC_TRIGGER_MODE_LEVEL | APIC_INTR_COMMAND_1_ASSERT
		...
		/* deassert INIT */
		apic_set_interrupt_command(gKernelArgs.arch_args.cpu_apic_id[i], APIC_TRIGGER_MODE_LEVEL | APIC_DELIVERY_MODE_INIT);
