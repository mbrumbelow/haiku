...
// From entry.S
extern "C" void arch_enter_kernel(uint32_t cr3, struct kernel_args *kernelArgs,
...
arch_start_kernel(addr_t kernelEntry)
{
	...
	// Enter the kernel!
	dprintf("arch_enter_kernel(cr3: 0x%08x, kernelArgs: 0x%08x, "
		"kernelEntry: 0x%08x, sp: 0x%08x, gBootGDTDescriptor: 0x%08x)\n",
		final_cr3, (uint32_t)&gKernelArgs, (uint32_t)kernelEntry,
		(uint32_t)(gKernelArgs.cpu_kstack[0].start + gKernelArgs.cpu_kstack[0].size),
		(uint32_t)&gBootGDTDescriptor);

	arch_enter_kernel(final_cr3, &gKernelArgs, kernelEntry,
