...
{
	uint32_t *page_table = (uint32_t *)mmu_allocate_page();
	memset(page_table, 0, B_PAGE_SIZE);
	return page_table;
}
...
arch_mmu_init_gdt(void)
{
	...
	TRACE("gdt phys 0x%08x virt 0x%08x desc 0x%08x\n",
		(uint32_t)gBootGDT, (uint32_t)virtualGDT,
...
map_page(addr_t virtAddr, phys_addr_t physAddr, uint32_t flags)
{
	...
	uint32 pageTableIndex = (virtAddr & 0x003ff000) >> 12;
...
{
	//TRACE("map 0x%08x --> 0x%08x, len=0x%08x, flags=0x%08x\n",
	//	(uint32_t)virtAddr, (uint32_t)physAddr, (uint32_t)size, flags);
	...
	uint32_t descriptorVersion)
{
	...
	map_page((addr_t)arch_enter_kernel, (phys_addr_t)arch_enter_kernel, kDefaultPageFlags);

	// identity mapping for kernel args
	map_range((addr_t)&gKernelArgs, (phys_addr_t)&gKernelArgs, B_PAGE_SIZE*2, kDefaultPageFlags);

	// identity mapping for the boot GDT descriptor
	map_range((addr_t)&gBootGDTDescriptor, (phys_addr_t)&gBootGDTDescriptor, B_PAGE_SIZE, kDefaultPageFlags);
	...
	addr_t virPageDirectory;
	platform_bootloader_address_to_kernel_address((void*)sPageDirectory, &virPageDirectory);

	gKernelArgs.arch_args.phys_pgdir = (uint32_t)sPageDirectory;
	gKernelArgs.arch_args.vir_pgdir = (uint32_t)virPageDirectory;
	...
	TRACE("virtual_end = 0x%08x\n", (uint32_t)gKernelArgs.arch_args.virtual_end);
