...
arch_mmu_init_gdt(void)
{
	...
	addr_t vir_gdt;
	platform_bootloader_address_to_kernel_address(gBootGDT, &vir_gdt);

	gBootGDTDescriptor.limit = BOOT_GDT_SEGMENT_COUNT * sizeof(segment_descriptor);
	gBootGDTDescriptor.base = (uint32_t)vir_gdt;

	TRACE("gdt phys 0x%08x virt 0x%08x desc 0x%08x\n",
		(uint32_t)gBootGDT, (uint32_t)vir_gdt,
...
map_page(addr_t virt_addr, phys_addr_t phys_addr, uint32_t flags)
{
	...
	uint32_t pageDirectoryIndex = virt_addr >> 22;
	...
	uint32 pageTableIndex = (virt_addr & 0x003ff000) >> 12;
	pageTable[pageTableIndex] = phys_addr | flags;
}
	...
	uint32_t descriptor_version)
{
	...
	gKernelArgs.arch_args.vir_pgdir = (uint32_t)vir_pgdir;
