...
extern "C" void arch_enter_kernel(uint64 stap, struct kernel_args *kernelArgs,
...
extern uint64_t arch_mmu_generate_post_efi_page_tables(size_t memory_map_size,
	efi_memory_descriptor *memory_map, size_t descriptor_size,
	uint32_t descriptor_version);
extern uint64_t arch_mmu_generate_satp(uint64_t sv39);
...
arch_start_kernel(addr_t kernelEntry)
{
	...
	uint64_t final_sv39 = arch_mmu_generate_post_efi_page_tables(
		memory_map_size, memory_map, descriptor_size, descriptor_version);
	dprintf("Final SV39 at %#lx\n", final_sv39);
	uint64_t satp = arch_mmu_generate_satp(final_sv39);
