...
#define RESERVED_MEMORY_SIZE	0x00020000
...
static void
fix_memory_map_for_m_mode()
{
	uint64 initialPhysicalMemory = total_physical_memory();

	// remove M-mode resident code from physical memory list
	remove_physical_memory_range(RESERVED_MEMORY_BASE, RESERVED_MEMORY_SIZE);

	gKernelArgs.ignored_physical_memory += initialPhysicalMemory - total_physical_memory();

	// sort physical memory ranges after the fixup
