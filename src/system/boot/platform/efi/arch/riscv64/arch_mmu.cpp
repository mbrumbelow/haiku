...
#define RV64_VA_BASE			0xFFFFFFC000000000
...
static void
arch_mmu_dumpmap(uint64_t va_base, uint64_t* pagetable, int level)
{
	if (level == 2)
	...
	for(int k = 0; k < 512; k++) {
		...
		uint64_t va = va_base | k << ((level * 9) + 12);
		if ((pte & RISCV_PAGE_VALID) != 0) {
			dprintf("  %s%s%03d: va 0x%016lx pte 0x%016lx pa 0x%016lx (",
				level < 2 ? ".." : "",
				level < 1 ? ".." : "",
				k, va, pte, SV39_PTE_TO_ADDR(pte));
			...
			if (level > 0 && SV39_PTE_TO_FLAGS(pte) & kLeafPageFlags) {
				...
				arch_mmu_dumpmap(va, pt, next_level);
			}
		}
	}
}
	...
	uint32_t descriptor_version)
{
	...
	arch_mmu_dumpmap(RV64_VA_BASE, pageDirL2, 2);
