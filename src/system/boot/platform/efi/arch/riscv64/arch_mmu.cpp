...
#define RV64_PTE_TO_PA(ppn)	(uint64_t*)((uint64_t)ppn >> 10)
#define RV64_PA_TO_PPN(pa)	((((uint64_t)pa) >> 12) << 10)
...
void
arch_mmu_dumpmap(uint64_t *pageDirectory)
{
	dprintf("Generated Page Directory: %p\n", pageDirectory);
	for (size_t lTwo = 0; lTwo < 512; lTwo++) {
		if ((pageDirectory[lTwo] & RISCV_PAGE_VALID) == 0)
			continue;
		uint64_t *nextPageDirectory = RV64_PTE_TO_PA(pageDirectory[lTwo]);
		dprintf("  %03" B_PRIdSSIZE ": raw: 0x%" B_PRIx64
			", pd[1]: 0x%p\n", lTwo, pageDirectory[lTwo],
			nextPageDirectory);
		for (size_t lOne = 0; lOne < 512; lOne++) {
			if ((nextPageDirectory[lOne] & RISCV_PAGE_VALID) == 0)
				continue;
			uint64_t *doubleNextPageDirectory =
				RV64_PTE_TO_PA(nextPageDirectory[lOne]);
			dprintf("    %03" B_PRIdSSIZE ": raw: 0x%" B_PRIx64
				", pd[0]: 0x%p\n", lOne,
				nextPageDirectory[lOne],
				doubleNextPageDirectory);
		}
	}
}
	...
	uint32_t descriptor_version)
{
	...
	uint64_t *pageDirectoryL0;

	// Allocate the top level PTE (sv39 assumed)
	pageDirectoryL2 = NULL;
	if (platform_allocate_region((void**)&pageDirectoryL2, B_PAGE_SIZE, 0, false) != B_OK)
		panic("Failed to allocate page directory root.");
	gKernelArgs.arch_args.phys_pgdir = (uint64_t)(addr_t)pageDirectoryL2;
	memset(pageDirectoryL2, 0, B_PAGE_SIZE);
	platform_bootloader_address_to_kernel_address(pageDirectoryL2,
	...
	// Currently only use 1 pageDirectoryL2 (512GB). This will need to change if
	...
	pageDirectoryL1 = (uint64*)mmu_allocate_page();
	memset(pageDirectoryL1, 0, B_PAGE_SIZE);
	pageDirectoryL2[510] = (addr_t)pageDirectoryL1 | kTableMappingFlags;
	pageDirectoryL2[0] = (addr_t)pageDirectoryL1 | kTableMappingFlags;

	for (uint64 i = 0; i < maxAddress; i += 0x40000000) {
		pageDirectoryL0 = (uint64*)mmu_allocate_page();
		memset(pageDirectoryL0, 0, B_PAGE_SIZE);
		pageDirectoryL1[i / 0x40000000] = (addr_t)pageDirectoryL0 | kTableMappingFlags;
	...
	pageDirectoryL1 = (uint64*)mmu_allocate_page();
	memset(pageDirectoryL1, 0, B_PAGE_SIZE);
	pageDirectoryL2[511] = (addr_t)pageDirectoryL1 | kTableMappingFlags;

	pageDirectoryL0 = (uint64*)mmu_allocate_page();
	memset(pageDirectoryL0, 0, B_PAGE_SIZE);
	pageDirectoryL2[510] = (addr_t)pageDirectoryL0 | kTableMappingFlags;
	...
	*/
	...
	arch_mmu_dumpmap(pageDirectoryL2);

	return (uint64)pageDirectoryL2;
}
