...
const struct supported_uarts {
	...
	{ "ns16550a", &get_uart<DebugUART8250> },
	{ "sifive,uart0", &get_uart<ArchUARTSifive> },
	{ "arm,pl011", &get_uart<ArchUARTPL011> },
};
	...
	if (sUart.kind == NULL) {
		for (uint32 i = 0; i < KNOWN_UART_COUNT; i++) {
			if (HasFdtString(compatible, compatibleLen,
				...
				strncpy(sUart.kind, kSupportedUarts[i].dtb_compat,
					sizeof(sUart.kind));

				GetReg(fdt, node, addressCells, sizeCells, 0, sUart.regs);
				sUart.irq = GetInterrupt(fdt, node, interruptCells);
				const void* prop = fdt_getprop(fdt, node,
					"clock-frequency", NULL);

				sUart.clock = (prop == NULL)
					? 0 : fdt32_to_cpu(*(uint32*)prop);
