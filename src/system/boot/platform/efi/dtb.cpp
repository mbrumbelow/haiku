...
const struct supported_uarts {
	...
	{ "ns16550a", UART_DRIVER_8250, &get_uart<DebugUART8250> },
	{ "ns16550", UART_DRIVER_8250, &get_uart<DebugUART8250> },
	{ "sifive,uart0", UART_DRIVER_SIFIVE, &get_uart<ArchUARTSifive> },
	{ "arm,pl011", UART_DRIVER_PL011, &get_uart<ArchUARTPL011> },
	{ "arm,primecell", UART_DRIVER_PL011, &get_uart<ArchUARTPL011> },
};

#define KNOWN_UART_COUNT sizeof(kSupportedUarts) / sizeof(kSupportedUarts[0])
	...
	uint32 interruptCells /* from parent node */)
{
	...
	for (uint32 i = 0; i < KNOWN_UART_COUNT; i++) {
		if (HasFdtString(compatible, compatibleLen,
				kSupportedUarts[i].dtb_compat)) {

			strncpy(sUart.kind, kSupportedUarts[i].name,
				sizeof(sUart.kind));
			...
			gUART = kSupportedUarts[i].uart_driver_init(sUart.regs.start,
				sUart.clock);
		}
	}
