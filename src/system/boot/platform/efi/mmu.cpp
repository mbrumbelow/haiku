...
static addr_t sNextExecuteVirtualAddress = KERNEL_LOAD_BASE;
static const addr_t kInitialNextVirtualAddress = KERNEL_LOAD_BASE + 32 * 1024 * 1024;
static addr_t sNextVirtualAddress = kInitialNextVirtualAddress;
...
get_next_virtual_address(size_t size, bool execute)
{
	...
	if (execute && (sNextExecuteVirtualAddress + size) <= kInitialNextVirtualAddress) {
		addr_t address = sNextExecuteVirtualAddress;
		sNextExecuteVirtualAddress += ROUNDUP(size, B_PAGE_SIZE);
		return address;
	}
