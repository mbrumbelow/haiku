...
serial_putc(char ch)
{
	...
	// First we use EFI serial output if available
	if (sSerial != NULL) {
		size_t bufSize = 1;
	...
	// Then we try any EFI services we may (still) have access to.
	if (gUART != NULL) {
		gUART->PutChar(ch);
		return;
	}
...
serial_init(void)
{
	// If we already have a UART, use it instead of EFI Serial services
	if (gUART != NULL)
		return;
...
serial_uninit(void)
{
	...
	// On x86, we wire up a legacy i/o port UART
#if defined(__i386__) || defined(__x86_64__)
	if (gUART == NULL)
	...
	// Init the uart the kernel will use
	if (gUART != NULL) {
		gUART->InitEarly();
