...
serial_putc(char ch)
{
	...
	// If EFI serial_io services are unavailable... try any UART
	if (gUART != NULL) {
		gUART->PutChar(ch);
		return;
	}

	// If we don't have EFI serial_io, *and* No UART, fallback
	// to stdout as a last resort if EFI is available.
	if (sEFIAvailable) {
...
serial_init(void)
{
	// If we somehow already have a UART, use it instead of EFI Serial services
	if (gUART != NULL)
		return;
	...
	// On x86, we wire up a legacy i/o port UART as a last resort if EFI serial_io is unavailable
#if defined(__i386__) || defined(__x86_64__)
	if (gUART == NULL) {
		gUART = arch_get_uart_8250(0x3f8, 115200);
		gUART->InitEarly();
...
{
	// Disconnecting from EFI serial_io services is important as we leave the bootloader
	sEFISerialIO = NULL;
	sEFIAvailable = false;
