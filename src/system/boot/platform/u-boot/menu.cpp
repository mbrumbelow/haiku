...
platform_add_menus(Menu *menu)
{
	...
	/*switch (menu->Type()) {
		case MAIN_MENU:
			menu->AddItem(item = new(std::nothrow) MenuItem("Select fail-safe video mode", video_mode_menu()));
			item->SetTarget(video_mode_hook);
			break;
		case SAFE_MODE_MENU:
			menu->AddItem(item = new(std::nothrow) MenuItem("Use fail-safe video mode"));
