...
user_mutex_init()
{
	...
	add_debugger_command_etc("umtx", &dump_user_mutex,
	...
	generic_addr_t address, bool noInsert = false, bool alreadyLocked = false)
{
	...
	entry = new UserMutexEntry;
	entry->address = address;
	entry->ref_count = 1;
	mutex_init(&entry->lock, "UserMutexEntry lock");
...
{
	ASSERT_LOCKED_MUTEX(&entry->lock);
...
user_mutex_lock_locked(UserMutexEntry* entry, int32* mutex,
	uint32 flags, bigtime_t timeout, MutexLocker& locker, bool isWired)
{
...
static void
user_mutex_unblock_locked(UserMutexEntry* entry, int32* mutex, uint32 flags,
	bool isWired)
{
	if (entry->condition.EntriesCount() == 0) {
...
user_mutex_sem_release(UserMutexEntry* entry, int32* sem, bool isWired)
{
	...
	MutexLocker entryLocker(entry->lock);
	entry->condition.NotifyOne(B_OK);
	if (entry->condition.EntriesCount() == 0) {
	...
	int32* toMutex, const char* name, uint32 toFlags, bigtime_t timeout)
{
	...
	{
		ConditionVariableEntry waiter;
		MutexLocker entryLocker;

		entryLocker.SetTo(toEntry->lock, false);
		const bool alreadyLocked = user_mutex_prepare_to_lock(toEntry, toMutex,
			toFetcher.IsWired());
		if (!alreadyLocked)
		...
		if (fromEntry != NULL) {
			entryLocker.SetTo(fromEntry->lock, false);
			user_mutex_unblock_locked(fromEntry, fromMutex, fromFlags,
				fromFetcher.IsWired());
			entryLocker.Unlock();
		}
...
_user_mutex_unblock(int32* mutex, uint32 flags)
{
	...
	if (entry == NULL) {
		...
		tableReadLocker.Unlock();
		MutexLocker entryLocker(entry->lock);
		user_mutex_unblock_locked(entry, mutex, flags, contextFetcher.IsWired());
	}
