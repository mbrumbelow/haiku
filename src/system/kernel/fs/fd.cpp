...
status_t
select_fd(int32 fd, struct selectsync* info, bool kernel)
{
	...
	status_t status = descriptor->ops->fd_select(descriptor, &events, info);
	...
	if (context->fds[fd] != descriptor) {
		// Someone close()d this FD in the meantime. Nothing more to do.
