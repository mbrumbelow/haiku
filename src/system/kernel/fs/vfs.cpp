...
*/
static status_t
release_advisory_lock(struct vnode* vnode, struct file_descriptor* descriptor,
	struct flock* flock)
{
	...
	while (iterator.HasNext()) {
		...
		if (lock->session == session && lock->descriptor == descriptor)
			removeLock = true;
		else if (lock->team == team && lock->descriptor == descriptor
			...
			if (!startsBefore && !endsBeyond) {
				...
				secondLock->descriptor = lock->descriptor;
...
*/
static status_t
acquire_advisory_lock(struct vnode* vnode, struct file_descriptor* descriptor,
	struct flock* flock, bool wait)
{
	...
	while (true) {
		...
		while (iterator.HasNext()) {
			...
			if ((lock->team != team || lock->descriptor != descriptor)
					&& advisory_lock_intersects(lock, flock)) {
				// locks do overlap
...
file_close(struct file_descriptor* descriptor)
{
	...
	if (status == B_OK) {
		...
		if (HAS_FS_CALL(vnode, release_lock))
			...
			// Report as many errors as possible: failure to release flock locks
			// shadows failure to release fcntl ones
			if (status == B_OK)
				status = status2;
		}
	}
