...
{
	size_t bytes = (areaSize / B_PAGE_SIZE + 1) / 2;
	...
	addr_t size, VMArea** _secondArea, bool kernel)
{
	...
	if (area->Base() == address) {
		...
		if (area->page_protections != NULL) {
			...
			// bits.
			uint8* newProtections = realloc_page_protections(NULL,
				area->Size(), allocationFlags);
			...
			bitmap_shift<uint8>(area->page_protections, oldBytes * 8,
				-(pagesShifted * 4));
	...
	if (area->page_protections != NULL) {
		...
		bitmap_shift<uint8>(areaOldProtections, oldBytes * 8,
	...
	team_id sourceTeam, void* _sourceAddress)
{
	...
	// Cut the source areas if needed.
	addr_t sourceEndAddress = sourceAddress + size;
	VMArea* sourceLastArea = sourceAddressSpace->LookupArea(sourceEndAddress - 1);
	if (sourceLastArea->Base() + sourceLastArea->Size() > sourceEndAddress) {
		status = cut_area(sourceAddressSpace, sourceLastArea, sourceEndAddress,
			0, NULL, false);
		if (status != B_OK)
			return status;
	}
	VMArea* sourceFirstArea = sourceAddressSpace->LookupArea(sourceAddress);
	if (sourceFirstArea->Base() < sourceAddress) {
		status = cut_area(sourceAddressSpace, sourceFirstArea, sourceAddress,
			0, NULL, false);
		if (status != B_OK)
			return status;
	}

	// Cut areas in the target team if needed.
	...
	if (status == B_OK) {
		...
		for (size_t i = 0; i < sourceAreaCount; i++) {
			...
			status = map_backing_store(targetAddressSpace, area->cache,
				area->cache_offset, area->name, area->Size(),
			...
			currentTargetAddress += area->Size();
