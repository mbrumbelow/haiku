...
_user_mlock(const void* address, size_t size) {
	...
	while (pointer_locked_pages) {
		if (pointer_locked_pages->start > address && pointer_locked_pages->end < end_address) {
			...
			if (error != B_OK)
				goto end_mlock;
			error = lock_memory(pointer_locked_pages->end, end_address - (addr_t*)pointer_locked_pages->end);
			if (error != B_OK)
				goto end_mlock;
...
_user_munlock(const void* address, size_t size) {
	...
	addr_t* end_address = (addr_t*)address + size;
	...
	while (pointer_locked_pages) {
		...
		if (pointer_locked_pages->start > address && pointer_locked_pages->end == end_address) {
			error = unlock_memory(pointer_locked_pages->start, end_address - (addr_t*)pointer_locked_pages->start);
			if (error == B_OK)
				locked_pages->Remove(pointer_locked_pages);
			else goto end_munlock;
		}
