...
_user_mlock(const void* address, size_t size) {
	...
	int error = B_OK;
	BKernel::LockedPages* new_locked_pages_ele = new BKernel::LockedPages();
	if (!new_locked_pages_ele)
		return ENOMEM;

	BKernel::LockedPagesList* locked_pages = &team->locked_pages_list;
	if (!locked_pages)
		locked_pages = new BKernel::LockedPagesList();

	void* end_address = (addr_t*)address + size;

	BKernel::LockedPages* pointer_locked_pages = locked_pages->Head();
	while (pointer_locked_pages) {
		if (pointer_locked_pages->start <= end_address && pointer_locked_pages->end >= address) { // overlapping
			if (pointer_locked_pages->start > address && pointer_locked_pages->end < end_address) {
				...
				address = pointer_locked_pages->end;
				size = (addr_t)end_address - (addr_t)pointer_locked_pages->end;
				break;
			}
			if (pointer_locked_pages->start == address && pointer_locked_pages->end == end_address) {
				teamLocker.Unlock();
	...
	if (error == B_OK) {
		...
		BKernel::LockedPages* last_pointer_locked_pages = locked_pages->Head();
