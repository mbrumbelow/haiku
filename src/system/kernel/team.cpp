...
/*!	Kill all other threads but the one specified.
	If it was not the main thread, it gets promoted to become the main thread.
*/
static void 
kill_other_threads(Team* team, TeamLocker teamLocker, Thread* survivor)
{
	...
	while (true) {
		...
		while (thread != NULL) {
			if (thread != survivor && thread->id != nubThreadID) {
	...
	int32 argCount, int32 envCount, mode_t umask)
{
	...
	kill_other_threads(team, teamLocker, currentThread);
...
team_shutdown_team(Team* team)
{
	...
	kill_other_threads(team, teamLocker, team->main_thread);
