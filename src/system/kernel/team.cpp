...
static status_t
create_team_user_data(Team* team, void* exactAddress = NULL, void* baseAddress = NULL)
{
	void* address;
	uint32 addressSpec;
	...
	if (exactAddress != NULL) {
		...
		address = baseAddress != NULL ? baseAddress : (void*)KERNEL_USER_DATA_BASE;
...
team_create_thread_start_internal(void* args)
{
	...
	if ((teamArgs->flags & TEAM_ARGS_FLAG_COMPAT_FLATARGS) == 0
		...
		*elfArgs = NULL;
		...
		if (user_strlcpy(programArgs->program_path, path,
			...
			|| user_memcpy(&programArgs->args, &user32Args, sizeof(user32Args)) < B_OK
			|| user_memcpy((void*)(((addr_t)&programArgs->args) + 4), &user32Env, sizeof(user32Env)) < B_OK
			...
			|| user_memcpy(userArgs, teamArgs->flat_args,
					teamArgs->flat_args_size - (stringsOffset - strings32Offset)) < B_OK) {
		...
		if (user_strlcpy(programArgs->program_path, path,
				sizeof(programArgs->program_path)) < B_OK
			|| user_memcpy(&programArgs->arg_count, &argCount, sizeof(int32)) < B_OK
			|| user_memcpy(&programArgs->args, &user32Args, sizeof(uint32)) < B_OK
			|| user_memcpy(&programArgs->env_count, &envCount, sizeof(int32)) < B_OK
			|| user_memcpy(&programArgs->env, &user32Env, sizeof(uint32)) < B_OK
			|| user_memcpy(&programArgs->error_port, &teamArgs->error_port,
					sizeof(port_id)) < B_OK
			|| user_memcpy(&programArgs->error_token, &teamArgs->error_token,
					sizeof(uint32)) < B_OK
	...
	if ((thread->flags & THREAD_FLAGS_COMPAT_MODE) != 0) {
		...
		commPageArea = clone_commpage_compat_area(team->id, &team->commpage_address);
	} else
#endif
	{
