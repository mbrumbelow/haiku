...
void
VMSAv8TranslationMap::ProcessRange(phys_addr_t ptPa, addr_t va, size_t size,
	vm_page_reservation* reservation, UpdatePte &&updatePte)
{
	...
	for (uint64_t effectiveVa = alignedDownVa; effectiveVa < alignedUpEnd; effectiveVa += entrySize) {
		...
		if constexpr (kLevel == 3) {
			...
			ProcessRange<kLevel+1>(subTable, subVa, subSize, reservation, updatePte);
	...
	vm_page_reservation* reservation)
{
	...
	ProcessRange(fPageTable, va & vaMask, B_PAGE_SIZE, reservation,
...
VMSAv8TranslationMap::Unmap(addr_t start, addr_t end)
{
	...
	ProcessRange(fPageTable, start & vaMask, size, nullptr,
		[=](uint64_t *ptePtr, uint64_t effectiveVa) {
