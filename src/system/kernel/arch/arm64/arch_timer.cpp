...
system_time(void)
{
	uint64 ctl = READ_SPECIALREG(CNTV_CTL_EL0);

	if (ctl & TIMER_ENABLE) {
		return READ_SPECIALREG(CNTVCT_EL0) / sTimerTicksUS;
	} else {
		// The timer continues to tick while it is "disabled",
		// but we have to temporarily enable it in order to
		// read the value.
		WRITE_SPECIALREG(CNTV_CTL_EL0, ctl | TIMER_ENABLE);
		uint64 r = READ_SPECIALREG(CNTV_TVAL_EL0);
		WRITE_SPECIALREG(CNTV_CTL_EL0, ctl);
		return r;
	}
}
