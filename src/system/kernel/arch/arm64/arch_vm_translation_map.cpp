...
static void arch_vm_alloc_empty_table(void) {
	...
	sEmptyTable = page->physical_page_number << 12;
}
...
arch_vm_translation_map_init(kernel_args* args, VMPhysicalPageMapper** _physicalPageMapper)
{
	...
	// Create an empty page table for use in break-before-make changes.
	...
	if (ID_AA64MMFR2_CNP(mmfr2) == ID_AA64MMFR2_CNP_IMPL) {
		VMSAv8TranslationMap::fHwFeature |= VMSAv8TranslationMap::HW_CNP;
	}
