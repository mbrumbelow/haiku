...
public:
								RISCV64VMTranslationMap(bool kernel, phys_addr_t pageTable = 0);
	...
	inline	uint64				Satp();
			...
			status_t			MemcpyToMap(addr_t to, const char *from, size_t size);
			status_t			MemcpyFromMap(char *to, addr_t from, size_t size);
			status_t			MemsetToMap(addr_t to, char c, size_t count);
			ssize_t				StrlcpyFromMap(char *to, addr_t from, size_t size);
			ssize_t				StrlcpyToMap(addr_t to, const char *from, size_t size);
	
private:
			Pte*				LookupPte(addr_t virtAdr, bool alloc, vm_page_reservation* reservation);
