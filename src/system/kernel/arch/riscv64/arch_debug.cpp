...
static void WriteImage(preloaded_image* _image)
{
...
static void WriteImages()
{
	...
	for (preloaded_image* image = sKernelArgs->preloaded_images; image != NULL; image = image->next)
		WriteImage(image);
}

static bool AddressInImage(preloaded_image* _image, addr_t adr)
{
	preloaded_elf64_image* image = (preloaded_elf64_image*)_image;
	if (
...
static bool SymbolAt(preloaded_image* _image, addr_t adr, const char **name, ssize_t *ofs)
{
	preloaded_elf64_image* image = (preloaded_elf64_image*)_image;
	adr -= image->text_region.delta;
	for (uint32 i = 0; i < image->num_debug_symbols; i++) {
		Elf64_Sym& sym = image->debug_symbols[i];
		if (sym.st_shndx != STN_UNDEF && adr >= sym.st_value && adr < sym.st_value + sym.st_size) {
			if (name != NULL)
...
static preloaded_image* FindImage(addr_t adr)
{
	if (AddressInImage(sKernelArgs->kernel_image, adr)) return sKernelArgs->kernel_image;
	for (preloaded_image* image = sKernelArgs->preloaded_images; image != NULL; image = image->next)
		if (AddressInImage(image, adr)) return image;
	return NULL;
}
...
void WritePCBoot(addr_t pc)
{
	...
	if (image != NULL) {
		...
		dprintf("<%s> 0x%" B_PRIxADDR, (char*)image->name, pc - ((preloaded_elf64_image*)image)->text_region.delta);
		return;
	}
/*
...
void WritePC(addr_t pc)
{
	...
	bool exactMatch;
	if (lookup_symbol(thread_get_current_thread(), pc, &baseAddress, &symbolName, &imageName, &exactMatch) >= B_OK) {
		if (symbolName != NULL) {
			dprintf("<%s> %s + %" B_PRIdSSIZE, imageName, symbolName, pc - baseAddress);
			return;
		}
...
static int stack_trace(int argc, char **argv)
{
	DoStackTrace(Fp(), 0);
	return 0;
}
