	...
	void (*function)(void*), const void* data)
{
	...
	VMTranslationMap* map = (thread->team == team_get_kernel_team()) ? VMAddressSpace::Kernel()->TranslationMap() : thread->team->address_space->TranslationMap();
...
arch_thread_enter_userspace(Thread *thread, addr_t entry, void *arg1, void *arg2)
{
	...
	if (arch_setjmp(&thread->arch_info.context) == 0) {
		...
		SetSepc(entry);
		RestoreUserRegs();
		arch_enter_userspace(arg1, arg2, thread->user_stack_base + thread->user_stack_size);
	} else {
...
static uint8*
get_signal_stack(Thread* thread, struct iframe* frame, struct sigaction* action, size_t spaceNeeded)
{
	...
	struct signal_frame_data *signalFrameData)
{
	...
	uint8* userStack = get_signal_stack(thread, frame, sa, sizeof(*signalFrameData));
	dprintf("  user stack: 0x%" B_PRIxADDR "\n", (addr_t)userStack);
	status_t res = user_memcpy(userStack, signalFrameData, sizeof(*signalFrameData));
	if (res < B_OK)
		return res;
	...
	ASSERT(user_memcpy(&signalHandlerAddr, &((addr_t*)commpageAdr)[COMMPAGE_ENTRY_RISCV64_SIGNAL_HANDLER], sizeof(signalHandlerAddr)) >= B_OK);
...
arch_store_fork_frame(struct arch_fork_arg *arg)
{
	...
	memcpy(&arg->frame, thread_get_current_thread()->arch_info.userFrame, sizeof(iframe));
	arg->frame.a0 = 0; // fork return
}
