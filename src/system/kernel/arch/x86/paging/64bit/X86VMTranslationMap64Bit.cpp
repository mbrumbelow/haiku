	...
	ReverseMappingInfoCallback& callback)
{
	...
	for (uint32 i = 0; i < (fIsKernelMap ? 512 : 256); i++) {
		...
		for (uint32 j = 0; j < 512; j++) {
			...
			if ((virtualPDPT[j] & X86_64_PDE_LARGE_PAGE) != 0) {
			...
			for (uint32 k = 0; k < 512; k++) {
				...
				for (uint32 l = 0; l < 512; l++) {
					...
					if ((virtualPageTable[l] & X86_64_PTE_ADDRESS_MASK) == physicalAddress) {
						addr_t virtualAddress = i * k64BitPML4TRange
							+ j * k64BitPDPTRange
							+ k * k64BitPageDirectoryRange
							+ l * k64BitPageTableRange;
	...
	if (fIsKernelMap) {
		// Also report the instance of this page in the physical map area.
		addr_t virtualAddress = KERNEL_PMAP_BASE + physicalAddress;
		if (callback.HandleVirtualAddress(virtualAddress))
			return true;
	}
