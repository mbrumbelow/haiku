...
KernelDaemon::_DaemonThread()
{
	...
	while (true) {
		...
		while (struct daemon* daemon = _NextDaemon(marker)) {
			...
			bigtime_t run = (daemon->last + (daemon->frequency * iterationToUsecs)) - time;
			if (run <= 0) {
				daemon->last = time;
				run = daemon->frequency * iterationToUsecs;
		...
		acquire_sem_etc(fDaemonAddedSem, 1, B_RELATIVE_TIMEOUT, next);
	}
