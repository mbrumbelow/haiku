...
class BlockPrefetch {
public:
								BlockPrefetch(block_cache* cache, off_t fBlockNumber,
									size_t numBlocks);
								~BlockPrefetch();
...
#ifndef BUILDING_USERLAND_FS_SERVER
//	#pragma mark - BlockPrefetch


BlockPrefetch::BlockPrefetch(block_cache* cache, off_t blockNumber, size_t numBlocks)
	:
	fCache(cache),
	fBlockNumber(blockNumber),
...
*/
status_t
BlockPrefetch::Allocate()
{
	TRACE(("BlockPrefetch::Allocate: looking up %" B_PRIuSIZE " blocks, starting with %" B_PRIdOFF
	...
	for (size_t i = 0; i < fNumRequested; ++i) {
		...
		if (blockNumIter < 0 || blockNumIter >= fCache->max_blocks) {
			panic("BlockPrefetch::Allocate: invalid block number %" B_PRIdOFF " (max %" B_PRIdOFF
				")", blockNumIter, fCache->max_blocks - 1);
			return B_BAD_VALUE;
		}
		cached_block* block = fCache->hash->Lookup(blockNumIter);
		if (block != NULL) {
			// truncate the request
			TRACE(("BlockPrefetch::Allocate: found an existing block (%" B_PRIdOFF ")\n",
				blockNumIter));
			fBlocks[i] = NULL;
			finalNumBlocks = i;
			break;
...
BlockPrefetch::Read()
{
	TRACE(("BlockPrefetch::Read: reading %" B_PRIuSIZE " blocks\n", fNumAllocated));
	...
	if (status != B_OK) {
		...
		TRACE_ALWAYS("BlockPrefetch::Read: failed to initialize IO request for %" B_PRIuSIZE
...
/*static*/ status_t
BlockPrefetch::IterativeIOGetVecsHook(void* cookie, io_request* request, off_t offset, size_t size,
	struct file_io_vec* vecs, size_t* _count)
{
	TRACE(("BlockPrefetch::IterativeIOGetVecsHook: setting offset %" B_PRIdOFF " and length %"
		B_PRIuSIZE "\n", offset, size));
...
/*static*/ status_t
BlockPrefetch::IterativeIOFinishedHook(void* cookie, io_request* request, status_t status,
	bool partialTransfer, size_t bytesTransferred)
{
	TRACE(("BlockPrefetch::IterativeIOFinishedHook: status %s, partial %d\n", strerror(status),
		partialTransfer));

	BlockPrefetch* blockPrefetch = reinterpret_cast<BlockPrefetch*>(cookie);
	block_cache* cache = blockPrefetch->fCache;
	cached_block** blocks = blockPrefetch->fBlocks;
	size_t blockSize = cache->block_size;
	off_t blockNumber = blockPrefetch->fBlockNumber;
	size_t numBlocks = blockPrefetch->fNumAllocated;

	MutexLocker locker(&cache->lock);

	if (bytesTransferred < numBlocks * blockSize) {
		blockPrefetch->_RemoveAllocated(numBlocks, numBlocks);
...
*/
void
BlockPrefetch::_RemoveAllocated(size_t unbusyCount, size_t removeCount)
{
	TRACE(("BlockPrefetch::_RemoveAllocated:  unbusy %" B_PRIuSIZE " and remove %" B_PRIuSIZE
...
block_cache_prefetch(void* _cache, off_t blockNumber, size_t* _numBlocks)
{
	...
	*_numBlocks = 0;
	...
	BlockPrefetch* blockPrefetch = new BlockPrefetch(cache, blockNumber, numBlocks);

	status_t status = blockPrefetch->Allocate();
	if (status != B_OK || blockPrefetch->NumAllocated() == 0) {
		TRACE(("block_cache_prefetch returning early (%s): allocated %" B_PRIuSIZE "\n",
			strerror(status), blockPrefetch->NumAllocated()));
		delete blockPrefetch;
		return status;
	}

	numBlocks = blockPrefetch->NumAllocated();

	status = blockPrefetch->Read();
