...
ConditionVariableEntry::_RemoveFromVariable()
{
	...
	while (true) {
		if (atomic_pointer_get(&fVariable) == NULL) {
			// The variable must have cleared us out.
		...
		if (try_acquire_spinlock(&variable->fLock) == B_OK)
			break;
	}
...
ConditionVariableEntry::Wait(uint32 flags, bigtime_t timeout)
{
	...
	if (waitStatus <= 0) {
		atomic_set(&fWaitStatus, waitStatus);
		return waitStatus;
	}
...
ConditionVariable::_NotifyLocked(bool all, status_t result)
{
	...
	while (ConditionVariableEntry* entry = fEntries.RemoveHead()) {
		...
		if (thread == NULL) {
			...
			if (atomic_get(&entry->fWaitStatus) == STATUS_WAITING)
				thread_unblock(thread, result);
			atomic_set(&entry->fWaitStatus, result);
