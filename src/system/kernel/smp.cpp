...
smp_intercpu_int_handler(int32 cpu)
{
	...
	TRACE("smp_intercpu_int_handler: done on cpu %d\n", cpu);
	...
	addr_t data3, void* dataPointer, uint32 flags)
{
	...
	TRACE("smp_send_ici: target 0x%x, mess 0x%x, data 0x%lx, data2 0x%lx, "
		"data3 0x%lx, ptr %p, flags 0x%x\n", targetCPU, message, data, data2,
	...
	void *dataPointer, uint32 flags)
{
	...
	TRACE("smp_send_broadcast_ici: cpu %d mess 0x%x, data 0x%lx, data2 "
		"0x%lx, data3 0x%lx, ptr %p, flags 0x%x\n", smp_get_current_cpu(),
	...
	addr_t data, addr_t data2, addr_t data3, void *dataPointer, uint32 flags)
{
	...
	TRACE("smp_send_broadcast_ici_interrupts_disabled: cpu %d mess 0x%x, "
		"data 0x%lx, data2 0x%lx, data3 0x%lx, ptr %p, flags 0x%x\n",
	...
	TRACE("smp_send_broadcast_ici_interrupts_disabled %d: inserting msg %p "
		"into broadcast mbox\n", currentCPU, msg);
	...
	TRACE("smp_send_broadcast_ici_interrupts_disabled %d: sent interrupt\n",
		currentCPU);
	...
	if ((flags & SMP_MSG_FLAG_SYNC) != 0) {
		...
		// the interrupt handler will ref count it to <0
		// if the message is sync after it has removed it from the mailbox
		TRACE("smp_send_broadcast_ici_interrupts_disabled %d: waiting for "
			"ack\n", currentCPU);
		...
		TRACE("smp_send_broadcast_ici_interrupts_disabled %d: returning "
