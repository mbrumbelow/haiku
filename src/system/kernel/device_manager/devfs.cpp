	...
	if (S_ISCHR(vnode->stream.type)) {
		switch (op) {
			...
			case B_TRIM_DEVICE:
			{
				...
				// TODO not so great approach here: we copy the fixed data back
				// to userland, and the underlying device will get it from
				// there again. It's probably possible for disk devices to know
				// if the pointer is already to a kernel side buffer, and not
				...
				return vnode->stream.u.dev.device->Control(
					cookie->device_cookie, op, buffer, length);
			}
