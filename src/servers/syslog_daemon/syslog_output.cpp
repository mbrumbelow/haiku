...
init_syslog_output(SyslogDaemon *daemon)
{
	...
	if (handle != NULL) {
		...
		const char *param = get_driver_parameter(handle, "syslog_max_backups", "1", "1");
		sMaxBackups = strtol(param, NULL, 0);
		if (sMaxBackups < 0)
			sMaxBackups = 0;
