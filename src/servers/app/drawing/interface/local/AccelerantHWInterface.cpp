	...
	for (int32 i = 0; i < fModeCount; i++) {
		...
		int32 diff
			...
			+ abs(mode.timing.h_total * mode.timing.v_total
				...
				/ 100
			+ ((mode.timing.pixel_clock >= compareMode.timing.pixel_clock)
				? (mode.timing.pixel_clock - compareMode.timing.pixel_clock)
				: (compareMode.timing.pixel_clock - mode.timing.pixel_clock))
				/ 100
			+ (int32)(500 * fabs(aspectRatio - compareAspectRatio))
			+ 100 * ((mode.space >= compareMode.space)
				? (mode.space - compareMode.space)
				: (compareMode.space - mode.space));
