...
AccelerantHWInterface::SetMode(const display_mode& mode)
{
	...
	if (fBackBuffer.Get() == NULL || fBackBuffer->Width() != fFrontBuffer->Width()
		...
		if (doubleBuffered) {
			if (fOffscreenBackBuffer) {
				fBackBuffer.SetTo(new(nothrow) AccelerantBuffer(*fFrontBuffer.Get(),
					true));
			} else {
				fBackBuffer.SetTo(new(nothrow) MallocBuffer(fFrontBuffer->Width(),
					fFrontBuffer->Height()));
			}

			status = fBackBuffer.Get() != NULL ? fBackBuffer->InitCheck() : B_NO_MEMORY;
			if (status < B_OK) {
				fBackBuffer.Unset();
				fOffscreenBackBuffer = false;
				return status;
