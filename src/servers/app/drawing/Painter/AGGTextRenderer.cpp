...
public:
	StringRenderer(const IntRect& clippingFrame, uint16 flags,
		...
		fRenderer(renderer)
	{
		fDryRun = (fFlags & STRING_RENDERER_DRY_RUN) != 0;
		fSubpixelAntiAliased = (fFlags & STRING_RENDERER_SUBPIXEL_ANTI_ALIASED) != 0;
	}
	...
	void Finish(double x, double y)
	{
		...
		if (!fDryRun) {
			if ((fFlags & STRING_RENDERER_UNDERSCORE) != 0)
				_DrawHorizontalLine(y + 2);

			if ((fFlags & STRING_RENDERER_STRIKEOUT) != 0) {
...
//printf("RenderString(\"%s\", length: %ld, dry: %d)\n", string, length, dryRun);
	...
	uint16 flags = 0;
	if (dryRun)
		flags |= STRING_RENDERER_DRY_RUN;
	if (gSubpixelAntialiasing && fAntialias)
		flags |= STRING_RENDERER_SUBPIXEL_ANTI_ALIASED;
	if ((fFont.Face() & B_UNDERSCORE_FACE) != 0)
		flags |= STRING_RENDERER_UNDERSCORE;
	if ((fFont.Face() & B_STRIKEOUT_FACE) != 0)
		flags |= STRING_RENDERER_STRIKEOUT;

	StringRenderer renderer(clippingIntFrame, flags, transformedOutline, transformedContourOutline,
