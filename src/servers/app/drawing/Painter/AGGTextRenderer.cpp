...
public:
	...
	void Finish(double x, double y)
	{
		...
		if (fStrikeout && !fDryRun) {
			...
			b.bottom = (int)y;
			b.OffsetBy(fTransformOffset);
			p.move_to(b.left + 0.5, round((b.top + b.bottom) / 2) + 2.5);
			p.line_to(b.right + 0.5, round((b.top + b.bottom) / 2) + 2.5);
			p.close_polygon();
			agg::conv_stroke<agg::path_storage> ps(p);
			ps.width(fRenderer.fFont.Size() / 12.0f);
			if (fRenderer.fMaskedScanline != NULL) {
...
//printf("RenderString(\"%s\", length: %ld, dry: %d)\n", string, length, dryRun);
	...
	bool strikeout = fFont.Face() & B_STRIKEOUT_FACE;

	StringRenderer renderer(clippingIntFrame, dryRun,
		gSubpixelAntialiasing && fAntialias, underscore, strikeout,
