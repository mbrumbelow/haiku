...
ServerApp::_SetDefaultFonts()
{
	int32 cookie = 0;
	image_info info;
	bool isBeOSApp = get_next_image_info(fClientTeam, (int32*)&cookie, &info)
			== B_OK
	...
	if (isBeOSApp) {
		fPlainFont = *gFontManager->BeOSPlainFont();
		fBoldFont = *gFontManager->BeOSBoldFont();
		fFixedFont = *gFontManager->BeOSFixedFont();
	} else {
