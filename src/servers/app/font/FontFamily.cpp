...
FontFamily::StyleAt(int32 index, ServerApp* owner) const
{
	...
	// this can return NULL for a valid index if the style is not owned.
	// For example, a font family may have two styles but each is owned by
	// a different app. System font styles have an owner of NULL.
	if ((styleAt != NULL) && 
