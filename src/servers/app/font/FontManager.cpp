...
FontManager::_AddPath(BEntry& entry, font_directory** _newDirectory)
{
	...
	if (status != B_OK) {
		...
		syslog(LOG_DEBUG, 
			"FontManager: could not watch directory %" B_PRIdDEV ":%" B_PRIdINO "\n",
...
FontFamily*
FontManager::_FindFamily(const char* name, ServerApp* owner) const
{
	if (name == NULL)
		return NULL;

	FontFamily family(name, 0, owner);
...
FontManager::GetFamily(const char* name, ServerApp* owner)
{
	...
	FontFamily* family = _FindFamily(name, owner);
...
FontManager::RemoveStyle(FontStyle* style, ServerApp* owner)
{
	...
	if (family != NULL) {
		...
		if (check != NULL) {
			FTRACE(("FontManager::RemoveStyle style removed: %s but still exists!\n", 
				style->Name()));
...
// TODO: Could possibly refactor _AddFont since this function is very similar
status_t
FontManager::_AddUserFont(FT_Face face, node_ref nodeRef, const char* path, 
	ServerApp* owner, uint16& familyID, uint16& styleID)
{
	FontFamily* family = _FindFamily(face->family_name, owner);
	if (family != NULL && family->Owner() == owner && 
		family->HasStyle(face->style_name, owner)) {
		// prevent adding the same style twice
		// (this indicates a problem with the installed fonts maybe?)
		FT_Done_Face(face);
	...
	if ((family == NULL) || (family->Owner() != owner)) {
		family = new (std::nothrow) FontFamily(face->family_name, fNextID++, owner);
	...
	// the FontStyle takes over ownership of the FT_Face object
	FontStyle* style = new (std::nothrow) FontStyle(nodeRef, path, face, owner);
...
*/
status_t
FontManager::AddUserFontFromFile(const char* path, 
	uint16& familyID, uint16& styleID, ServerApp* owner)
{
	BAutolock locker(this);
	...
	status = _AddUserFont(face, nodeRef, path, owner, familyID, styleID);
...
*/
status_t
FontManager::AddUserFontFromMemory(const area_info* fontAreaInfo, 
	uint16& familyID, uint16& styleID, ServerApp* owner)
{
	BAutolock locker(this);
	...
	FT_Face face;
	FT_Error error = FT_New_Memory_Face(gFreeTypeLibrary, 
		(const FT_Byte*)fontAreaInfo->address, fontAreaInfo->size, 0, &face);
	if (error != 0)
		return error;

	status = _AddUserFont(face, nodeRef, "", owner, familyID, styleID);
...
FontManager::RemoveUserFont(uint16 familyID, uint16 styleID, ServerApp* owner)
{
	...
	FontKey fKey(familyID, styleID, owner);
	FontStyle *styleRef = fStyleHashTable.Get(fKey);
	fStyleHashTable.Remove(fKey);

	FontFamily* family = styleRef->Family();
	bool removed = family->RemoveStyle(styleRef, owner);

	if (!removed)
		syslog(LOG_DEBUG, "FontManager::RemoveUserFont style not removed from family\n");

	int32 numStyles = family->CountStyles();

	if (numStyles == 0) {
		fFamilies.RemoveItem(family);
		delete family;
	} else
		syslog(LOG_DEBUG, "FontManager::RemoveUserFont family still has %d styles!\n", 
			numStyles);
