...
private:
	...
	class TAlertView : public BView {
	  public:
		...
		void _AnimateWaitIcon()
		{
			while (atomic_get(&fAnimationActive)) {
				if (LockLooperWithTimeout(kIconAnimateInterval) != B_OK)
					continue;

				uint8* currentIconBits = (uint8*)fCurrentIconBitmap->Bits();

				int32 width = fCurrentIconBitmap->Bounds().IntegerWidth() + 1;
				int32 rowLength = fCurrentIconBitmap->BytesPerRow();

				if (fCurrentAnimationRow < 0) {
					fCurrentAnimationRow =
						fCurrentIconBitmap->Bounds().IntegerHeight() + 1;
					fAnimationLightenPhase = !fAnimationLightenPhase;
				}

				for (int32 x = 0; x < width; x++) {
					int32 offset = (fCurrentAnimationRow * rowLength) + (x * 4);

					uint8* iconBits = (uint8*)(fAnimationLightenPhase ?
						fNormalIconBitmap->Bits() : fTintedIconBitmap->Bits());

					currentIconBits[offset] = iconBits[offset];
					currentIconBits[offset + 1] = iconBits[offset + 1];
					currentIconBits[offset + 2] = iconBits[offset + 2];
					currentIconBits[offset + 3] = iconBits[offset + 3];
				}
