...
//		tool_bar_icon(kIconDocumentOpen), B_TRANSLATE("Open" B_UTF8_ELLIPSIS));
	...
	viewFrame.bottom -= B_H_SCROLL_BAR_HEIGHT * be_plain_font->Size() / 12.f;
...
ShowImageWindow::_ResizeWindowToImage()
{
	...
	float width = r.Width() + B_V_SCROLL_BAR_WIDTH * be_plain_font->Size() / 12.f;
...
ShowImageWindow::_ToggleFullScreen()
{
	...
	if (fFullScreen) {
		...
		frame.right += B_H_SCROLL_BAR_HEIGHT * be_plain_font->Size() / 12.f;
		frame.bottom += B_H_SCROLL_BAR_HEIGHT * be_plain_font->Size() / 12.f;
