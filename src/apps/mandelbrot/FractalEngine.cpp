...
void FractalEngine::MessageReceived(BMessage* msg)
{
	...
	case MSG_RESIZE: {
		...
		if (fStopRender) {
			...
			TRACE("Breaking out of MSG_RESIZE handler because fStopRender = "
				"true\n");
			break;
		}
		StopRender();
...
void FractalEngine::StopRender()
{
	if (fRenderStopped)
		return;
	fRenderStopped = true;
		// Basically this is true if another call to StopRender() won't work
		// properly
...
void FractalEngine::Render(double locationX, double locationY, double size)
{
	...
	if (fStopRender)
		debugger("Error: Render is not supposed to be called while fStopRender"
			" = true\n");
...
status_t FractalEngine::RenderThread(void* data)
{
	...
	while (true) {
		...
		if (!engine->fStopRender && !initLoop) {
