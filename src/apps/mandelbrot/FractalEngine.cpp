...
void FractalEngine::MessageReceived(BMessage* msg)
{
	...
	case MSG_RESIZE: {
		...
		if (fStopRender) { //Will be true throughout this whole handler. Set false at the end
			TRACE("Breaking out of MSG_RESIZE handler because fStopRender = true\n");
			break;
		}
		StopRender();
	...
	case MSG_THREAD_RENDER_COMPLETE: {
		...
		int32 threadsRendering;
		get_sem_count(fRenderStoppedSem, &threadsRendering);
		if (threadsRendering == fThreadCount) {
...
void FractalEngine::StopRender()
{
	if (fRenderStopped)
		return;
	fRenderStopped = true; //Basically this is true if another call to StopRender() won't work properly
	TRACE("Stopping render...\n");
	fStopRender = true;
	for (uint i = 0; i<fThreadCount; i++)
		acquire_sem(fRenderStoppedSem);

	//note that fStopRender is NOT set to false at the end here. This is to allow the message handlers to use this variable to block duplication of stuff.
...
{
	fRenderStopped = false; //This means that future Render calls will need to call stop render
	if (fStopRender)
		debugger("Error: Render is not supposed to be called while fStopRender = true\n");
...
status_t FractalEngine::RenderThread(void* data)
{
	...
	while (true) {
		...
		for (uint16 y = 0; y<halfHeight; y++) {
			...
			if (engine->fStopRender) { //will return true if should restart
