	...
	fFractalView(new FractalView)
{
	...
	BLayoutBuilder::Menu<>(fMenuBar)
		.AddMenu(B_TRANSLATE("File"))
			.AddItem(B_TRANSLATE("Save Image"), FractalView::MSG_START_SAVE, 'S')
			.AddItem(B_TRANSLATE("Toggle Fullscreen"), MSG_TOGGLE_FULLSCREEN, B_RETURN)
...
void MandelbrotWindow::ToggleFullscreen() {
	BRect frame;
	...
	if ((message->what == B_KEY_DOWN || message->what == B_UNMAPPED_KEY_DOWN)
		...
		if (bytes[0] == B_FUNCTION_KEY) {
			...
			if (message->FindInt32("key", &key) == B_OK) {
				switch (key) {
					case B_F11_KEY: {
						BMessage msg(MSG_TOGGLE_FULLSCREEN);
						PostMessage(&msg);
						break;
					}
