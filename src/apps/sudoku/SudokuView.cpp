	...
	for (uint32 y = 0; y < size; y++) {
		for (uint32 x = 0; x < size; x++) {
			...
			if (value == fValueHintValue)
				backgroundColor = tint_color(ui_color(B_CONTROL_BACKGROUND_COLOR), B_DARKEN_2_TINT);
			...
			if (fField->IsInitialValue(x, y))
				...
				if ((fHintFlags & kMarkInvalid) == 0
					...
					if (fField->IsValueCompleted(value))
						SetHighUIColor(B_SUCCESS_COLOR);
					else {
						if (ui_color(B_CONTROL_TEXT_COLOR).IsDark())
							SetHighUIColor(B_CONTROL_HIGHLIGHT_COLOR, B_DARKEN_1_TINT);
						else
							SetHighUIColor(B_CONTROL_HIGHLIGHT_COLOR, B_LIGHTEN_1_TINT);
					}
				} else
