...
static const rgb_color kIdealValueColor = {0, 91, 162};
static const rgb_color kIdealValueCompletedColor = {55, 140, 35};
static const rgb_color kIdealInvalidValueColor = {200, 0, 0};
			...
			if (fField->IsInitialValue(x, y))
				...
				if ((fHintFlags & kMarkInvalid) == 0
					...
					if (fField->IsValueCompleted(value))
						SetHighColor(mix_color(ui_color(B_CONTROL_TEXT_COLOR), kIdealValueCompletedColor, 191));
					else
						SetHighColor(mix_color(ui_color(B_CONTROL_TEXT_COLOR), kIdealValueColor, 191));
				} else
					SetHighColor(mix_color(ui_color(B_CONTROL_TEXT_COLOR), kIdealInvalidValueColor, 191));
			}
