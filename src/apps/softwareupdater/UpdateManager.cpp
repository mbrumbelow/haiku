...
UpdateManager::UpdateManager(BPackageInstallationLocation location,
	...
	fStatusWindowMessenger(new BMessenger(fStatusWindow)),
	fProblemWindow(NULL),
	fProblemWindowMessenger(NULL),
...
{
	delete fStatusWindowMessenger;
	delete fProblemWindowMessenger;
}
...
UpdateManager::HandleProblems()
{
	if (fProblemWindow == NULL) {
		...
		fProblemWindowMessenger = new BMessenger(fProblemWindow);
	}
