...
public:
	...
	virtual	status_t			ResizeTo(int32 width, int32 height,
			...
			void				GetCellAttributes(int32 row, int32 column,
									Attribute& attributes, uint32& count) const;
			...
			void				GetLineColor(int32 index, Attribute& attr) const;
	...
	inline	Attribute			GetAttributes();
	inline	void				SetAttributes(const Attribute& attributes);
			...
			void				FillScreen(UTF8Char c, Attribute &attr);
...
void
BasicTerminalBuffer::SetAttributes(const Attribute& attributes)
{
