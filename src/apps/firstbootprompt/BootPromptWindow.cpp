	...
	fDefaultKeymapItem(NULL)
{
	...
	IconView* languageIcon = new IconView(B_MINI_ICON);
	languageIcon->SetIcon(data, size, B_MINI_ICON);

	data = (const uint8_t*)res->LoadResource('VICN', "Keymap", &size);
	IconView* keymapIcon = new IconView(B_MINI_ICON);
	keymapIcon->SetIcon(data, size, B_MINI_ICON);
	...
	fKeymapsMenuLabel->SetExplicitMinSize(BSize(
		fKeymapsMenuLabel->StringWidth("Dispostion du clavier") + 16,
		B_SIZE_UNSET));
	...
	fLanguagesListView->SetExplicitMinSize(
		BSize(fLanguagesListView->StringWidth("Português (Brasil)") + 32,
		height));
