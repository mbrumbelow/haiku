...
void
ExpressionTextView::SetValue(BString value, BString decSeparator)
{
	// save the value
	fCurrentValue = value;
	...
	if (value.CountChars() > 3 && stringWidth > viewWidth) {
		...
		int32 exponent = 0;
		int32 offset = value.FindFirst(decSeparator);
		if (offset == B_ERROR) {
			exponent = value.CountChars() - decSeparator.CountChars() - firstDigit;
			value.InsertChars(decSeparator, firstDigit + 1);
		} else {
			if (offset == firstDigit + 1) {
				...
				if (value[firstDigit] != '0' || value[firstDigit + 2] != '0'
					...
					// remove the period
					value.Remove(offset, decSeparator.CountChars());
		...
		while (offset > firstDigit && stringWidth > viewWidth) {
			if (value.CharAt(offset) != decSeparator)
		...
		if (digit >= 5) {
			...
			if (digit == 10) {
				...
				if (value.CharAt(firstDigit + 1) == decSeparator) {
					value.SetByteAt(firstDigit + decSeparator.CountChars(), '0');
					value.RemoveChars(firstDigit, decSeparator.CountChars());
					value.InsertChars(decSeparator, firstDigit);
				}
		...
		if (value.FindFirst(decSeparator) != B_ERROR) {
