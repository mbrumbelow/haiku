...
PowerStatusView::_DrawBattery(BView* view, BRect rect)
{
	...
	} else if((fBatteryInfo.state & BATTERY_CHARGING) == 0
		&& (fBatteryInfo.state & BATTERY_DISCHARGING) == 0
		&& (fBatteryInfo.state & BATTERY_CRITICAL_STATE) == 0) {
...
PowerStatusView::Update(bool force, bool notify)
{
	...
	if ((fBatteryInfo.state & BATTERY_CHARGING) != 0
		&& fPercent >= kFullBatteryPercentage
