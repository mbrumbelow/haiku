	...
	fIsReplicant(false)
{
	...
	BStringView* gpuLabel = _CreateLabel("gpulabel", B_TRANSLATE("Graphics:"));
	...
	fIsReplicant(true)
{
	...
	for (int32 index = 0; index < itemCount; index++) {
		...
		if (name == "uptimetext") {
			...
			else if (name == "gputext")
				fGPUInfoView = dynamic_cast<BStringView*>(view);
...
SysInfoView::_GetRamSize(system_info* sysInfo)
{
	...
	ramSize.SetToFormat(B_TRANSLATE_COMMENT("%d MiB Memory:",
		"2048 MiB Memory:"), ignored_pages(sysInfo) <= 0
			? max_pages(sysInfo) : max_and_ignored_pages(sysInfo));
...
SysInfoView::_GetRamUsage(system_info* sysInfo)
{
	...
	if (inaccessibleMemory <= 0) {
		...
		ramUsage.SetToFormat(
			B_TRANSLATE_COMMENT("%d MiB used (%s) %d MiB inaccessible",
				"326 MiB used (16%) 1 MiB inaccessible"),
			used_pages(sysInfo), data.String(), inaccessibleMemory);
	}
...
BString
SysInfoView::_GetGPUInfo()
{
	BString deviceString;
	accelerant_device_info deviceInfo;
	BScreen screen(Window());
	screen.GetDeviceInfo(&deviceInfo);

	if (deviceInfo.name[0] && deviceInfo.chipset[0]) {
		deviceString.SetToFormat("%s (%s)", deviceInfo.name,
			deviceInfo.chipset);
	} else if (deviceInfo.name[0])
		deviceString = deviceInfo.name;
	else
		deviceString = deviceInfo.chipset;
