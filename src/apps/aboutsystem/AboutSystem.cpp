...
AboutView::Pulse()
{
	...
	if (fGoScroll && system_time() > fLastActionTime + 5000000) {
		...
		if (scrollBar != NULL) {
			...
			if (currentVerticalPos < max && currentVerticalPos == fLastVerticalPos)
				fCreditsView->ScrollBy(0, 6);
			else {
				fLastActionTime = system_time();
	...
	switch (msg->what) {
		...
		case B_MOUSE_DOWN:
		{
			...
			if (msg->FindPoint("where", &where) == B_OK
				...
				&& clicks == 3)
				fGoScroll = true;
			// fall through for normal MouseDown() which provides less information
		}
