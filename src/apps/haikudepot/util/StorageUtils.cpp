	...
	while (result == B_OK &&
		...
		if (result == B_OK) {
			...
			if (remove(directoryEntryPath.Path()) == 0) {
				HDDEBUG1("did delete [%s]", directoryEntryPath.Path())
			} else {
				HDERROR1("unable to delete [%s]", directoryEntryPath.Path())
				result = B_ERROR;
			}
		}
...
StorageUtils::CheckCanWriteTo(const BPath& path)
{
	...
	if (result == B_OK && exists) {
		...
		if (remove(path.Path()) == 0) {
			HDTRACE1("did delete the candidate file [%s]", path.Path())
		} else {
			HDERROR1("unable to delete the candidate file [%s]", path.Path())
			result = B_ERROR;
		}
	}
	...
	if (result == B_OK) {
		...
		if (file.Write(buffer, 16) != 16) {
			HDERROR1("unable to write test data to candidate file [%s]",
				path.Path())
			result = B_ERROR;
		}
	}
	...
	if (result == B_OK) {
		...
		if (file.Read(readBuffer, 16) != 16) {
			HDERROR1("unable to read test data from candidate file [%s]",
				path.Path())
			result = B_ERROR;
		}
		...
		for (int i = 0; result == B_OK && i < 16; i++) {
			if (readBuffer[i] != buffer[i]) {
				HDERROR1("mismatched read..write check on candidate file [%s]",
					path.Path())
				result = B_ERROR;
			}
		}
