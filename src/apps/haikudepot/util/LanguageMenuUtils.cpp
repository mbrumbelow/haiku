...
/* static */ void
LanguageMenuUtils::AddLanguagesToMenu(const LanguageList& languages, BMenu* menu)
{
...
/* static */ void
LanguageMenuUtils::MarkLanguageInMenu(const BString& languageCode, BMenu* menu) {
	if (menu->CountItems() == 0) {
		printf("menu contains no items; not able to set the language");
		return;
	}
...
/* static */ void
LanguageMenuUtils::_AddLanguageToMenu(const BString& code, const BString& name, BMenu* menu)
{
...
{
	LanguageMenuUtils::_AddLanguageToMenu(language->Code(),
		language->Name(), menu);
}
