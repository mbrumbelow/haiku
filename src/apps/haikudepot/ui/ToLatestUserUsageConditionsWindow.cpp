	...
	const UserUsageConditions& userUsageConditions)
{
	...
	BString label = B_TRANSLATE("I am %MinimumAgeYears% years of age or older");
...
ToLatestUserUsageConditionsWindow::_HandleAgree()
{
	...
	if (!ageChecked || !conditionsChecked) {
		BAlert* alert = new(std::nothrow) BAlert(
			...
			B_TRANSLATE("Both the minimum age as well as the usage conditions "
				"must be acknowledged by selecting the checkboxes before "
