...
PackageColumn::DrawField(BField* field, BRect rect, BView* parent)
{
	...
	if (packageIconAndTitleField != NULL) {
		...
		if (showActivated) {
			...
			float minimalWidth = iconSize.Width() + indicatorSize.Width() + sTextMargin
				+ sMinimalTextPart;
		...
		activatedIndicatorRect = BRect(
			BPoint(
				ceilf(titleRect.left + titleTextWidth + (1.5f * textMargin)) + 0.5f,
...
PackageRow::UpdateIconAndTitle()
{
	if (!fPackage.IsSet())
		return;
	BField* field = new PackageIconAndTitleField(fPackage->Name(), fPackage->Title(),
		fPackage->State() == ACTIVATED);
...
PackageListView::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case MSG_UPDATE_PACKAGE:
		{
			...
			if (row != NULL) {
				if ((changes & PKG_CHANGED_TITLE) != 0 || (changes & PKG_CHANGED_STATE) != 0)
