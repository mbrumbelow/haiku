		...
		if (status == B_OK) {
			...
			switch (interface.ErrorCodeFromResponse(info)) {
				...
				case ERROR_CODE_OBJECTNOTFOUND:
					...
					HDINFO0("there was no previous rating for this"
...
RatePackageWindow::_SendRatingThread()
{
	...
	if (repositoryCode.Length() == 0) {
		HDERROR1("unable to find the web app repository code for the local "
			"depot %s",
	...
	if (ratingID.Length() > 0) {
		HDINFO1("will update the existing user rating [%s]", ratingID.String())
		status = interface.UpdateUserRating(ratingID,
			languageCode, comment, stability, rating, active, info);
	} else {
		HDINFO1("will create a new user rating for pkg [%s]", package.String())
