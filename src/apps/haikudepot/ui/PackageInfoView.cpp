...
public:
	...
	void SetPackage(const PackageInfoRef package)
	{
		...
		status_t iconResult = fPackageIconRepository.GetIcon(package->Name(), iconSize.Width(),
			bitmapHolderRef);
...
PackageInfoView::_ScreenshotThumbCoordinate(const PackageInfoRef& package)
{
	...
	uint32 screenshotSizeScaled = MAX(
		static_cast<uint32>(BControlLook::ComposeIconSize(kScreenshotSize).Width()), MAX_IMAGE_SIZE);

	return ScreenshotCoordinate(package->ScreenshotInfoAtIndex(0)->Code(), screenshotSizeScaled,
		screenshotSizeScaled);
}
