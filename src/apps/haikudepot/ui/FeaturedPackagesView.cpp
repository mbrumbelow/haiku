...
// these metrics constants would be at a 1.0 scaling factor
#define HEIGHT_PACKAGE 84.0f
#define SIZE_ICON 64.0f
#define X_POSITION_RATING 350.0f
#define PADDING 8.0f
#define MINIMUM_M_WIDTH_SIZE 10.0f
...
public:
	...
	{
		fHeight = HEIGHT_PACKAGE * SCALING_FACTOR;
		float padding = PADDING * SCALING_FACTOR;

		float xPositionRating = X_POSITION_RATING * SCALING_FACTOR;
...

		{
			...
			float rightTitlePublisherAndChronologicalInfo = fminf(xPositionRating - padding, width);
...
// left, top, right bottom
...

		{
			...
			fRatingStarsRect = BRect(BPoint(xPositionRating + padding,
			...
			if (fRatingStarsRect.right > width)
				...
				float xPositionSummary = xPositionRating + ratingStarsMetrics.Size().Width()
					+ (2.0 * padding);

				fSummaryContainerRect = BRect(xPositionSummary + padding, padding, width - padding,
					fHeight - (padding * 2.0));

				float metadataWidthM = metadataFont->StringWidth("M");

				if (fSummaryContainerRect.Size().Width() < MINIMUM_M_WIDTH_SIZE * metadataWidthM)
