...
// #pragma mark - update / add / remove / clear data
	...
	{
		int32 insertionIndex = _InsertionIndex(package);
		if (insertionIndex >= 0) {
			if (fSelectedIndex >= insertionIndex)
				fSelectedIndex++;
			fPackages.insert(fPackages.begin() + insertionIndex, package);
	...
	int32 _InsertionIndex(const PackageInfoRef& package) const
	{
		if (fPackages.empty())
			return 0;
		return _InsertionIndexLinear(package, 0, fPackages.size() - 1);
	}


	int32 _InsertionIndexLinear(const PackageInfoRef& package, int32 startIndex,
		int32 endIndex) const
	{
		for (int32 i = startIndex; i <= endIndex; i++) {
			int compare = package->Title().Compare(fPackages[i]->Title());
			if (compare < 0)
				return i;
			if (compare == 0) {
				if (package->Name() == fPackages[i]->Name())
					return -1;
						// don't insert because it is already present.
				return i;
			}
		}
		return endIndex + 1;
	...
	*/
	...
	int32 _InsertionIndexBinary(const PackageInfoRef& package, int32 startIndex,
		int32 endIndex) const
	{
		...
		if (endStartSpan < 5)
			return _InsertionIndexLinear(package, startIndex, endIndex);

		int midIndex = startIndex + (endStartSpan / 2);
		int compare = package->Title().Compare(fPackages[midIndex]->Title());

		if (compare == 0) {
			if (package->Name() == fPackages[midIndex]->Name())
				return -1;
					// don't insert because it is already present.
			return midIndex;
		}
		if (compare < 0)
			return _InsertionIndexBinary(package, startIndex, midIndex);
		return _InsertionIndexBinary(package, midIndex, endIndex);
	}
...
// #pragma mark - selection and index handling
	...
	int32 _IndexOfName(const BString& name) const
	{
		...
		for (int32 i = 0; i < fPackages.size(); i++) {
...
// #pragma mark - drawing and rendering
		...
		bool selected)
	{
		...
		if (icon.Get() != NULL) {
			...
			DrawBitmap(bitmap, bitmap->Bounds(), targetRect, B_FILTER_BITMAP_BILINEAR);
		}
	}
