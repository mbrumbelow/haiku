...
TextDocumentView::Paste(BClipboard* clipboard)
{
	...
	if (clip != NULL) {
		...
		if (clip->FindData(
			kMimeTypePlainText, B_MIME_TYPE, &plainTextData, &plainTextDataSize) == B_OK) {

			if (plainTextDataSize > 0) {
				_PasteUnfiltered(static_cast<const char *>(plainTextData),
					static_cast<int32>(plainTextDataSize));
			}
		}
	}
...
*/
void
TextDocumentView::_PasteUnfiltered(const char* str, int32 maxLength)
{
	if (maxLength <= 0)
		return;

	if (TextDocumentView::_AreCharsAllowed(str, maxLength))
		_PasteFiltered(str, maxLength);
	else {
		char* strFiltered = new char[maxLength];
		...
		_PasteFiltered(strFiltered, strFilteredLength);
...
*/
void
TextDocumentView::_PasteFiltered(const char* str, int32 maxLength)
{
...
TextDocumentView::Copy(BClipboard* clipboard)
{
	...
	if (clip != NULL) {
		...
		clip->AddData(kMimeTypePlainText, B_MIME_TYPE, text.String(),
			text.Length());
