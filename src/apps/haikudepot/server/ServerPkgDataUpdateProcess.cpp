...
PackageFillingPkgListener::Handle(DumpExportPkg* pkg)
{
	...
	if (depotInfo != NULL) {
		...
		if (-1 != packageIndex) {
			...
			HDINFO1("[PackageFillingPkgListener] unable to find the pkg [%s]",
				packageName.String())
		}
	} else {
		HDINFO1("[PackageFillingPkgListener] unable to find the depot [%s]",
			fDepotName.String())
	}
...
ServerPkgDataUpdateProcess::RunInternal()
{
	if (_DeriveWebAppRepositorySourceCode().IsEmpty()) {
		HDINFO2("[%s] am not updating data for depot [%s] as there is no"
