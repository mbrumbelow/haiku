	...
	const BPath& path) const
{
	...
	if (file == NULL) {
		HDERROR2("[%s] unable to find the meta data file at [%s]", Name(),
			path.Path())
		return B_FILE_NOT_FOUND;
	}
	...
	const BUrl& url, uint32 redirects, uint32 failures)
{
	...
	HDINFO3("[%s] will stream '%s' to [%s]", Name(), url.UrlString().String(),
		targetFilePath.Path())
	...
	if (BHttpRequest::IsSuccessStatusCode(statusCode)) {
		...
		HDERROR1("[%s] unable to find 'Location' header for redirect", Name())
		return B_IO_ERROR;
	} else {
		if (statusCode == 0 || (statusCode / 100) == 5) {
			HDERROR1("error response from server [%" B_PRId32 "] --> retry...",
				statusCode)
