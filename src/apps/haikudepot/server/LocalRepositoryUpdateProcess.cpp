...
LocalRepositoryUpdateProcess::RunInternal()
{
	...
	if (result == B_OK) {
		HDINFO2("[%s] did update %" B_PRIi32 " local repositories' caches",
...
{
	if (fForce) {
		HDINFO2("[%s] am refreshing cache for repo [%s] as it was forced",
			Name(), repoName.String())
		return true;
	}

	if (roster.GetRepositoryCache(repoName, cache) != B_OK) {
		HDINFO2("[%s] am updating cache for repo [%s] as there was no cache",
			Name(), repoName.String())
		return true;
	}

	if (static_cast<App*>(be_app)->IsFirstRun()) {
		HDINFO2("[%s] am updating cache for repo [%s] as this is the first"
	...
	if (result == B_OK) {
		if (_ShouldRunForRepositoryName(repoName, roster, cache)) {
			try {
				...
				HDINFO2("[%s] did update local repo [%s] cache", Name(),
					repoName.String());
				result = B_OK;
