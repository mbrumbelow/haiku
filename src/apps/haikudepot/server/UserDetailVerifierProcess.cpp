		...
		switch (result) {
			...
			case B_OK:
				if (!userDetail.Agreement().IsLatest()) {
					HDINFO0("the user has not agreed to the latest user usage"
						" conditions.")
...
UserDetailVerifierProcess::_TryFetchUserDetail(UserDetail& userDetail)
{
	...
	if (result != B_OK) {
		HDERROR1("a problem has arisen retrieving the current user detail: %s",
			strerror(result))
	}
	...
	if (result == B_OK) {
		...
		switch (errorCode) {
			...
			case ERROR_CODE_AUTHORIZATIONFAILURE:
				...
				break;
			default:
				HDERROR2("a problem has arisen retrieving the current user "
	...
	if (result == B_OK) {
		...
		if (result != B_OK)
			HDERROR0("it was not possible to unpack the user details.")
	}
