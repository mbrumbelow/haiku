	...
	fCountIconsSet(0)
{
	...
	if (fModel->IconStoragePath(fLocalIconStoragePath) != B_OK) {
		HDINFO1("[%s] unable to obtain the path for storing icons", Name())
...
ServerIconExportUpdateProcess::Populate()
{
	...
	if (Logger::IsInfoEnabled()) {
		...
		HDINFO3("[%s] did populate %" B_PRId32 " packages' icons (%6.3g secs)",
...
ServerIconExportUpdateProcess::_DownloadAndUnpack()
{
	...
	switch (result) {
		case HD_ERR_NOT_MODIFIED:
			HDINFO1("[%s] icons not modified - will use existing", Name())
			return result;
			break;
		case B_OK:
...
ServerIconExportUpdateProcess::_HandleDownloadFailure()
{
	...
	if (result == B_OK) {
		if (hasData) {
			HDINFO1("[%s] failed to update data, but have old data anyway "
...
ServerIconExportUpdateProcess::_Unpack(BPath& tarGzFilePath)
{
	...
	if (result == B_OK) {
		...
		if (result == B_OK) {
			double secs = watch.ElapsedTime() / 1000000.0;
			HDINFO2("[%s] did unpack icon tgz in (%6.3g secs)", Name(), secs)

			if (0 != remove(tarGzFilePath.Path())) {
				HDERROR2("[%s] unable to delete the temporary tgz path; %s",
