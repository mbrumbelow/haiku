	...
	UserUsageConditions& conditions)
{
	...
	if ( (resultMessage.FindString("code", &metaDataCode) != B_OK)
		...
		HDERROR0("unexpected response from server with missing user usage "
			"conditions data")
		return B_BAD_DATA;
	}
	...
	size_t requestDataSize, uint32 flags, BMessage& reply) const
{
	...
	if (ServerSettings::IsClientTooOld()) {
		HDDEBUG1("jrpc; dropping request to ...[%s] as client is too old",
			domain)
		delete requestData;
	...
	switch (statusCode) {
		...
		default:
			HDERROR2("jrpc request to endpoint [.../%s] failed with http "
				"status [%" B_PRId32 "]\n", domain, statusCode)
			return B_ERROR;
	}
