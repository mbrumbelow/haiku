	...
	const BString& userNickname, BMessage& message)
{
	...
	return _SendJsonRequest("user-rating/get-user-rating-by-user-and-pkg-version", requestEnvelopeData,
	...
	const UserCredentials& credentials, BMessage& message)
{
	...
	status_t result = _SendJsonRequest("user/get-user", credentials, requestEnvelopeData,
		_LengthAndSeekToZero(requestEnvelopeData), NEEDS_AUTHORIZATION,
		message);
	...
	const BString& jsonString, uint32 flags, BMessage& reply) const
{
	...
	return _SendJsonRequest(urlPathComponents, data, jsonString.Length() - 1, flags,
		reply);
}
