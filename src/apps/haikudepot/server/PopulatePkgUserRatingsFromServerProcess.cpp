...
PopulatePkgUserRatingsFromServerProcess::RunInternal()
{
	...
	if (status == B_OK) {
		status = fModel->GetWebAppInterface()->RetrieveUserRatingsForPackageForDisplay(
				packageName, webAppRepositoryCode, BString(), 0, PACKAGE_INFO_MAX_USER_RATINGS,
				info);
	...
	if (status == B_OK) {
		status = fModel->GetWebAppInterface()->RetrieveUserRatingSummaryForPackage(
			packageName, webAppRepositoryCode, summaryResponse);
	}
	...
	if (status == B_OK) {
		...
		if (hasData) {
			...
			while(status == B_OK) {
				...
				if (status == B_OK) {
					status = ratingDistributionItem.FindDouble("rating",
