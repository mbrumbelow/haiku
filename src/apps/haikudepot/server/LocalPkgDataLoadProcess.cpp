...
LocalPkgDataLoadProcess::RunInternal()
{
 	...
 	for (int32 i = 0; i < repositoryNames.CountStrings(); i++) {
 		...
 		if (getRepositoryConfigStatus == B_OK) {
 			...
 			HDINFO3("[%s] unable to obtain the repository config for local "
 				"repository '%s'; %s", Name(),
 	...
 	for (int32 i = 0; i < packages.CountItems(); i++) {
 		...
 		if (remoteRepository != NULL) {
 			...
 			if (it == depots.end()) {
 				HDDEBUG2("pkg [%s] repository [%s] not recognized --> ignored",
 	...
 	try {
 		...
 		HDERROR2("Fatal exception occurred while resolving system dependencies: "
 			"%s, details: %s", strerror(ex.Error()), ex.Details().String())
	} catch (BNothingToDoException&) {
 		// do nothing
	} catch (BException& ex) {
 		HDERROR1("Exception occurred while resolving system dependencies: %s",
 			ex.Message().String())
 	} catch (...) {
