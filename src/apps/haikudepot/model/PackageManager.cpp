...
public:
	...
	virtual status_t Perform()
	{
		...
		try {
			...
			HDINFO2("Installation of package %s is aborted by user: %s",
				packageNameString, ex.Message().String())
			_SetDownloadedPackagesState(NONE);
			ref->SetState(state);
			return B_OK;
			...
			HDERROR2("Exception occurred while installing package %s: %s",
				packageNameString, ex.Message().String())
			_SetDownloadedPackagesState(NONE);
			ref->SetState(state);
			return B_ERROR;
...
public:
	...
	virtual status_t Perform()
	{
		...
		try {
			...
			HDERROR2("Exception occurred while uninstalling package %s: %s",
				packageName, ex.Message().String())
			ref->SetState(state);
			return B_ERROR;
		}
...
public:
	...
	virtual status_t Perform()
	{
		...
		if (fDeskbarLink.link.FindFirst('/') == 0) {
			...
			HDINFO1("trying to launch (absolute link): %s", path.Path())
		} else {
