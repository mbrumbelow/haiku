...
{
	into->AddUInt64(KEY_TIMESTAMP_AGREED, fTimestampAgreed);
	into->AddString(KEY_CODE, fCode);
	into->AddBool(KEY_IS_LATEST, fIsLatest);
	return B_OK;
}
...
{
	BMessage agreementMessage;
	if (fAgreement.Archive(&agreementMessage, true) == B_OK) {
		into->AddMessage(KEY_AGREEMENT, &agreementMessage);
	}
	into->AddString(KEY_NICKNAME, fNickname);
	return B_OK;
}
