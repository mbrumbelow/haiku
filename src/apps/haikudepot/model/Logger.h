...
#define HDLOG(L, M, ...) if (Logger::IsLevelEnabled(L)) { \
	HDLOGPREFIX(L) \
	printf(M __VA_OPT__(,) __VA_ARGS__); \
	putchar('\n'); \
}

#define HDINFO(M, ...) HDLOG(LOG_LEVEL_INFO, M, __VA_ARGS__)
#define HDDEBUG(M, ...) HDLOG(LOG_LEVEL_DEBUG, M, __VA_ARGS__)
#define HDTRACE(M, ...) HDLOG(LOG_LEVEL_TRACE, M, __VA_ARGS__)
#define HDERROR(M, ...) HDLOG(LOG_LEVEL_ERROR, M, __VA_ARGS__)
