			...
			if (info.FindMessage("result", &result) == B_OK
				...
				while (true) {
					...
					if (item.FindString("code", &code) != B_OK) {
						HDERROR1("corrupt user rating at index %" B_PRIi32,
							index)
						continue;
					}
					...
					if (item.FindMessage("user", &userInfo) != B_OK
						...
						HDERROR1("ignored user rating [%s] without a user "
					...
					if (comment.Length() == 0 && rating == -1) {
						HDERROR1("rating [%s] has no comment or rating so will"
					...
					HDDEBUG1("rating [%s] retrieved from server", code.String())
				}
				HDDEBUG2("did retrieve %" B_PRIi32 " user ratings for [%s]",
	...
	if (status == B_OK) {
		...
		if (info.FindMessage("result", &result) == B_OK) {
			if (result.FindString("content", &content) == B_OK
				...
				HDDEBUG1("changelog populated for [%s]", packageName.String())
			} else {
				HDDEBUG1("no changelog present for [%s]", packageName.String())
			}
		} else {
...
model_remove_key_for_user(const BString& nickname)
{
	...
	switch (result) {
		case B_OK:
			...
			if (result != B_OK) {
				HDERROR2("error occurred when removing password for nickname "
		...
		case B_ENTRY_NOT_FOUND:
			return;
		default:
			HDERROR2("error occurred when finding password for nickname "
				"[%s] : %s", nickname.String(), strerror(result))
			break;
	}
}
	...
	for (i = 0; i < fDepots.CountItems(); i++) {
		...
		if (depot.WebAppRepositoryCode().Length() == 0) {
			if (depot.URL().Length() > 0) {
				HDINFO2("depot [%s] (%s) correlates with no repository in the"
					" the haiku depot server system", depot.Name().String(),
					depot.URL().String())
			}
			else {
				HDINFO1("depot [%s] correlates with no repository in the"
					" the haiku depot server system", depot.Name().String())
			}
		}
	}
	...
	const char *sourceDescription) const
{
	...
	if (responsePayload.FindMessage("error", &error) == B_OK
		...
		HDERROR1("[%s] --> an undefined error has occurred", sourceDescription)
	}
}
