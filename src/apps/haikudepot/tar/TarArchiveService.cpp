	...
	Stoppable* stoppable)
{
	...
	if (B_OK != result) {
		HDERROR1("error occurred unpacking tar items; %s", strerror(result))
	}
	...
	BPath& targetDirectoryPath, BString &tarItemPath)
{
	...
	for (int32 i = 0; i < components.CountStrings(); i++) {
		...
		if (_ValidatePathComponent(component) != B_OK) {
			HDERROR1("malformed component; [%s]", component.String())
			return B_ERROR;
		}
	}
	...
	BPath& targetFilePath, uint32 length)
{
	...
	if (result != B_OK)
		HDERROR1("unable to unpack item data to; [%s]", targetFilePath.Path())
