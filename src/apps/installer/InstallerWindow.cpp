...
InstallerWindow::_UpdateControls()
{
	...
	// We use a BString instead of a const char* here so that we can
	// benefit from the API's formatting features without memory problems.
	BString statusText;
...
//	fLaunchBootManagerItem->SetEnabled(dstItem != NULL);
	...
	if (!fEncouragedToSetupPartitions && !foundOneSuitableTarget) {
		// Focus the users attention on the DriveSetup button, if they
		// have attempted to proceed with the installation without a
		// usable partition.
