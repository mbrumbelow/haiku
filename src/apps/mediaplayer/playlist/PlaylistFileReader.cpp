...
PlaylistFileReader::IdentifyType(const entry_ref& ref)
{
	BString path(BPath(&ref).Path());
	PlaylistFileType type = PlaylistFileType::unknown;
	if (path.FindLast(".m3u") == path.CountChars() - 4
		|| path.FindLast(".m3u8") == path.CountChars() - 5)
		type = PlaylistFileType::m3u;
	else if (path.FindLast(".pls") == path.CountChars() - 4)
		type = PlaylistFileType::pls;
	return type;
}
	...
	const int32 lastAssignedIndex)
{
	...
	if (lastAssignedIndex != -1)
	{
		int64 lengthInt = strtoll(length.String(), nullptr, 10);
