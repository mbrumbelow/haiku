	...
	fLastEnabledButtons(0)
{
	...
	BMessage* hover = new BMessage(MSG_SEEK_HOVER);
	...
	switch (message->what) {
		...
		case MSG_SEEK_HOVER:
		{
			...
			if (message->FindInt32("hover value", &value) == B_OK) {
				...
				SetToolTipPosition(position, fDurationView->TimeDuration());
			}
			break;
		}
...
void
TransportControlGroup::SetToolTipPosition(bigtime_t position, bigtime_t duration)
{
	if (duration == 0)
		return;
