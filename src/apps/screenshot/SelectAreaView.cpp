...
SelectAreaView::Draw(BRect updateRect)
{
	...
	if (fResizable && frame.Width() > 20 && frame.Height() > 20) {
		FillRect(_GetHandle(DRAG_TOP_LEFT));
		FillRect(_GetHandle(DRAG_TOP_RIGHT));
		FillRect(_GetHandle(DRAG_BOTTOM_LEFT));
		FillRect(_GetHandle(DRAG_BOTTOM_RIGHT));
	}
}
...
SelectAreaView::_CurrentFrame(bool zeroed)
{
	...
	return zeroed ? BRect(B_ORIGIN, bottomRight - topLeft) : BRect(topLeft, bottomRight);
}
