...
const char* kInfoText1 = B_TRANSLATE("Left click and drag to select, "
	"press ENTER or Right click to confirm.");
const char* kInfoText2 = B_TRANSLATE("Right click and drag to directly "
	"save the rect to the clipboard.");
...
SelectionView::Draw(BRect updateRect)
{
	...
	SetHighColor(make_color(255, 255, 255, 200));
	BFont font;
	GetFont(&font);
	font.SetFalseBoldWidth(4);
	SetFont(&font);
	...
	if (SelectionRect().Contains(where)) {
		...
		switch (_MouseOnDragger(where)) {
			...
			case DRAGGER_RIGHT_BOTTOM:
				...
				break;
			default:
				// Do not invalidate the rectangle on right click when used to confirm a selection
				if ((btn & B_PRIMARY_MOUSE_BUTTON) || !SelectionRect().IsValid()) {
					Invalidate();
...
SelectionViewRegion::MouseMoved(BPoint where, uint32 code, const BMessage* message)
{
	if (fDragMode != DRAG_MODE_NONE) {
		...
		invalidateRect.InsetBySelf(-(kDraggerSize + kDraggerSpacing),
			-(kDraggerSize + kDraggerSpacing));
...
SelectionViewRegion::Draw(BRect updateRect)
{
	...
	if (SelectionRect().IsValid()) {
		...
		SetLowColor(0, 0, 0);
		StrokeRect(selection, {0x0F, 0x1E, 0x3C, 0x78, 0xF0, 0xE1, 0xC3, 0x87});
...
{
	if (fSelectionStart == BPoint(-1, -1) && fSelectionEnd == BPoint(-1, -1))
		return BRect(0, 0, -1, -1);
...
SelectionWindow::Show()
{
	...
	bool cursorIsHidden = be_app->IsCursorHidden();
	if (!cursorIsHidden)
		be_app->HideCursor();
