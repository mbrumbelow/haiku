...
SettingsWindow::MessageReceived(BMessage* message)
{
	switch (message->what) {
		...
		{
			if (!fSearchPageMenuCustom->IsMarked()) {
				BString searchString = message->FindString("searchstring");
...
SettingsWindow::_CreateGeneralPage(float spacing)
{
	...
	fSearchPageMenuCustom = new BMenuItem(B_TRANSLATE("Custom"), new BMessage(MSG_SEARCH_PAGE_CHANGED_MENU));
	...
	BMessage* message_google = new BMessage(MSG_SEARCH_PAGE_CHANGED_MENU);
	message_google->AddString("searchstring", "https://google.com/search?q=%s");
	searchPageMenu->AddItem(new BMenuItem("Google", message_google));
	BMessage* message_bing = new BMessage(MSG_SEARCH_PAGE_CHANGED_MENU);
	message_bing->AddString("searchstring", "https://bing.com/search?q=%s");
	searchPageMenu->AddItem(new BMenuItem("Bing", message_bing));
	BMessage* message_yahoo = new BMessage(MSG_SEARCH_PAGE_CHANGED_MENU);
	message_yahoo->AddString("searchstring", "https://search.yahoo.com/search?p=%s");
	searchPageMenu->AddItem(new BMenuItem("Yahoo", message_yahoo));
	BMessage* message_duckduckgo = new BMessage(MSG_SEARCH_PAGE_CHANGED_MENU);
	message_duckduckgo->AddString("searchstring", "https://duckduckgo.com/?q=%s");
	searchPageMenu->AddItem(new BMenuItem("DuckDuckGo", message_duckduckgo));
