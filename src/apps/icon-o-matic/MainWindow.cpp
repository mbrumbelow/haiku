	...
	switch (message->what) {
		...
		case B_PASTE:
		{
			...
			if (clip->HasData("text/plain", B_MIME_TYPE)) {
				...
				if (archive.what == PathListView::selection_archive_code)
					fPathListView->HandlePaste(&archive);
				if (archive.what == ShapeListView::selection_archive_code)
					fShapeListView->HandlePaste(&archive);
				if (archive.what == StyleListView::selection_archive_code)
					fStyleListView->HandlePaste(&archive);
				if (archive.what == TransformerListView::selection_archive_code)
