...
void
SimpleItem::DrawItem(BView* owner, BRect frame, bool complete)
{
	// label
	if (IsSelected())
...
void
SimpleItem::DrawBackground(BView* owner, BRect frame, bool even)
{
	rgb_color bgColor;
	if (!IsEnabled()) {
...
DragSortableListView::SetDropTargetRect(const BMessage* message, BPoint where)
{
	if (AcceptDragMessage(message)) {
		...
		BRect rect = Bounds();
		if (replaceAll) {
			rect.bottom--;	// compensate for scrollbar offset
			fDropRect = rect;
			fDropIndex = -1;
		} else {
...
SimpleListView::Draw(BRect updateRect)
{
	...
	if (firstIndex >= 0) {
		...
		for (int32 i = firstIndex; i <= lastIndex; i++) {
			item = dynamic_cast<SimpleItem*>(ItemAt(i));
			if (item == NULL)
				break;
			itemFrame = ItemFrame(i);
			item->DrawBackground(this, itemFrame, (i % 2) == 0);
			item->DrawItem(this, itemFrame);
...
bool
SimpleListView::InitiateDrag(BPoint where, int32 index, bool selected)
{
	...
	if (dragBitmap && dragBitmap->IsValid()) {
		if (BView* view = new BView(dragBitmap->Bounds(), "helper",
			...
			for (int32 i = 0; i < numItems; i++) {
				selectedIndex = CurrentSelection(i);
				item = dynamic_cast<SimpleItem*>(ItemAt(selectedIndex));
				if (item == NULL)
					break;
