...
PerspectiveBox::MouseDown(BPoint where)
{
	...
	if (fCurrentState) {
		...
		delete fCurrentCommand;
		fCurrentCommand = new PerspectiveCommand(this, fPerspective,
...
PerspectiveBox::Bounds()
{
	...
	BRect r;
	r.left = min4(lt.x, rt.x, lb.x, rb.x);
	r.top = min4(lt.y, rt.y, lb.y, rb.y);
	r.right = max4(lt.x, rt.x, lb.x, rb.x);
	r.bottom = max4(lt.y, rt.y, lb.y, rb.y);
	return r;
}
