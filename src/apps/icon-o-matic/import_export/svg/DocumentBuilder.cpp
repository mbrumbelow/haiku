						   ...
						   const Transformable& transform, Icon* icon)
{
	...
	if (paint->type != NSVG_PAINT_COLOR) {
		...
		// FIXME gradients are not properly converted yet. Figure out the
		// correct algorithm to get what we need here.

		// Convert the base matrix of the gradient to our screenspace
		gradient.FitToBounds(BRect(0, 0, 64, 64));
		// The transform generated by nanosvg is the shape transform multiplied
		// by the inverse of the gradient transform, in other words it is the
		// matrix that converts a screen coordinate into an offset in the
		// gradient. This is the form that is ultimately fed to agg scanline
		// gradient function, but it is not what we need here.
