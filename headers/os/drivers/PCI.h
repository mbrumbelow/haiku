...
//	PCI device-related information
 ...
 typedef struct pci_info {
    ...
    uint8 bus, device, function, revision, class_api, class_sub, class_base, 
        line_size, latency, header_type, bist, reserved;
    union { 
        struct { 
            uint32 cardbus_cis, subsystem_id, subsystem_vendor_id, rom_base, 
                rom_base_pci, rom_size; 
            uint32 base_registers[6], base_registers_pci[6], base_register_sizes[6]; 
            uint8 base_register_flags[6], interrupt_line, interrupt_pin, min_grant, max_latency; 
        }; 
        struct { 
            uint32 base_registers[2], base_registers_pci[2], base_register_sizes[2]; 
            uint8 base_register_flags[2], primary_bus, secondary_bus, subordinate_bus, secondary_latency, 
                io_base, io_limit, secondary_status, memory_base, memory_limit, 
                prefetchable_memory_base, prefetchable_memory_limit, prefetchable_memory_base_upper32, 
                prefetchable_memory_limit_upper32, io_base_upper16, io_limit_upper16, rom_base, 
                rom_base_pci, interrupt_line, interrupt_pin, bridge_control, subsystem_id, 
                subsystem_vendor_id; 
        }; 
        struct { 
   ...
   // for testing only, not final (do not use!)
            ...
            uint8 primary_bus, secondary_bus, subordinate_bus, secondary_latency, reserved; 
            uint32 memory_base, memory_limit, memory_base_upper32, memory_limit_upper32, 
                io_base, io_limit, io_base_upper32, io_limit_upper32, secondary_status, bridge_control; 
...
typedef struct pci_module_info {
    ...
    uint32 (*read_pci_config)(
        ...
        uint8 bus, device, function, size;
        uint16 offset
    );
    ...
    status_t (*find_pci_capability)(
        uint8 bus, device, function, cap_id, *offset
    );
    ...
    status_t (*update_interrupt_line)(
        uint8 bus, device, function, newInterruptLineValue
    );
    status_t (*find_pci_extended_capability)(
        uint8 bus, device, function;
        uint16 cap_id, *offset
    );
    status_t (*get_powerstate)(
        uint8 bus, device, function, *state
    );
    status_t (*set_powerstate)(
        uint8 bus, uint8 device, function, newState
    );
