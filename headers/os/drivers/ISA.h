...
typedef struct {
	ulong				address;					// memory address in little endian, 4
	ushort				transfer_count;				// number of transfers -1 in little endian, 2
	uchar				reserved;					// empty space, 1
	uchar				flag;						// end of link flag, 1
} isa_dma_entry;
...
typedef struct isa_module_info {
	...
	long				(*make_isa_dma_table) (
							...
							long					num_entries			// max number of entries in table
						);
