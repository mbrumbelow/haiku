...
class BFtpResult : public BUrlResult {
			friend class 				BFtpConnection;

public:
										BFtpResult(const BUrl& url);
										BFtpResult(BMessage*);
										BFtpResult(const BFtpResult& other);
										~BFtpResult();
...
public:
	...
	virtual						~BFtpConnection();
			...
			status_t			Connect();
			status_t			Stop();
			const BFtpResponse&	ResultantResponse() const;
