...
typedef struct scsi_periph_callbacks {
	...
	void (*set_capacity)(periph_device_cookie cookie, uint64 capacity, uint32 blockSize,
...
typedef struct scsi_periph_interface {
	...
	status_t (*register_device)(periph_device_cookie cookie, scsi_periph_callbacks* callbacks,
		scsi_device scsiDevice, scsi_device_interface* scsi, device_node* node, bool removable,
	...
	// exec simple command
	status_t (*simple_exec)(scsi_periph_device device, void* cdb, uint8 cdbLength, void* data,
		size_t dataLength, int ccbFlags);

	// *** file handling ***
	// to be called when a new file is opened
	status_t (*handle_open)(scsi_periph_device device, periph_handle_cookie periph_handle,
		scsi_periph_handle* _handle);
	...
	status_t (*read_write)(scsi_periph_device_info* device, scsi_ccb* request, uint64 offset,
		size_t numBlocks, physical_entry* vecs, size_t vecCount, bool isWrite,
		size_t* _bytesTransferred);
	status_t (*io)(scsi_periph_device device, io_operation* operation, size_t* _bytesTransferred);

	// block ioctls
	status_t (*ioctl)(scsi_periph_handle handle, int op, void* buffer, size_t length);
	...
	err_res (*send_start_stop)(scsi_periph_device device, scsi_ccb* request, bool start,
		bool withLoadEject);
