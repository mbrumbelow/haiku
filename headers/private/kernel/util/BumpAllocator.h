...
template<size_t InlineDataSize = (15 * sizeof(void*)), size_t SlabSize = 4096>
class BumpAllocator {
public:
	BumpAllocator()
		:
	...
	void* Allocate(size_t _size)
	{
		...
		uint8* pointer = fCurrentSlab->data + fCurrentSlab->remaining - size;
		fCurrentSlab->remaining -= size;

		Allocation* header = (Allocation*)pointer;
		header->size = size;
		return header->data;
	}
...
#undef FLA_SIZE
	...
	Slab*			fCurrentSlab;
	uint8			fInlineData[InlineDataSize];
};
